...
static int __cpuinit hifone_boot_secondary(unsigned int cpu,
					struct task_struct *idle)
{
	unsigned long timeout;

	prepare_slave_cores_boot(0xFFFF0000,
		(unsigned int)virt_to_phys(hifone_secondary_startup));
...
static void __cpuinit hifone_secondary_init(unsigned int cpu)
{
	/*
...
static void __init hifone_smp_init_cpus(void)
{
...
void slave_cores_power_up(int cpu)
{
	...
	writel(0x306, (void __iomem *)IO_ADDRESS(REG_BASE_CPU_LP));
	writel(0x706, (void __iomem *)IO_ADDRESS(REG_BASE_CPU_LP));

	/* Enable the slave cpu clock */
	regval = readl((void __iomem *)IO_ADDRESS(CPU_REG_BASE_RST));
	regval |= 1 << (cpu + CPU_REG_CKEN);
	writel(regval, (void __iomem *)IO_ADDRESS(CPU_REG_BASE_RST));
	...
	regval = readl((void __iomem *)IO_ADDRESS(CPU_REG_BASE_RST));
	regval &= ~(1 << (cpu + CPU_REG_COREPDBG_SRST));
	writel(regval, (void __iomem *)IO_ADDRESS(CPU_REG_BASE_RST));
...
static int hifone_cpu_kill(unsigned int cpu)
{
	unsigned int regval;
...
struct smp_operations hifone_smp_ops __initdata = {
	.smp_init_cpus = hifone_smp_init_cpus,
	.smp_secondary_init = hifone_secondary_init,
	.smp_boot_secondary = hifone_boot_secondary,
	.cpu_kill = hifone_cpu_kill,
#ifdef CONFIG_HOTPLUG_CPU
	.cpu_die = hifone_cpu_die,
#endif
};
