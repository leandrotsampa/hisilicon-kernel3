	...
	for(i = 0; i < 4; i++){
		...
		if((param_type == TEEC_MEMREF_TEMP_INPUT) ||
		   ...
		   (param_type  == TEEC_MEMREF_TEMP_INOUT)){
            ...
            //start:modified by fangjian00208632, 2015-07-14
            //fixed GP 3.2.5 Variable Length Return Buffers,TEEC_ERROR_SHORT_BUFFER
            if (buffer_size > 0)
            {
            ...
            //end:modified by fangjian00208632, 2015-07-14
			if((param_type == TEEC_MEMREF_TEMP_INPUT) ||
			   (param_type  == TEEC_MEMREF_TEMP_INOUT)){
            ...
            if (!temp_buf)
            {
                operation->params[i].memref.buffer = NULL;
            }
            else
            {
			  ...
			  bool incomplete)
{
	...
	int i;
	uint32_t buffer_size = 0;
    uint32_t temp_buffer_size = 0;//added by fangjian : fixed bug
	...
	for(i = 0; i < 4; i++){
		...
		if((param_type ==  TEEC_MEMREF_TEMP_OUTPUT)||
		   (param_type  == TEEC_MEMREF_TEMP_INOUT)){
		    temp_buffer_size = *(uint32_t *)client_param->memref.size_addr;
			/* temp buffer */
			...
			if(!incomplete && temp_buffer_size <
