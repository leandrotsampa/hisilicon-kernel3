...
static int rpmb_agent_open_blkdev(struct tee_agent_kernel_ops* agent_instance, int stage)
{
    ...
    if (IS_ERR(bdev)) {
        TCERR("open %s failed with %d in stage-%d\n",
...
#ifdef CA_CRASH_DEBUG
    event_data = find_event_control(agent_instance->agent_id);

    memcpy(lock_info.uuid, phys_to_virt(event_data->cmd.uuid_phys) + 1, sizeof(lock_info.uuid));
    lock_info.context_id = event_data->cmd.context_id;
    lock_info.lock_need_free = true;
    TCDEBUG("rpmb counter lock context: uuid=%s, cxtID=%d\n",
        lock_info.uuid, lock_info.context_id);
#endif
    return 0;
}
...
static void send_ioccmd(struct tee_agent_kernel_ops* agent_instance)
{
    ...
    for (i=0;i<MMC_IOC_MAX_RPMB_CMD;i++) {
        rpmb_data->data[i].buf = ioc_rpmb->cmds[i].data_ptr;
...
static int rpmb_agent_crash_work(struct tee_agent_kernel_ops* agent_instance, TC_NS_ClientContext *context)
{
    TCDEBUG("check free lock or not, uuid=%s, sessID=%d\n",
        context->uuid, context->session_id);
    if (lock_info.lock_need_free &&
        (memcmp(lock_info.uuid, context->uuid, sizeof(lock_info.uuid)) == 0) &&
        lock_info.context_id == context->session_id) {
        TCDEBUG("CA crash, need to free lock\n");
        process_rpmb_unlock(agent_instance);
    }
    return 0;
}
