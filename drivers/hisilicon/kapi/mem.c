...
{
#if !defined(CONFIG_ARCH_DISCARD_MEMBLOCK)
	return (unsigned long)memblock.memory.total_size;
#else
	/* non-arm platform, such as sh/x86/mips/ia64/score */
	return 0;
...
int get_mem_size(unsigned int *size, int flags)
{
	...
	case HIKAPI_GET_RAM_SIZE:
	{
		...
		if (!ramsize)
			ramsize = (unsigned int)(get_dram_size() >> 20);
		if (size)
			*size = ramsize;
		break;
	}
