...
static int sdcardfs_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	if (make_nomedia_in_obb ||
		...
		if (!strncasecmp(nomedia_dir_name, "/mnt/mmcblk1/mmcblk1p1",
		                 sizeof("/mnt/mmcblk1/mmcblk1p1")-1)){
			char *p = nomedia_dir_name+strlen("/mnt/mmcblk1/mmcblk1p1");
			strcpy(nomedia_fullpath,"/storage/sdcard");
			strcat(nomedia_fullpath,p);
		} else if (!strncasecmp(nomedia_dir_name, "/mnt/sda/sda1",
		                        sizeof("/mnt/sda/sda1")-1)) {
			char *r = nomedia_dir_name+strlen("/mnt/sda/sda1");
			strcpy(nomedia_fullpath,"/storage/sdcard");
			strcat(nomedia_fullpath,r);
		} else if (!strncasecmp(nomedia_dir_name, "/data/media/0",
		                        sizeof("/data/media/0")-1)) {
			char *q = nomedia_dir_name+strlen("/data/media/0");
			strcpy(nomedia_fullpath,"/storage/sdcard");
			strcat(nomedia_fullpath,q);
		}
