	...
	if (!err) {
		...
		switch(sb_info->options.derive) {
			...
			case DERIVE_LEGACY:
				...
				if(!strncasecmp("/data/media",dev_name,sizeof(dev_name)-1)){
					setup_derived_state(sb->s_root->d_inode,
						PERM_LEGACY_PRE_ROOT, 0, AID_MEDIA_RW, AID_SDCARD_R, 00751);
				}else{
					setup_derived_state(sb->s_root->d_inode,
						PERM_LEGACY_PRE_ROOT, 0, AID_MEDIA_RW, AID_SDCARD_R, 00771);
				}
