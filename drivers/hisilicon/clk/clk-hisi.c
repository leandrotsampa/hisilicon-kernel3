...
static void __hisi_clk_enable(struct hiclk_hw *clk)
{
	if (clk->value) {
		...
		if ((val & clk->mask) != clk->value)
			pr_warning("enable '%s' clock fail: address:%#lx want:%#x, real:%#x\n",
				clk->name, (unsigned long)clk->peri_crgx, clk->value, val);
	}
...
int hiclk_enable(struct clk_hw *hw)
{
	...
	pr_debug("%s: %s offset:%#x peri_crgx:%#x\n",
		__func__, clk->name, clk->offset, (u32)clk->peri_crgx);
			      ...
			      int nr_hw, int clk_num, struct clk_ops *clkops)
{
	...
	pr_debug("%s: peri_crg_base:%#x\n", __func__, (u32)peri_crg_base);
	...
	pr_debug("%s: peri_ctrl_base:%#x\n", __func__, (u32)peri_ctrl_base);
