									...
									HDMI_SINK_CAPABILITY_S *pstDrvCap)
{
    ...
    pstExtCap->stMfrsInfo.u8MfrsName[0]     = pstDrvCap->stMfrsInfo.u8MfrsName[0];
    pstExtCap->stMfrsInfo.u8MfrsName[1]     = pstDrvCap->stMfrsInfo.u8MfrsName[1];
    pstExtCap->stMfrsInfo.u8MfrsName[2]     = pstDrvCap->stMfrsInfo.u8MfrsName[2];
    pstExtCap->stMfrsInfo.u8MfrsName[3]     = pstDrvCap->stMfrsInfo.u8MfrsName[3];
    pstExtCap->stMfrsInfo.u32ProductCode    = pstDrvCap->stMfrsInfo.u32ProductCode ;    
    pstExtCap->stMfrsInfo.u32SerialNumber   = pstDrvCap->stMfrsInfo.u32SerialNumber ;     
    pstExtCap->stMfrsInfo.u32Week           = pstDrvCap->stMfrsInfo.u32Week ;             
    pstExtCap->stMfrsInfo.u32Year           = pstDrvCap->stMfrsInfo.u32Year ;   
    ...
    if((pstDrvCap->stHdrCap.stEotf.bEotfSdr == HI_TRUE ||
        ...
        pstDrvCap->stColorSpace.bYCbCr422 == HI_TRUE)
    {
        pstExtCap->bHdrSupport = HI_TRUE;
    }
    else
    {
        pstExtCap->bHdrSupport = HI_FALSE;
    }

    pstExtCap->bDolbySupport = (pstDrvCap->stDolbyCap.bYUV422_12Bit == HI_TRUE) ? HI_TRUE : HI_FALSE;
...
static HI_S32 Disp2HdmiVoAttr(HDMI_DEVICE_S* pstHdmiDev, HDMI_VO_ATTR_S *pstDestAttr, HDMI_VIDEO_ATTR_S *pstSrcAttr)
{
    ...
    */
    ...
    else if (pstDestAttr->enVideoTiming  > HDMI_VIDEO_TIMING_720X576P_50000
    ...
    if(HDMI_VIDEO_TIMING_640X480P_60000 == pstDestAttr->enVideoTiming)
    {
        pstDestAttr->enRGBQuantization = HDMI_QUANTIZATION_RANGE_LIMITED;
    }

    if(   HI_DRV_CS_BT2020_YUV_LIMITED == pstSrcAttr->u32ColorSpace
       || HI_DRV_CS_BT2020_YUV_FULL == pstSrcAttr->u32ColorSpace
       || HI_DRV_CS_BT2020_RGB_LIMITED == pstSrcAttr->u32ColorSpace
       || HI_DRV_CS_BT2020_RGB_FULL == pstSrcAttr->u32ColorSpace)
    {
        pstDestAttr->enColorimetry = HDMI_COLORIMETRY__EXTENDED;
        switch(pstSrcAttr->enBt2020Lum)
        {
            case HI_DRV_BT2020_CONSTANCE_LUMINACE_SYS:
                pstDestAttr->enExtendedColorimetry = HDMI_EXTENDED_COLORIMETRY_2020_non_const_luminous;
            case HI_DRV_BT2020_NON_CONSTANCE_LUMINACE_SYS:
                pstDestAttr->enExtendedColorimetry = HDMI_EXTENDED_COLORIMETRY_2020_const_luminous;
            default:
                break;
        }
    }
...
HI_S32 HI_DRV_HDMI_SetHdrMode(HI_UNF_HDMI_ID_E enHdmi, HI_DRV_HDMI_HDR_USERMODE_E enUserHdrMode)
{
    ...
    pstHdmiDev->stAttr.stHdrAttr.enUserHdrMode = enUserHdrMode;
...
HI_S32 HI_DRV_HDMI_SetHdrAttr(HI_UNF_HDMI_ID_E enHdmi, HI_DRV_HDMI_HDR_ATTR_S *pstHdrAttr)
{
    ...
    pstHdmiDev = GetHdmiDevice(enHdmi);
    if(pstHdrAttr == HI_NULL || pstHdmiDev == HI_NULL)
    {
        return HI_FAILURE;
    }
