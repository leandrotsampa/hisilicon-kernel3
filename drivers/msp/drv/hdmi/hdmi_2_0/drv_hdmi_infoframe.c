...
HI_S32 DRV_HDMI_DRMInfoFrameSend(HDMI_INFOFRAME_S* pstInfoFrame, HI_BOOL bEnable)
{
    ...
    memset(pstDRMInfoframe, 0, sizeof(HDMI_DRM_INFOFRAME_S));
    ...
    memcpy(&uInfoframe.stDynRanInfoFrame, pstDRMInfoframe, sizeof(HDMI_DRM_INFOFRAME_S));
    ...
    switch(pstHdrAttr->enHdrMode)
    {
        ...
        case HDMI_HDR_MODE_CEA_861_3_AUTHEN:
            if(pstHdrAttr->enColorimetry > HDMI_HDR_COLORIMETRY_EXTENDED)
            {
                ...
                pstAVIInfoframe->extended_colorimetry = pstHdrAttr->enColorimetry - HDMI_HDR_EXTENDED_COLORIMETRY_XV_YCC_601;
            }
            else
            {
                switch(pstHdrAttr->enColorimetry)
                {
                    case HDMI_HDR_COLORIMETRY_NONE:
                        pstAVIInfoframe->colorimetry = HDMI_COLORIMETRY__NONE;
                        break;
                    case HDMI_HDR_COLORIMETRY_ITU_601:
                        pstAVIInfoframe->colorimetry = HDMI_COLORIMETRY__ITU_601;
                        break;
                    case HDMI_HDR_COLORIMETRY_ITU_709:
                       pstAVIInfoframe->colorimetry = HDMI_COLORIMETRY__ITU_709;
                        break;
