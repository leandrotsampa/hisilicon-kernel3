...
const static HI_CHAR *s_eBitDepth2Str[] = {
	...
	"UNKNOW"       //HDMI_VIDEO_BITDEPTH_BUTT
};
...
//    PROC_PRINT(pFile, "-------------------------- CECtatus ----------------------------\n");
	...
	/* line 1 */
    PROC_PRINT(pFile, "%-15s: %-20s  ", "CECEnable", pstCecInfo->bCecIsOn? STR_YES: STR_NO);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "CECStatus", pstCecInfo->bCecWasOn? STR_ON: STR_OFF);
...
//    PROC_PRINT(pFile, "-------------------------- HDCPtatus ----------------------------\n");
    ...
    /* line 1 */ 
    if(pstHdcpInfo->eHdcp22Support != HDMI_HDCP22_UNKNOWN)
    {
        PROC_PRINT(pFile, "%-15s: %-20s  ", "HDCP2.2Support", pstHdcpInfo->eHdcp22Support == HDMI_HDCP22_SUPPORT ? STR_YES : STR_NO);
    }
    else
    {
        PROC_PRINT(pFile, "%-15s: %-20s  ", "HDCP2.2Support", STR_NA);
    }
    PROC_PRINT(pFile, "%-15s: %-20s \n", "HDCPEnable", pstHdcpInfo->bHdcpIsOn? STR_YES: STR_NO);
    PROC_PRINT(pFile, "%-15s: %-20s  ", "HDCPIsOn", pstHdcpInfo->bHdcpWasOn? STR_YES: STR_NO);

    /* line 2 */
    CHECK_MAX_VALUE(u32Index, pstHdcpInfo->enHdcpMode, HDMI_HDCP_MODE_BUTT);
    PROC_PRINT(pFile, "%-15s: %-20s \n", "HDCPMode", s_eHdcpMode2Str[u32Index]);
    PROC_PRINT(pFile, "%-15s: %-20s  ", "KeyLoad", pstHdcpInfo->bHdcp1Prep? STR_YES: STR_NO);
...
static HI_VOID Proc_AudioInfo(struct seq_file *pFile, HDMI_DEVICE_ID_E enHdmiID)
{
	...
	/* line 15 */
	PROC_PRINT(pFile, "%-15s: %-20s |", "CTS", STR_NA);
	PROC_PRINT(pFile, "\n");

	/* line 16 */
	PROC_PRINT(pFile, "%-15s: %-20s |", "N", STR_NA);
	PROC_PRINT(pFile, "\n");
...
static HI_VOID Proc_SinkBasicCap(struct seq_file *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    ...
    /* line 4 */
    PROC_PRINT(pFile, "%-20s: %-20s ", "HDCP1.4Support", STR_NA);
    PROC_PRINT(pFile, "%-20s: %-20u \n", "SerialNumber", pstCap->stMfrsInfo.u32SerialNumber);
...
static HI_VOID Proc_SinkHdrCap(struct seq_file *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    ...
    PROC_PRINT(pFile,"%-20s: 0x%-20x\n", "DolbyOUI", pstCap->stDolbyCap.u32DolbyOui);
    /* line 1 */
    ...
    /* line 6 */
    PROC_PRINT(pFile,"%-20s: %-20d ", "DolbyWhite_X", pstCap->stDolbyCap.u16White_X);
    PROC_PRINT(pFile,"%-20s: %-20d\n", "DolbyWhite_Y", pstCap->stDolbyCap.u16White_Y);
    /* line 7 */
