...
{
	HI_BOOL     bYccQrangeSelectable;   /**<flag of RGB Quantization Range selectable.when HI_TRUE,sink expect full/limited range is selectable;or sink expect default range.  *//**<CNcomment:Sink??RGB???????/?? ?????HI_TRUE??sink??????,HI_FALSE??sink???????*/
    HI_BOOL     bRgbQrangeSelectable;   /**<flag of YCC Quantization Range selectable.when HI_TRUE,sink expect full/limited range is selectable;or sink expect default range.  *//**<CNcomment:Sink??YCC???????/?? ?????HI_TRUE??sink??????,HI_FALSE??sink???????*/
    ...
    HI_U16      u16White_X;             /* white point chromaticity coordinate x     ,bit[11:0]valid.Real value =SUM OF bit[N]*2^-(12-N) */
    HI_U16      u16White_Y;             /* white point  chromaticity coordinate y    ,bit[11:0]valid.Real value =SUM OF bit[N]*2^-(12-N) */
...
typedef struct
{
    ...
    HDMI_EDID_HDR_CAP_S             stHdrCap;                  /**<HDR capability*//**<CNcomment:HDR ??? */
