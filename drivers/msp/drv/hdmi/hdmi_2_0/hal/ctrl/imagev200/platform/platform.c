...
int16_t getAvailableDevice(void)
{
    ...
    SII_PRINTF("No Free Aardvark Device\n");
    return -1;
...
void BB_Version( void )
{
    ...
    SII_PRINTF( "BlackBox version %lx.%02lx.%02lx.%02lx\n", 
        dwVersion/0x1000000,
...
int BB_SetI2C100kHz( BB_HANDLE handle )
{
    ...
    if( I2C_GetOptions( handle, &i2cOptions ) != BB_SUCCESS){
        SII_PRINTF( "I2C_GetOptions error\n" );
        ...
        if( I2C_SetOptions( handle, i2cOptions ) != BB_SUCCESS){
            SII_PRINTF( "I2C_SetOptions error\n" );
            return SII_PLATFORM_STATUS__FAILED;
        }
    }
        ...
        case SII_PLATFORM_HARDWARE__BB:
			{
				...
				BlackBox_Number( &num );
				if( num == 0 ){
					SII_PRINTF( "No BlackBox connected\n" );
					return SII_PLATFORM_STATUS__FAILED;
				}
				...
				if(num <= 0)
				{
					SII_PRINTF("None of the Balckbox could be opened\n");
					return SII_PLATFORM_STATUS__FAILED;
				}
        ...
        case SII_PLATFORM_HARDWARE__AARDVARK:
            {
                ...
                if (port >= 0)
                {
                    ...
                    if (pInterfaceInfo->handle <= 0) {
		                SII_PRINTF("Unable to open Aardvark device on port %d\n",getAvailableDevice());
                        SII_PRINTF("Error code = %d\n", pInterfaceInfo->handle);
                        return SII_PLATFORM_STATUS__FAILED;
                    }
                    //SII_PRINTF("Opened Aardvark; features = 0x%02x\n", aaext.features);
...
SiiPlatformStatus_t SiiPlatformRead(SiiPlatformInterface_t *pInterfaceInfo, uint8_t devId, uint16_t address, uint8_t *data_in, uint16_t length, uint32_t *pu32VirtualBaseAddr)
{
	...
	case SII_PLATFORM_HARDWARE__BB:
		...
		case SII_PLATFORM_TYPE__PRIF_OVER_I2C:
            ...
            {
                SII_PRINTF("failed to read 0x%x - 0x%x\n", devId, address);
				return SII_PLATFORM_STATUS__FAILED;
            }
			else
				return SII_PLATFORM_STATUS__SUCCESS;
		...
		case SII_PLATFORM_TYPE__SPI:
            {
                ...
                if (num_bytes_written < 0) {
                    ...
                    SII_PRINTF("error: only a partial number of bytes written\n");
                    SII_PRINTF("  (%d) instead of full (%d)\n", num_bytes_written, 2);
                    return SII_PLATFORM_STATUS__FAILED;
                }
...
SiiPlatformStatus_t SiiPlatformWrite(SiiPlatformInterface_t *pInterfaceInfo, uint8_t devId, uint16_t address, uint8_t *data_out, uint16_t length, uint32_t *pu32VirtualBaseAddr)
{
	...
	case SII_PLATFORM_HARDWARE__BB:
		...
		case SII_PLATFORM_TYPE__PRIF_OVER_I2C:
            ...
            {
                SII_PRINTF("failed to write 0x%x - 0x%x\n", devId, address);
				return SII_PLATFORM_STATUS__FAILED;
            }
			else
				return SII_PLATFORM_STATUS__SUCCESS;
