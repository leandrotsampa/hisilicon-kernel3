...
HI_S32 LogPrintOutOfMem(HI_VOID)
{
    ...
    while(left >= 0)
    {
        ...
        else
        {
            ...
            if(((pos - pstMem->pVirAddr) >= 0) && (pos - pstMem->pVirAddr < LOG_HEAD_INFO_DATA_LEN))
            {
        ...
        if ((u32FuncNum < LOG_TYPE_FUNC_BUTT) && 
            (u32StrLen > 0 && u32StrLen < LOG_MEM_BUF_SIZE) &&
            (u8DbgLevel < LOG_LEVEL_BUTT))
        {
            if (!((1 << u32FuncNum) & pLogCtx->u32Mask) && u8DbgLevel <= pLogCtx->u8Level)
            {
                HI_U8 buf_out[LOG_MEM_BUF_SIZE + LOG_INFO_HEAD_SIZE + 8] = {0};
                switch (u8DbgLevel)
                {
                    case LOG_LEVEL_INFO:
                        HDMI_OSAL_Strncpy(buf_out, "INFO : ", 7);
                        break;
                    case LOG_LEVEL_WARN:
                        HDMI_OSAL_Strncpy(buf_out, "WARN : ", 7);
                        break;
                    case LOG_LEVEL_ERROR:
                        HDMI_OSAL_Strncpy(buf_out, "ERR  : ", 7);
                        break;
                    case LOG_LEVEL_FATAL:
                        HDMI_OSAL_Strncpy(buf_out, "FATAL: ", 7);
                        break;
                    default:
                        HDMI_OSAL_Strncpy(buf_out, "UNKNOW:", 7);
                        break;
                }
                ...
                switch (pLogCtx->eMode)
                {
                    ...
                    case LOG_PRINT_TO_ALL:
                        ...
                        if(HI_NULL != pLogCtx->pFile)
                        {
                            ret = HDMI_OSAL_FileWrite(pLogCtx->pFile, buf_out, OSAL_STRLEN(buf_out));
                            if (ret <= 0)
                            {
            ...
            else
            {
                if(right > 0)
                {
                    right = pstMem->pVirAddr + pstMem->u32MemSize - pmem;
                }
                else
                {
                    left = pstMem->pPos - pmem;
                }
            }
        }
        else
