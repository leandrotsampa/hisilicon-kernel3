											...
											HI_U8 u8Len)
{
    ...
    pstDolbyCap->u16White_X     = (pau8Data[14] << 4) | ((pau8Data[13] >> 4) & EDID_DOLBY_LOWER_NIBBLE_MASK);
    pstDolbyCap->u16White_Y     = (pau8Data[15] << 4) | (pau8Data[13] & EDID_DOLBY_LOWER_NIBBLE_MASK);
														...
														HI_U8 u8Len)
{
    ...
    {	
        ...
        case EDID_DOLBY_VENDOR_SPECIFIC_VIDEO_DB:
                u32Ret = EdidDolbyVSVDB(pstCap, pau8Data, u8Len);
        ...
        case    EDID_HDR_STATIC_METADATA_DB:
                u32Ret = EdidHdrMetaDB(pstCap, pau8Data, u8Len);
				EDID_SUCCESS_CHK(u32Ret);
                break;
									...
									HDMI_EDID_UPDATE_MODE_E enMode)
{
	...
	if ( HDMI_EDID_UPDATE_SINK == enMode )
	{
        ...
        if (stHwStatus.stCommonStatus.bHotPlug || stHwStatus.stCommonStatus.bRsen)
        {
									 				...
									 				HDMI_SINK_CAPABILITY_S **pstCapDest)
{
	...
	*pstCapDest = &pstEdidInfo->stCapability;
	...
	{
		return (pstStatus->bCapSink) ? HDMI_EDID_DATA_VALIDSINK : HDMI_EDID_DATA_VALIDTEST;
	}
	else
	{
