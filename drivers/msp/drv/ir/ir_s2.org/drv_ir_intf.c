...
#define CFG_LINUX_INPUT

#if defined (CFG_LINUX_INPUT)
#include "hiir_ir2.h"
#endif
...
static int ir_request_resource(struct ir_priv *ir)
{
    ...
    ret = -EFAULT;
    if (request_irq(ir->irq, ir->callbacks.isr_handle, /*IRQF_DISABLED*/ 0, "hi_ir_s2_irq", (void *)ir))
    {
        goto irq;
    }
	else
...
#if defined (CFG_LINUX_INPUT)
	ret = android_ir_register_inputdevice();
	if (ret) {
		hiir_error("register platform driver failed!");
		goto _error_register_android_ir;
	}
#endif
...
#if defined (CFG_LINUX_INPUT)
	android_ir_unregister_inputdevice();
#endif
