...
rZsbndhFQMuwjRkk3PPlsRutevs/dZ3biTEmYHbW1ydJsAraBjBiw+AqB2DaCWt1WGkF3g1Z
RBitp80Vv3JWQz2S7gJJPwhHEa/CWNA5D0TESTH+Mr5lE3qtCUgQj2CJa8F47Q==#*/
...
static int decode_user_data(MpegEncContext *s, GetBitContext *gb)
{
    ...
    /* divx detection */
    e = sscanf(buf, "DivX%4dBuild%4d%c", &ver, &build, &last);

    if (e < 2)
    { e = sscanf(buf, "DivX%4db%4d%c", &ver, &build, &last); }
    ...
    /* ffmpeg detection */
    e = sscanf(buf, "FFmpe%*[^b]b%4d", &build) + 3;

    if (e != 4)
    { e = sscanf(buf, "FFmpeg v%4d.%4d.%4d / libavcodec build: %d", &ver, &ver2, &ver3, &build); }

    if (e != 4)
    {
        e = sscanf(buf, "Lavc%4d.%4d.%4d", &ver, &ver2, &ver3) + 1;
