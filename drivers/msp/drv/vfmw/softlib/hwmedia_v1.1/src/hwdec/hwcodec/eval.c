...
vtZ3Tb+MKyBRiUXAvv9+stlybNk2QoCbTJO8dKdDjuxEwjcpjt9N+qwYorHZfhayN2Acl6sN
999Q3iX5d20ScKMyWW68i9k/W45nmPI0xJKv5Pnm8l84bXnenEsZVdi0VV/G9w==#*/
...
static double eval_expr(Parser *p, AVEvalExpr *e)
{
    switch (e->type)
    {
        ...
        case e_const:
            return e->value * p->const_value[e->a.const_index];
        ...
        case e_gauss:
        { double d = eval_expr(p, e->param[0]); return exp(-d * d / 2) / sqrt(2 * M_PI); }

        case e_ld:
            return e->value * p->var[av_clip((int)eval_expr(p, e->param[0]), 0, VARS - 1)];
        ...
        default:
        {
            ...
            switch (e->type)
            {
                ...
                case e_st :
                    return e->value * (p->var[av_clip((int)d, 0, VARS - 1)] = d2);
            }
        }
    }
