    ...
    if (!CONFIG_GRAY || !(s->flags & CODEC_FLAG_GRAY))
    {
...

        {
            ...
            if (!iEgdeMcFlag)
            {
                ...
                if ((ptr_cb + uvlinesize * (h >> s->chroma_y_shift)) > (ptr + uvlinesize * ((s->height + 32) >> s->chroma_y_shift) - align))
                {
                    av_log(s->avctx, AV_LOG_WARNING,
                           "MPEG ref picture  wrong\n");
                    IMEDIA_SET_ERR_MB(s->avctx->iErrorCode, IMEDIA_ERR_MB_REF);
...

        {
            ...
            if (!iEgdeMcFlag)
            {
                ...
                if ((ptr_cr + uvlinesize * (h >> s->chroma_y_shift)) > (ptr + uvlinesize * ((s->height + 32) >> s->chroma_y_shift) - align))
                {
                    av_log(s->avctx, AV_LOG_WARNING,
                           "MPEG ref picture  wrong\n");
                    IMEDIA_SET_ERR_MB(s->avctx->iErrorCode, IMEDIA_ERR_MB_REF);
