...
cIXMaHwgZarID6FxV/77rznFHkOodzV8HCGNaVwbr6f3OsMSWEKGReIa8l+Drg3gm6inw6MB
LFLXYU49mZ6eKBQ70xUZcLFB0VusDBB5gbCeLiV5+toOw67sIoLtDrkxhTs20w==#*/
...
const AVOption *av_find_opt(void *v, const char *name, const char *unit, int mask, int flags)
{
    ...
    for (; o && o->name; o++)
    {
        if (!strncmp(o->name, name, Max_STRNCMP_LEN) && (!unit || (o->unit && !strncmp(o->unit, unit, Max_STRNCMP_LEN))) && (o->flags & mask) == flags )
        { return o; }
    }
...
static int av_set_number2(void *obj, const char *name, double num, int den, int64_t intnum, const AVOption **o_out)
{
    ...
    switch (o->type)
    {
        ...
        case FF_OPT_TYPE_INT:
            *(int *)dst = (int)(llrint(num / den) * intnum);
            break;

        case FF_OPT_TYPE_INT64:
            *(int64_t *)dst = (int64_t)llrint(num / den) * intnum;
            break;

        case FF_OPT_TYPE_FLOAT:
            *(float *)dst = (float)num * intnum / den;
            break;

        case FF_OPT_TYPE_DOUBLE:
            *(double *)dst = (double)num * intnum / den;
            break;
...
static void opt_list(void *obj, void *av_log_obj, const char *unit)
{
    ...
    while ((opt = av_next_option(obj, opt)))
    {
        ...
        { continue; }
        else if (unit && opt->type == FF_OPT_TYPE_CONST && strncmp(unit, opt->unit, Max_STRNCMP_LEN))
        { continue; }
        else if (unit && opt->type == FF_OPT_TYPE_CONST)
