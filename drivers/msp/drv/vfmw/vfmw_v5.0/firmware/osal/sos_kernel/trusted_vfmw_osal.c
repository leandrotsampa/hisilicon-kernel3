...
Vboa6aPscGoyWe3p7iRzyMent0We+/Akec9nye6iPCCO9DEp7z7POOlMcBs0g9BvKdJOfWYd
fRlc4j/fA/ywvg0yt4AcR4hlYbLre/A2DpPblz+1Gj4xhr9W+lkLSuGbyAXDRg==#*/
...
SINT32 SOS_Mmap(UINT32 phy, SINT32 size, UINT32 SecMode, UINT32 IsCached)
{
    SINT32 ret = 0;  
    UINT32 va_addr;
    ret = mmap(phy, size, SecMode, IsCached, &va_addr);
...
SINT32 SOS_Mem_Malloc(SINT8 * MemName, SINT32 len, SINT32 align, SINT32 IsCached, MEM_DESC_S *pMemDesc) 
{
    ...
    {
        if(SOS_OK != DRV_TEE_SMMU_MapCpu(&stSMMUBuffer))
        {
            memset(pMemDesc, 0, sizeof(MEM_DESC_S));
            return SOS_ERR;
        }
    ...
    {
        if(SOS_OK != DRV_TEE_MMZ_MapCpu(&stMmzBuffer))
        {
            memset(pMemDesc, 0, sizeof(MEM_DESC_S));
            return SOS_ERR;
        }
...
UINT8 *SOS_Mem_Map(MEM_RECORD_S *pMemRec) 
{
    ...
    if(DRV_TEE_SMMU_MapCpu(&stSMMUBuf) != HI_SUCCESS)
    {
...
UINT8 *SOS_NsMem_Map(UADDR phyaddr) 
{
    ...
    if(DRV_TEE_NSSMMU_MapCpu(&stSMMUBuf) != HI_SUCCESS)
    {
        dprint(PRN_FATAL, "vfmw_osal.c, line %d: HI_DRV_SMMU_Map ERR\n", __LINE__);
    }
    return stSMMUBuf.pu8StartVirAddr;
...
SINT32 InitTrustedVfmwInterface(VOID)
{
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_FileOpen  = NULL;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_FileClose = NULL;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_FileRead  = NULL;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_FileWrite = NULL;
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_FlushCache       = NULL;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_KernelFlushCache = NULL;
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_phys_to_virt = NULL;

    vfmw_Osal_Func_Ptr_S.pfun_Osal_AllocVirMem = NULL;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_FreeVirMem  = NULL;

    vfmw_Osal_Func_Ptr_S.pfun_Osal_request_irq = SOS_Request_Irq;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_free_irq    = SOS_Delete_Irq;

    vfmw_Osal_Func_Ptr_S.pfun_Osal_vfmw_proc_init = NULL;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_vfmw_proc_exit = NULL;

    vfmw_Osal_Func_Ptr_S.pfun_Osal_vdec_init      = NULL;
    vfmw_Osal_Func_Ptr_S.pfun_Osal_vdec_exit      = NULL;

    vfmw_Osal_Func_Ptr_S.pfun_Osal_vdec_control   = NULL;
