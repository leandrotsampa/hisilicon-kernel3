...
/***********************************************************************
      VFMW的版本号
 ***********************************************************************/
#define  VFMW_VERSION_NUM       (2016032800)
...
#define REPORT_ARRANGE_FRAME_BUF(chan_id, stVfmwFrameSizeInfo, Width, Height)                     	\
    do{                                                         \
        ...
        {                                                       \
            UINT32  para[7];                                    \
            para[0] = ((VFMW_FRAME_SIZE_INFO_S)stVfmwFrameSizeInfo).u32ImageNum;                         \
            para[1] = ((VFMW_FRAME_SIZE_INFO_S)stVfmwFrameSizeInfo).u32ImageSize;                       \
            para[2] = ((VFMW_FRAME_SIZE_INFO_S)stVfmwFrameSizeInfo).u32PmvNum;                         \
            para[3] = ((VFMW_FRAME_SIZE_INFO_S)stVfmwFrameSizeInfo).u32PmvSize;                       \
            para[4] = ((VFMW_FRAME_SIZE_INFO_S)stVfmwFrameSizeInfo).u32HdrMetadataSize;                       \
            para[5] = (UINT32)(Width);                          \
...
#define REPORT_ALLOC_FRAME_BUF_ONLY(chan_id, stVfmwFrameSizeInfo, Width, Height)                     	\
            do{                                                         \
                if( NULL != g_event_report )                            \
                {                                                       \
                    UINT32  para[7];                                    \
...
#define REPORT_RELSE_FRAME_BUF_ONLY(chan_id, PhyAddr, VirAddr)  \
    do{                                                         \
        if( NULL != g_event_report )                            \
        {                                                       \
            UINT64  para[2];                                    \
...
#define REPORT_POST_MODULE_OCCUPY_FRAMES(ChanID, pSpecialFrmBufRec, SpecialFrameNum)                       	\
    do{                                                         \
        if( NULL != g_event_report )                            \
        {                                                       \
            UINT64  para[2];                                    \
            para[0] = (UINT64)(ULONG)pSpecialFrmBufRec;          \
            para[1] = SpecialFrameNum;                           \
            g_event_report(ChanID, EVNT_POST_MODULE_OCCUPY_FRAMES, (UINT8*)para, SpecialFrameNum*sizeof(*pSpecialFrmBufRec));    \
        }                                                       \
    }while(0)
#else
/* type=121 上报后级模块占有的一帧 */
#define REPORT_POST_MODULE_OCCUPY_ONE_FRAME(ChanID, specialFramePhyAddr,specialFrameVirAddr, findSpecialFrame)                       	\
    do{                                                         \
        if( NULL != g_event_report )                            \
        {                                                       \
            UINT64 para[3];                                    \
...
do{                                                                 \
    ...
    {                                                               \
        vfmw_Osal_Func_Ptr_S.pfun_Osal_printk("S: ");               \
