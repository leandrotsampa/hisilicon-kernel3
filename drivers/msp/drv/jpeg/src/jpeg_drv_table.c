...
* others:		: NA
*****************************************************************************/
static HI_S32 jpeg_table_dc(HI_DRV_JPEG_OUTMSG_S *pstOutMsg)
{
		HI_U32 cnt		  = 0;
		HI_U32 u32Index   = 0;
		HI_U32 sum_syms    =0;
		...
		{
			pTmp+=1;
			memcpy(&(gs_huff_ptr[LU_TABL].bits[1]), pTmp, 16);
			pTmp   += 16;
			memcpy(&(gs_huff_ptr[LU_TABL].huffval[0]), pTmp, 12);
			pTmp   += 12;
			pTmp   += 1;
			memcpy(&(gs_huff_ptr[CU_TABL].bits[1]), pTmp, 16);
			pTmp   += 16;
			memcpy(&(gs_huff_ptr[CU_TABL].huffval[0]), pTmp, 12);
			pTmp   += 12;
		}
		else
		{
			memcpy(&(gs_huff_ptr[LU_TABL]), &pstImgInfo->DcHuffTbl[0], sizeof(gs_huff_ptr[LU_TABL]));
			if(HI_FALSE == pstImgInfo->DcHuffTbl[1].bHasHuffTbl)
			{
				memcpy(&(gs_huff_ptr[CU_TABL]),&pstImgInfo->DcHuffTbl[0],sizeof(gs_huff_ptr[LU_TABL]));
			}
			else
			{
				memcpy(&(gs_huff_ptr[CU_TABL]),&pstImgInfo->DcHuffTbl[1],sizeof(gs_huff_ptr[LU_TABL]));
			}
		}
...
* others:		: NA
*****************************************************************************/
static HI_VOID jpeg_table_ac(HI_DRV_JPEG_OUTMSG_S *pstOutMsg)
{
		HI_U32 cnt		  = 0;
		HI_U32 u32Index   = 0;
		HI_U32 pre_index  = 0;
		...
		{
			pTmp+=1;
			memset(&(gs_huff_ptr[LU_TABL].bits[0]),0,17);
			memcpy(&(gs_huff_ptr[LU_TABL].bits[1]), pTmp, 16);
			pTmp   += 16;
			memset(&(gs_huff_ptr[LU_TABL].huffval[0]),0,256);
			memcpy(&(gs_huff_ptr[LU_TABL].huffval[0]), pTmp, 162);
			pTmp   += 162;

			pTmp+=1;
			memset(&(gs_huff_ptr[CU_TABL].bits[0]),0,17);
			memcpy(&(gs_huff_ptr[CU_TABL].bits[1]), pTmp, 16);
			pTmp   += 16;
			memset(&(gs_huff_ptr[CU_TABL].huffval[0]),0,256);
			memcpy(&(gs_huff_ptr[CU_TABL].huffval[0]), pTmp, 162);
		}
		else
		{
			memset(&(gs_huff_ptr[LU_TABL].bits[0]),0,17);
			...
			memset(&(gs_huff_ptr[CU_TABL].bits[0]),0,17);
			memset(&(gs_huff_ptr[CU_TABL].huffval[0]),0,256);
			memcpy(&(gs_huff_ptr[LU_TABL]), &pstImgInfo->AcHuffTbl[0], sizeof(gs_huff_ptr[LU_TABL]));
            
			if(HI_FALSE == &pstImgInfo->AcHuffTbl[1].bHasHuffTbl)
			{
				memcpy(&(gs_huff_ptr[CU_TABL]),&pstImgInfo->AcHuffTbl[0],sizeof(gs_huff_ptr[LU_TABL]));
			}
			else
			{
				memcpy(&(gs_huff_ptr[CU_TABL]),&pstImgInfo->AcHuffTbl[1],sizeof(gs_huff_ptr[LU_TABL]));
			}
		}
