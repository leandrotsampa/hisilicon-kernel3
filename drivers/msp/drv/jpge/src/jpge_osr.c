...
long jpge_ioctl(struct file *pFile, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        {
            s32Ret = JPEG_ENC_CreateHandle(&u32Handle);
            if(HI_SUCCESS != s32Ret){
                return HI_FAILURE;
            }
            s32Ret = copy_to_user(argp, (HI_VOID *)&u32Handle,sizeof(HI_U32));
            if(s32Ret < 0){
                return -EFAULT;
            }
            return HI_SUCCESS;
        ...
        {
            memset(&stEncCfg,0,sizeof(Jpge_EncCfg_S));
        ...
        {
            memset(&EncInfo,0,sizeof(Jpge_EncInfo_S));
        ...
        {
            s32Ret = copy_from_user(&u32Handle, argp, sizeof(HI_U32));
            if(s32Ret < 0){
                return -EFAULT;
            }
            return JPEG_ENC_Destory(u32Handle);
        }
        default:
            break;
    }
...
long jpge_compat_ioctl(struct file *pFile, unsigned int cmd, unsigned long arg)
{
    ...
    {
        case JPGE_CREATEHANDLE_CMD:
        {/** 32bit and 64bit para is the same **/
        ...
        {
            memset(&stEncInfo,0,sizeof(Jpge_EncInfo_S));
    		memset(&CompatEncInfo,0,sizeof(Jpge_Compat_EncInfo_S));
