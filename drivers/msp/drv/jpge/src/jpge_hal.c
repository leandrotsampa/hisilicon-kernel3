...
static HI_S32 Jpge_StartOneFrame_toVenc(Jpge_EncPara_S  *pstEncPara, Venc2Jpge_EncIn_S *pEncIn)
{
    ...
    memcpy(pstEncPara->pJpgeYqt,pEncIn->pJpgeYqt,sizeof(HI_U8)*64);
    memcpy(pstEncPara->pJpgeCqt,pEncIn->pJpgeCqt,sizeof(HI_U8)*64);
...
{  
    ...
    {/** pic cfg **/
        ...
        if(   (pstEncPara->RotationAngle == 1 || pstEncPara->RotationAngle == 2) \
           && (pstEncPara->YuvSampleType == 1)){/* 90 or 270 @ 422 */
...
static HI_S32 Jpge_EndOneFrame(Jpge_EncPara_S  *pstEncPara, Jpge_EncIn_S *pstEncIn, Jpge_EncOut_S *pEncOut)
{
    ...
    memcpy(&pNaluHdr,(HI_CHAR*)((unsigned long)pstEncIn->pOutBuf + pstEncPara->Vir2BusOffset),sizeof(Jpge_NaluHdr_S));
...
* others:       : NA
****************************************************************************/
HI_S32  JPEG_ENC_CreateHandle(HI_U32 *pEncHandle)
{
...
* others:       : NA
****************************************************************************/
static HI_S32  JPEG_ENC_DestoryHandle(HI_U32 EncHandle)
{
    HI_S32 s32Cnt = 0;
...
* others:       : NA
****************************************************************************/
HI_S32 JPEG_ENC_CtreateForVenc(HI_U32 *pEncHandle, Jpge_EncCfg_S *pEncCfg)
{
    HI_S32 s32Ret = HI_SUCCESS;
    Jpge_EncPara_S  *pEncPara = NULL;
...
* others:       : NA
****************************************************************************/
static HI_VOID JPEG_ENC_FreeMmzBuf(HI_U32 u32Phyaddr,HI_U8* pVirDdr)
{
    MMZ_BUFFER_S  stMMZBuffer    = {0};
    stMMZBuffer.u32StartPhyAddr  = u32Phyaddr;
    if(NULL != pVirDdr){
        stMMZBuffer.pu8StartVirAddr = pVirDdr;
...
static HI_S32 JPEG_ENC_AllocMmzBuf(Jpge_EncPara_S *pstEncPara,HI_U32 *pu32YAddr,HI_U32 *pu32UAddr,HI_U32 *pu32VAddr)
{
    ...
    if(0 != pstEncPara->SrcYAddr){
        memset(&stMMZBuffer,0x0,sizeof(MMZ_BUFFER_S));
...
* others:       : NA
****************************************************************************/
HI_S32 JPEG_ENC_Start(HI_U32 EncHandle, Jpge_EncIn_S *pEncIn, Jpge_EncOut_S *pEncOut)
{
    HI_S32 s32Ret = HI_SUCCESS;
#ifdef CONFIG_GFX_MMU_SUPPORT
    HI_U32 u32YAddr   = 0;
...
#if 0
#ifdef CONFIG_GFX_MMU_SUPPORT
    memcpy(pInOutVir,pEncOut->pStream,pEncOut->StrmSize);
...
* others:       : NA
****************************************************************************/
HI_S32 JPEG_ENC_StartForVenc(HI_U32 EncHandle, Venc2Jpge_EncIn_S *pEncIn,HI_BOOL *pBufferFull )
{
#ifdef CONFIG_GFX_MMU_SUPPORT
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32YAddr   = 0;
...
* others:       : NA
****************************************************************************/
HI_S32 JPEG_ENC_Destory(HI_U32 EncHandle)
{
...
* others:       : NA
****************************************************************************/
HI_S32  JPEG_ENC_DestoryForVenc(HI_U32 EncHandle)
{
    return JPEG_ENC_DestoryHandle(EncHandle);
}
