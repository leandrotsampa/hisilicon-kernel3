        ...
        if (HI_INVALID_PTS != pstPtsInfo->u32LastSrcPts)
        {
            ...
            if (u32NumPass)
            {
                if (u32PtsPass <= PTS_MAX_PASSTIME)
                {
                    ...
                    {
                        u32FrameTimeABS = (u32PtsPass * 1000 / u32NumPass > pstPtsInfo->u32LastCalcFrmTime)?\
                          (u32PtsPass * 1000 / u32NumPass - pstPtsInfo->u32LastCalcFrmTime): (pstPtsInfo->u32LastCalcFrmTime - u32PtsPass * 1000 / u32NumPass);

                        if (u32FrameTimeABS > FRMTIME_MAX_JUMP)
                        {
