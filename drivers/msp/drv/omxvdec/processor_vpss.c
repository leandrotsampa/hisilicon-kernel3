...
static HI_S32 processor_report_frame(OMXVDEC_CHAN_CTX *pchan, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    else
    {
	    ...
	    user_buf.data_len = DEFAULT_FRAME_SIZE(pstFrame->stBufAddr[0].u32Stride_Y, pstFrame->u32Height);
	}
...
static HI_S32 processor_get_frame_buffer(OMXVDEC_CHAN_CTX *pchan, HI_VOID *pstArgs)
{
    ...
    if (pVpssFrm->u32FrmW != pchan->out_width || pVpssFrm->u32FrmH != pchan->out_height)
    {
        ...
        {
			if (pchan->protocol == STD_HEVC)
        	{
				pchan->out_stride = ImgSize.frame_stride = HI_SYS_GET_STRIDE(HI_OMX_ALIGN_10BIT(pchan->out_width));
			}
			else
			{
...
static HI_S32 processor_get_image(HI_S32 VpssId, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    if (HI_TRUE == g_BypassVpss && HI_SUCCESS == ret)
    {
    ...
    if (STD_HEVC == pchan->protocol || STD_VP9 == pchan->protocol)
    {
        if ((stImage.bit_depth_luma > 8) || (stImage.bit_depth_chroma > 8))
        {
    ...
    else
    {
        ...
        //if is 10 bit image, SHOULD send 2bit addr&stride to VPSS
        if ((STD_HEVC == pchan->protocol || STD_VP9 == pchan->protocol)\
	     && (HI_DRV_PIXEL_BITWIDTH_10BIT == pstFrame->enBitWidth))
        {
            pstFrame->stBufAddr_LB[0].u32PhyAddr_Y = stImage.luma_2bit_phy_addr;
            pstFrame->stBufAddr_LB[0].u32Stride_Y  = stImage.image_stride_2bit;
            pstFrame->stBufAddr_LB[0].u32PhyAddr_C = stImage.chrom_2bit_phy_addr;
            pstFrame->stBufAddr_LB[0].u32Stride_C  = stImage.image_stride_2bit;
        }
    }
	...
	if (stImage.image_width >= 1280 && stImage.image_height >= 720)
	{
...
#if 0
static HI_S32 processor_release_occupied_frame(OMXVDEC_CHAN_CTX *pchan, HI_U32 phyaddr)
{
    HI_U32 i;
    ...
    #if defined(HI_OMX_TEE_SUPPORT) && defined(HI_SMMU_SUPPORT)
    	if (HI_TRUE == pchan->is_tvp)
    	{
    		if (pBufSlot->frm_buf.sec_smmu_buf.smmu_addr == phyaddr)
            {
                omxvdec_release_mem(&pBufSlot->frm_buf.sec_smmu_buf, pBufSlot->frm_buf_type);
	...
	#endif
		{
        if (pBufSlot->frm_buf.mmz_buf.u32StartPhyAddr == phyaddr)
        {
            omxvdec_release_mem(&pBufSlot->frm_buf.mmz_buf, pBufSlot->frm_buf_type);
            pBufSlot->frm_buf_type = ALLOC_INVALID;
            pBufSlot->is_available  = HI_FALSE;
            pBufSlot->is_configured = HI_FALSE;
			break;
    ...
    {
        OmxPrint(OMX_FATAL, "%s occupied frame not found! phyaddr = %x\n", __func__, phyaddr);
        return HI_FAILURE;
    }
    else
    {
...
HI_S32 processor_release_image(HI_S32 VpssId, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    HI_S32 ret = HI_FAILURE;
    IMAGE stImage;
    OMXVDEC_CHAN_CTX *pchan = HI_NULL;
...
#ifdef VFMW_VPSS_BYPASS_EN
    if ((0 != g_OmxVdec->stRemainFrmList.s32Num) && (channel_IsOccupiedFrm(pstFrame->stBufAddr[0].u32PhyAddr_Y,&OccupiedFrmIndex)))
    {
        ...
        OMXVDEC_List_Del(&g_OmxVdec->stRemainFrmList,OccupiedFrmIndex);
    }
