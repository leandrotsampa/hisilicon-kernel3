...
OMXVDEC_CHAN_CTX* task_find_inst_need_wake_up(OMXVDEC_ENTRY *omxvdec)
{
    ...
    if (!list_empty(&omxvdec->chan_list))
    {
        list_for_each_entry(pchan, &omxvdec->chan_list, chan_list)
        {
            ...
            if(DFS_WAIT_TRY_ALLOC  == pchan->dfs.dfs_state)
            {
...
static DSF_SINGLE_BUF_S *task_get_free_dfs_slot(OMXVDEC_CHAN_CTX *pchan, SLOT_TYPE_E type)
{
    ...
    for (i=0; i < MAX_DFS_BUF_NUM; i++)
	{
		...
		if (HI_FALSE == pBufSlot->is_available && HI_FALSE == pBufSlot->is_configured)
		{
...
static HI_S32 task_alloc_buf_for_slot(OMXVDEC_CHAN_CTX *pchan, DSF_SINGLE_BUF_S *pBufSlot, FRAME_BUF_S *pstFrameBuf, ALLOC_BUF_TYPE_E type)
{
    ...
    HI_U32 NeededPmvNum, NeededPmvSize;
...
static HI_S32 task_handle_situation_1(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    for (i=0; i<lack_num; i++)
    {
        ...
        CHECK_And_SET_DFS_STATE(DFS_WAIT_CONFIG, DFS_WAIT_CONFIG);
...
static HI_S32 task_handle_situation_2(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    for (i=0; i<lack_num; i++)
    {
        ...
        if (pchan->dfs.alloc_frm_num < NeededFrmNum)
        {
    		ret = task_alloc_buf_for_slot(pchan, pBufSlot, &stFrameBuf, ALLOC_FRM);
    		if (ret != HI_SUCCESS)
    		{
        ...
        {
            OmxPrint(OMX_ERR, "%s shoud not be here! frm needed = %d, alloc = %d, config = %d\n", __func__, NeededFrmNum, pchan->dfs.alloc_frm_num, pchan->dfs.config_frm_num);
            return HI_FAILURE;
        }
    }
...
static HI_S32 task_alloc_channel_mem_single(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    if (DFS_WAIT_RESPONSE_1 == pchan->dfs.dfs_state)
    {
        ret = task_release_channel_mem(pchan);
        if (ret != HI_SUCCESS)
        {
            OmxPrint(OMX_ERR, "%s task_release_channel_mem failed!\n", __func__);
            return HI_FAILURE;
        }
        CHECK_And_SET_DFS_STATE(DFS_WAIT_RESPONSE_1, DFS_WAIT_TRY_ALLOC);
    }

    if (pchan->dfs.retry_count >= MAX_RETRY)
    {
        OmxPrint(OMX_ERR, "%s failed! retry_count >= %d!\n", __func__, MAX_RETRY);
        return HI_FAILURE;
    }

    CHECK_And_SET_DFS_STATE(DFS_WAIT_TRY_ALLOC, DFS_WAIT_CONFIG);
    ...
    */
    ...
    CHECK_And_SET_DFS_STATE(DFS_WAIT_CONFIG, DFS_DONE);
...
NO_MEM_EXIT:
    CHECK_And_SET_DFS_STATE(DFS_WAIT_CONFIG, DFS_WAIT_TRY_ALLOC);
...
static HI_S32 task_memory_manager(OMXVDEC_CHAN_CTX *pchan)
{
	...
	#if (VFMW_FLAG == 5)
        if (DFS_WAIT_RESPONSE_1 == pchan->dfs.dfs_state
              || DFS_WAIT_TRY_ALLOC  == pchan->dfs.dfs_state)
        {
	...
	#endif
        {
            OmxPrint(OMX_ERR, "%s invalid dfs flag %d\n", __func__, pchan->dfs.dfs_state);
        }
    }
