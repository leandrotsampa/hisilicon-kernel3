...
HI_S32 VI_DRV_AcquireFrame(VPSS_HANDLE hVpss, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    if (HI_TRUE != g_ViDrv[enPort][u32Chn].stAttr.bVirtual)
    {
        ...
        Ret = HI_PHY_VICAP_AcquireFrame(g_ViDrv[enPort][u32Chn].hVicap,  pstFrame);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_VI("HI_PHY_VICAP_AcquireFrame failed ret = 0x%08x\n",Ret);
            return HI_ERR_VI_BUF_EMPTY;
        }

        g_ViDrv[enPort][u32Chn].stStat.AcquireOK++;
        //HI_ERR_VI(":%c\n", pstFrame->bProgressive? 'P':'I');
    ...
    VI_DRV_Lowdelay_Stat(pstFrame->hTunnelSrc, EVENT_VI_FRM_IN, pstFrame->u32FrameIndex);
#endif
...
HI_S32 VI_DRV_ReleaseFrame(VPSS_HANDLE hVpss, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    if (HI_TRUE != g_ViDrv[enPort][u32Chn].stAttr.bVirtual)
    {
        Ret = HI_PHY_VICAP_ReleaseFrame(g_ViDrv[enPort][u32Chn].hVicap,  pstFrame);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_VI("HI_PHY_VICAP_ReleaseFrame failed ret = 0x%08x\n",Ret);
            return HI_FAILURE;
        }
...
HI_S32 VI_DRV_DestroyForce(HI_HANDLE hVi)
{
    ...
    if (HI_FALSE == g_ViDrv[enPort][u32Chn].stAttr.bVirtual)
    {
        ...
        Ret = HI_PHY_VICAP_Stop(g_ViDrv[enPort][u32Chn].hVicap);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_VI("HI_PHY_VICAP_Start failed ret = 0x%08x\n",Ret);
            return HI_FAILURE;
        }

        Ret = HI_PHY_VICAP_Destroy(g_ViDrv[enPort][u32Chn].hVicap);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_VI("HI_PHY_VICAP_Start failed ret = 0x%08x\n",Ret);
            return HI_FAILURE;
...
HI_S32 HI_DRV_VI_Create(VI_CREATE_S *pstCreate, HI_VOID *file)
{
    ...
    if (HI_FALSE == pstCreate->stViAttr.bVirtual)
    {
        ...
        Ret =  HI_PHY_VICAP_Create(&pstCreate->stViAttr, &g_ViDrv[pstCreate->enPort][u32Chn].hVicap);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_VI("HI_PHY_VICAP_Create failed, ret = 0x%08x\n", Ret);
            goto ERR2;
        }
...
HI_S32 HI_DRV_VI_SetAttr(HI_HANDLE hVi, HI_UNF_VI_ATTR_S *pstAttr)
{
        ...
        && (HI_FALSE == pstAttr->bVirtual))
    {
        ...
        Ret =  HI_PHY_VICAP_Create(pstAttr, &g_ViDrv[enPort][u32Chn].hVicap);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_VI("HI_PHY_VICAP_Create failed, ret = 0x%08x\n", Ret);
            return HI_FAILURE;
             ...
             && (HI_TRUE == pstAttr->bVirtual))
    {
        ...
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_VI("HI_PHY_VICAP_Destroy failed.\n");
            return HI_FAILURE;
        }
        bRealViOpened = HI_FALSE;
    }
...
#ifdef HI_VI_REAL_SUPPORT
#ifdef HI_HDMI_RX_SUPPORT
                    Ret =  HI_PHY_VICAP_Create( &g_ViDrv[enPort][u32Chn].stAttr, &g_ViDrv[enPort][u32Chn].hVicap);
                    if (Ret != HI_SUCCESS)
                    {
                        HI_ERR_VI("HI_PHY_VICAP_Create failed, ret = 0x%08x\n", Ret);
                    }
...
#ifdef HI_HDMI_RX_SUPPORT
        HI_S32 Ret;
        Ret = HI_PHY_VICAP_Start(pViDrv->hVicap);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_VI("HI_PHY_VICAP_Start failed ret = 0x%08x\n",Ret);
            return HI_FAILURE;
...
#ifdef HI_VI_REAL_SUPPORT
#ifdef HI_HDMI_RX_SUPPORT
        Ret = HI_PHY_VICAP_Stop(pViDrv->hVicap);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_VI("HI_PHY_VICAP_Start failed ret = 0x%08x\n",Ret);
            return HI_FAILURE;
...
HI_S32 HI_DRV_VI_UsrAcquireFrame(HI_HANDLE hVi, HI_UNF_VIDEO_FRAME_INFO_S *pstFrame, HI_U32 u32TimeoutMs)
{
    ...
    if (HI_TRUE != g_ViDrv[enPort][u32Chn].stAttr.bVirtual)
    {
        Ret = HI_PHY_VICAP_UserAcquireFrame(g_ViDrv[enPort][u32Chn].hVicap,  pstFrame, u32TimeoutMs);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_VI("HI_PHY_VICAP_UserAcquireFrame failed ret = 0x%08x\n",Ret);
            return HI_FAILURE;
        }
...
HI_S32 HI_DRV_VI_UsrReleaseFrame(HI_HANDLE hVi, HI_UNF_VIDEO_FRAME_INFO_S *pstFrame)
{
    ...
    if (HI_TRUE != g_ViDrv[enPort][u32Chn].stAttr.bVirtual)
    {
        Ret = HI_PHY_VICAP_UserReleaseFrame(g_ViDrv[enPort][u32Chn].hVicap,  pstFrame);
        if (HI_SUCCESS != Ret)
        {
            HI_WARN_VI("HI_PHY_VICAP_UserReleaseFrame failed ret = 0x%08x\n",Ret);
            return HI_FAILURE;
        }
