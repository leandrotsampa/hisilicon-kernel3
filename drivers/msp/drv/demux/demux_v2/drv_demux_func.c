...
#define DMX_FQ_AUD_BLOCK_SIZE              (1 * 1024)  /* some aud bandwidth is small, big block will delay data too much. */
...
static HI_S32 DmxInitTeec(Dmx_Cluster_S *DmxCluster)
{
    ...
    ret = TEEK_OpenSession(&DmxCluster->teec.context, &DmxCluster->teec.session, &DmxTaskUUID, TEEC_LOGIN_PUBLIC, NULL, NULL, NULL);
    if (HI_SUCCESS != ret)
    {
    ...
    ret = TEEK_OpenSession(&DmxCluster->teec.context, &DmxCluster->teec.session_lock, &DmxTaskUUID, TEEC_LOGIN_PUBLIC, NULL, NULL, NULL);
    if (HI_SUCCESS != ret)
    {
    ...
    ret = TEEK_OpenSession(&DmxCluster->teec.context, &DmxCluster->teec.session_unlock, &DmxTaskUUID, TEEC_LOGIN_PUBLIC, NULL, NULL, NULL);
    if (HI_SUCCESS != ret)
    {
...
static HI_S32 DmxTeecRegisterChannel(HI_U32 DmxId, HI_U32 ChanId)
{
    HI_S32 ret = HI_FAILURE;
    Dmx_Cluster_S *DmxCluster = GetDmxCluster();
...
static HI_S32 DmxTeecUnregisterChannel(HI_U32 DmxId, HI_U32 ChanId)
{
    HI_S32 ret = HI_FAILURE;
    Dmx_Cluster_S *DmxCluster = GetDmxCluster();
    TEEC_Operation operation;
...
static HI_S32 DmxTeecRegisterOq(HI_U32 DmxId, HI_U32 OqId)
{
    HI_S32 ret = HI_FAILURE;
    Dmx_Cluster_S *DmxCluster = GetDmxCluster();
    TEEC_Operation operation;
...
static HI_S32 DmxTeecUnregisterOq(HI_U32 DmxId, HI_U32 OqId)
{
    HI_S32 ret = HI_FAILURE;
    Dmx_Cluster_S *DmxCluster = GetDmxCluster();
    TEEC_Operation operation;
...
static HI_VOID DmxTeecLockChannel(HI_U32 DmxId, HI_U32 ChanId)
{
    HI_S32 ret = HI_FAILURE;
    Dmx_Cluster_S *DmxCluster = GetDmxCluster();
    TEEC_Operation operation;
    ...
    ret = DmxCluster->Ops->SendCmdToTA(TEEC_CMD_LOCK_CHANNEL, &operation, HI_NULL);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("send TEEC_CMD_LOCK_CHANNEL to TA failed(0x%x).\n", ret);
    }
}

static HI_VOID DmxTeecUnlockChannel(HI_U32 DmxId, HI_U32 ChanId)
{
    HI_S32 ret = HI_FAILURE;
    Dmx_Cluster_S *DmxCluster = GetDmxCluster();
    TEEC_Operation operation;
    ...
    ret = DmxCluster->Ops->SendCmdToTA(TEEC_CMD_UNLOCK_CHANNEL, &operation, HI_NULL);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("send TEEC_CMD_UNLOCK_CHANNEL to TA failed(0x%x).\n", ret);
    }
}
...
static inline HI_S32 DmxTeecRegisterChannel(HI_U32 DmxId, HI_U32 ChanId) 
{
    HI_ERR_DEMUX("DMX_TEE_SUPPORT not configured.\n");
    return HI_ERR_DMX_INVALID_PARA;
}
static inline HI_S32 DmxTeecUnregisterChannel(HI_U32 DmxId, HI_U32 ChanId) {return HI_ERR_DMX_INVALID_PARA;}

static inline HI_S32 DmxTeecRegisterOq(HI_U32 DmxId, HI_U32 OqId) 
{
    HI_ERR_DEMUX("DMX_TEE_SUPPORT not configured.\n");
    return HI_ERR_DMX_INVALID_PARA;
}
static inline HI_S32 DmxTeecUnregisterOq(HI_U32 DmxId, HI_U32 OqId) {return HI_ERR_DMX_INVALID_PARA;}

static inline HI_VOID DmxTeecLockChannel(HI_U32 DmxId, HI_U32 ChanId) {}
static inline HI_VOID DmxTeecUnlockChannel(HI_U32 DmxId, HI_U32 ChanId){}
...
static inline HI_S32 DmxTeecUnregisterAVSecBuf(Dmx_Set_S *DmxSet, DMX_ChanInfo_S *ChanInfo, SMMU_BUFFER_S *MmuBuf){return HI_ERR_DMX_INVALID_PARA;}
...
static inline HI_S32 DmxTeecUnregisterRecSecBuf(DMX_RecInfo_S  *RecInfo, SMMU_BUFFER_S *MmuBuf){return HI_ERR_DMX_INVALID_PARA;}
...
static HI_S32 DmxParsePesHeader(DMX_ChanInfo_S *ChanInfo,
        HI_U8           *ParserAddr,
        HI_U32          ParserLen,
        HI_U32         *PesHeaderLen,
        HI_U32          *PesPktLen,
...
static HI_S32 DmxGetPesHeaderLen(DMX_ChanInfo_S *ChanInfo, HI_U8 *ParserAddr, HI_U32 ParserLen, HI_U32 *PesHeaderLen)
{
    if (HI_UNF_DMX_SECURE_MODE_TEE == ChanInfo->ChanSecure)
    {
    ...
    do
    {
        ...
        if (index < DmxSet->DmxOqCnt)
        {
            ...
            if (index == find_first_zero_bit(DmxSet->OqBitmap, DmxSet->DmxOqCnt))
            {
                ...
                {
                    ret = DmxTeecRegisterOq(DmxId, index);
                    if (HI_SUCCESS != ret)
                    {
...
static HI_VOID DmxOqRelease(Dmx_Set_S *DmxSet, HI_U32 DmxId, HI_U32 OqId)
{
    ...
    if (unlikely(HI_UNF_DMX_SECURE_MODE_TEE == OqInfo->SecureMode))
    {
        DmxTeecUnregisterOq(DmxId, OqId);
    }
...
static HI_S32 DmxChnAcquire(HI_U32 DmxId, HI_UNF_DMX_CHAN_ATTR_S *ChanAttr, HI_U32 *ChanId, DMX_MMZ_BUF_S *ChanBuf)
{
    ...
    *ChanId = find_first_zero_bit(DmxSet->ChnBitmap, DmxSet->DmxChanCnt);
    ...
    {
        ret = DmxTeecRegisterChannel(DmxId, *ChanId);
        if (HI_SUCCESS != ret)
        {
            goto out2;
        }
...
static HI_S32 DmxChnRelease(HI_U32 DmxId, HI_U32 ChanId)
{
    ...
    if (unlikely(HI_UNF_DMX_SECURE_MODE_TEE == ChanInfo->ChanSecure))
    {
        DmxTeecUnregisterChannel(DmxId, ChanId);
    }
...
HI_S32 DmxGetChnInstance(Dmx_Set_S *DmxSet, HI_U32 ChanId)
{
    ...
    if (unlikely(HI_UNF_DMX_SECURE_MODE_TEE == ChanInfo->ChanSecure))
    {
        DmxTeecLockChannel(ChanInfo->DmxId, ChanId);
    } 
...
HI_VOID DmxPutChnInstance(Dmx_Set_S *DmxSet, HI_U32 ChnId)
{
    ...
    if (unlikely(HI_UNF_DMX_SECURE_MODE_TEE == ChanInfo->ChanSecure))
    {
        DmxTeecUnlockChannel(ChanInfo->DmxId, ChnId);
    }
...
HI_VOID DmxPutChnInstanceEx(Dmx_Set_S *DmxSet, HI_U32 DmxId, HI_U32 ChnId)
{
    ...
    if (unlikely(HI_UNF_DMX_SECURE_MODE_TEE == ChanInfo->ChanSecure))
    {
        DmxTeecUnlockChannel(DmxId, ChnId);
    }
...
static HI_S32 DMXOsiIsr(int irq, void* priv)
{
    ...
    if (IntStatus)
    {
        ...
        if (SmmuIntStatus.bits.tlbinvalid)
        {
                ...
                HI_FATAL_DEMUX("smmu tlb invalid: r:0x%x/w:0x%x error.\n", ReadErrAddr, WriteErrAddr);
        }
...
static HI_S32 GetChnDataStatus(Dmx_Set_S *DmxSet, HI_U32 ChanId)
{
    ...
    if ((ChanInfo->DmxId >= GetDmxCluster()->Ops->GetDmxNum()) || OqInfo->u32FQId != DMX_FQ_COMMOM)
    {
    ...
    FQDepth = DmxSet->DmxFqInfo[u32FQId].FQDepth;
...
static HI_VOID CommFQMonitor(Dmx_Set_S *DmxSet)
{
    ...
    if (GetFQStatus(DmxSet, DMX_FQ_COMMOM))//dmx pool buffer used > 80%
    {
        for ( i = 0 ; i < DmxSet->DmxChanCnt; i++ )
        {
            ...
            s32Ret = GetChnDataStatus(DmxSet, i);
            if (s32Ret == 2)//channel buffer > 20% of dmx pool buffer
            {
                if (GetChnAcqTimeStatus(DmxSet, i) >= 1)
                { 
                    DMX_OsiResetChannel(DmxSet, i, DMX_FLUSH_TYPE_REC_PLAY);//reset channel
            ...
            {
                if (GetChnAcqTimeStatus(DmxSet, i) == 2)// > 5s not receive data
                {
                    DMX_OsiResetChannel(DmxSet, i, DMX_FLUSH_TYPE_REC_PLAY);//reset channel
                    HI_DRV_SYS_GetTimeStampMs(&ChanInfo->u32AcqTime);
...
static HI_S32 DMX_OsiInitDmxSet(Dmx_Set_S * DmxSet)
{
    ...
    for (i = 0; i < DmxSet->DmxChanCnt; i++)
    {
        ...
        DmxHalSetChannelPlayDmxid(DmxSet, i, DMX_INVALID_DEMUX_ID);
        DmxHalSetChannelRecDmxid(DmxSet, i, DMX_INVALID_DEMUX_ID);
        DmxHalSetChannelPid(DmxSet, i, DMX_INVALID_PID);
...
static HI_VOID DmxStartCluster(Dmx_Cluster_S *DmxCluster)
{
    ...
    DmxHalEnableMmu(&DmxCluster->mmu);
...
#ifdef DMX_MMU_SUPPORT  
#define INIT_MMU_DEV(Mmu) ({ \
    Mmu->IoBase = ioremap_nocache(DMX_MMU_BASE, 0x1000); \
    BUG_ON(!Mmu->IoBase); \
    Mmu->CbTtbr = 0;\
})

#define UNIT_MMU_DEV(Mmu) ({ \
    BUG_ON(!Mmu->IoBase); \
    iounmap(Mmu->IoBase); \
})
#else
...
HI_S32 DMX_OsiInit(HI_VOID)
{
    ...
    INIT_MMU_DEV((&g_DmxClusterCtl->mmu));   
...
out2:    
    ...
    DmxSet1 = HI_NULL;
out1:
    UNIT_MMU_DEV((&g_DmxClusterCtl->mmu));
...
HI_S32 DMX_OsiTsBufferCreate(const HI_U32 PortId, const HI_U32 Size, DMX_MMZ_BUF_S *TsBuf)
{
    ...
    DMX_RamPort_Info_S *PortInfo = HI_NULL;
#ifdef DMX_TS_BUF_VERSION_2    
    HI_U32 BufSize = Size / DMX_MAX_BLOCK_SIZE * DMX_MAX_BLOCK_SIZE;
#else
...
HI_S32 DMX_OsiTsBufferPush(const HI_U32 PortId, DMX_DATA_BUF_S *pstData)
{
    ...
    DMX_OsrSaveIPTs(BufferVirAddr, WriteLen, PortId);
...
#ifdef HI_DEMUX_PROC_SUPPORT
    DMX_OsrSaveIPTs(PortInfo->KerAddr + Offset + StartPos, DataLen, PortId);
...
HI_S32 DMX_OsiGetChannelDescrambleFlag(HI_U32 DmxId, HI_U32 ChanId, HI_BOOL *pEnable)
{
    HI_S32 ret = HI_SUCCESS;
    Dmx_Set_S *DmxSet = GetDmxSetByDmxid(DmxId);
    ...
    DmxHalGetChannelDescrambleFlag(DmxSet, ChanId, pEnable);
...
HI_S32 DMX_OsiReadEsRequest(HI_U32 DmxId, HI_U32 ChanId, DMX_Stream_S *EsData)
{
    ...
    if (HI_SUCCESS == ret)
    {
        if (HI_UNF_DMX_SECURE_MODE_TEE == ChanInfo->ChanSecure)
        {
            if (HI_UNF_DMX_CHAN_TYPE_AUD == ChanInfo->ChanType)
            {
                ret = DmxTeecFixupAudSecBuf(ChanInfo, EsData->u32BufPhyAddr, EsData->u32BufLen);
                if (HI_SUCCESS == ret)
                {
                    DMX_OQ_Info_S *OqInfo = &DmxSet->DmxOqInfo[ChanInfo->ChanOqId];
                    DMX_FQ_Info_S *FqInfo = &DmxSet->DmxFqInfo[OqInfo->u32FQId];
...
static HI_S32 __DMX_OsiSelectDataFlag(HI_HANDLE *WatchHandleSet, HI_BOOL *ValidFlag, HI_U32 WatchCnt, HI_U32 TimeOutMs)
{
    HI_S32 ret = HI_FAILURE;
    HI_U32 index;
...
HI_S32 DMX_OsiSelectDataFlag(HI_HANDLE *WatchHandleSet, HI_U32 WatchCnt, HI_HANDLE *ValidHandleSet, HI_U32 *ValidCnt, HI_U32 TimeOutMs)
{
    ...
    do
    {
        ...
        ret = __DMX_OsiSelectDataFlag(WatchHandleSet, ValidFlag, WatchCnt, TimeOutMs);
        if (HI_SUCCESS == ret)
        {
            break;
        }
    ...
    if (HI_SUCCESS == ret)
    {
        for(index = 0; index < WatchCnt; index++)
        {
            ...
            {
                /* WatchHandleSet and ValidHandleSet maybe point to same buffer, update itself is ok here. */
                put_user(WatchHandleSet[index], &ValidHandleSet[cnt]);
                
                cnt ++;
            }
        }

        put_user(cnt, ValidCnt);
    }
...
HI_S32 DMX_OsiGetDataFlag(HI_U32 *ValidHandleSet, HI_U32 *ValidCnt, HI_U32 TimeOutMs)
{
    ...
    return DMX_OsiSelectDataFlag(WatchHandle, cnt > *ValidCnt ? *ValidCnt : cnt, ValidHandleSet, ValidCnt, TimeOutMs);
}
...
static HI_VOID DmxSetRecBuf(Dmx_Set_S *DmxSet, HI_U32 RecId, HI_U32 OqId)
{
    ...
    /* for compitable old create rec channel way(create -> setpid -> open), not by addrecpid. */
    for_each_set_bit(ChnId, DmxSet->ChnBitmap, DmxSet->DmxChanCnt)
    {
        if (HI_SUCCESS == DmxGetChnInstance(DmxSet, ChnId))
        {
            ChanInfo = &DmxSet->DmxChanInfo[ChnId];
            
            if (ChanInfo->DmxId == RecInfo->DmxId && (HI_UNF_DMX_CHAN_OUTPUT_MODE_REC & ChanInfo->ChanOutMode))
            {
                ChanInfo->ChanRecOqId = OqId;
...
static HI_S32 DmxStartSCD(Dmx_Set_S *DmxSet, HI_U32 RecId)
{
    ...
    /* for compitable old create rec channel (create -> setpid -> open) way, not by addrecpid. */
    for_each_set_bit(ChnId, DmxSet->ChnBitmap, DmxSet->DmxChanCnt)
    {
        if (HI_SUCCESS == DmxGetChnInstance(DmxSet, ChnId))
        {
...
static HI_VOID DmxRecFlush(Dmx_Set_S *DmxSet, HI_U32 RecId)
{
    ...
    /* for compitable old create rec channel (create -> setpid -> open)*/
    for_each_set_bit(ChnId, DmxSet->ChnBitmap, DmxSet->DmxChanCnt)
    {
        if (HI_SUCCESS == DmxGetChnInstance(DmxSet, ChnId))
        {
...
static HI_S32 DmxAcquireRecIndex(Dmx_Set_S *DmxSet, HI_U32 RecId, HI_UNF_DMX_REC_INDEX_S *RecIndex, HI_U32 Timeout)
{
    DMX_RecInfo_S  *RecInfo = &DmxSet->DmxRecInfo[RecId];
    DMX_FQ_Info_S  *FqInfo  = HI_NULL;
    DMX_OQ_Info_S  *OqInfo  = HI_NULL;
        ...
        if (HI_UNF_DMX_REC_INDEX_TYPE_VIDEO == RecInfo->IndexType)
        {
            for (i = 0; i < ScCount; i++)
            {
                if (HI_SUCCESS == DmxScdToVideoIndex(bUseTimeStamp,ScData, &EsScd))
                {
                    ...
                    {   
                        if (HI_SUCCESS != DmxFixupHevcIndex(DmxSet, RecInfo, &EsScd))
                        {
                            continue;
                        }
                    }
                    ...
                    if (u64LastFrameHeadOffset < CurrFrame->u64GlobalOffset)
                    {     
...
static HI_S32 DMXAcquireRecDataIndex(Dmx_Set_S *DmxSet, HI_U32 RecId, HI_UNF_DMX_REC_DATA_INDEX_S *pstRecDataIdx)
{
    HI_S32 ret = HI_FAILURE;
    DMX_RecInfo_S  *RecInfo = &DmxSet->DmxRecInfo[RecId];
    DMX_FQ_Info_S  *FqInfo  = &DmxSet->DmxFqInfo[RecInfo->RecFqId];
...
HI_S32 DMX_DRV_REC_AcquireRecDataIndex(HI_U32 DmxId, HI_U32 RecId, HI_UNF_DMX_REC_DATA_INDEX_S *pstRecDataIdx)
{
    HI_S32 ret = HI_FAILURE;
    Dmx_Set_S *DmxSet = GetDmxSetByDmxid(DmxId);
    DMX_RecInfo_S  *RecInfo = HI_NULL;
...
HI_S32 DMX_DRV_REC_ReleaseRecDataIndex(HI_U32 DmxId, HI_U32 RecId, HI_UNF_DMX_REC_DATA_INDEX_S *pstRecDataIdx)
{
    HI_S32 ret = HI_FAILURE;
    Dmx_Set_S *DmxSet = GetDmxSetByDmxid(DmxId);
    DMX_RecInfo_S  *RecInfo = HI_NULL;
...
HI_S32 DMX_DRV_REC_AcquireRecIndex(HI_U32 DmxId, HI_U32 RecId, HI_UNF_DMX_REC_INDEX_S *RecIndex, HI_U32 Timeout)
{
    HI_S32 ret = HI_FALSE;
    Dmx_Set_S *DmxSet = GetDmxSetByDmxid(DmxId);
    DMX_RecInfo_S  *RecInfo = HI_NULL;
...
HI_S32 DMX_OsiSuspend(PM_BASEDEV_S *himd, pm_message_t state)
{
    ...
    TraverseForEachDmxSet(DmxSet, &DmxCluster->head)
    {
        ...
        for_each_set_bit(RmxChanId, DmxSet->RmxChanBitmap, DmxSet->RmxChanCnt)
        {
           ...
           RMX_OsiStart(RmxChanInfo->RmxId, RmxChanId);
        }
        mutex_unlock(&DmxSet->LockAllRmx);
    }
...
static HI_S32 __DMX_OsiResume(Dmx_Set_S *DmxSet, PM_BASEDEV_S *himd)
{
    ...
    for (i = 0; i < DmxSet->DmxChanCnt; i++)
    {
        ...
        if (ChanInfo->DmxId < GetDmxCluster()->Ops->GetDmxNum())
        {
    ...
    for_each_set_bit(i, DmxSet->RmxChanBitmap, DmxSet->RmxChanCnt)
    {
        ...
        RMX_OsiStop(RmxChanInfo->RmxId, i);
    }
