...
static HI_S32 DMXTsBufferIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        case CMD_DEMUX_TS_BUFFER_INIT:
        {
            ...
            ret = HI_DRV_DMX_CreateTSBuffer(Param->PortId, Param->BufSize, &TsBuf, (HI_VOID *)file);
...
static HI_S32 DMXRecvIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        case CMD_DEMUX_GET_DATA_FLAG:
        {
            ...
            ret = HI_DRV_DMX_GetDataHandle(pPara->ValidChannel, pPara->ValidChannelNum, pPara->u32TimeOutMs);
        ...
        case CMD_DEMUX_SELECT_DATA_FLAG:
        {
            ...
            ret = HI_DRV_DMX_SelectDataHandle(pPara->channel, pPara->channelnum, pPara->ValidChannel, pPara->ValidChannelNum, pPara->u32TimeOutMs);
            break;
        }
...
static HI_S32 Compat_DMXRecvIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        case CMD_DEMUX_COMPAT_GET_DATA_FLAG:
        {
            ...
            ret = HI_DRV_DMX_GetDataHandle(compat_ptr(pPara->ValidChannel), compat_ptr(pPara->ValidChannelNum), pPara->u32TimeOutMs);
            break;
        }
        ...
        case CMD_DEMUX_COMPAT_SELECT_DATA_FLAG:
        {
            ...
            ret = HI_DRV_DMX_SelectDataHandle(compat_ptr(pPara->channel), pPara->channelnum, compat_ptr(pPara->ValidChannel), compat_ptr(pPara->ValidChannelNum), pPara->u32TimeOutMs);
        ...
        case CMD_DEMUX_COMPAT_ACQUIRE_MSG:
        {
            ...
            if (ret == HI_SUCCESS && pPara->u32AcquiredNum)
            {
                ...
                for (i = 0; i < pPara->u32AcquiredNum; i++)
                {
                    pstCompatBuf[i].u32BufStartAddr = pstBufTmp[i].u32BufStartAddr;
                    pstCompatBuf[i].u32MsgLen = pstBufTmp[i].u32MsgLen;
                    pstCompatBuf[i].enDataType = pstBufTmp[i].enDataType;
                }
            }
        ...
        case CMD_DEMUX_COMPAT_RELEASE_MSG:
        {
            ...
            for (i = 0; i < u32ReleaseNum; i++)
            {
                pstBufTmp[i].u32BufStartAddr = pstCompatBuf[i].u32BufStartAddr;
                pstBufTmp[i].u32MsgLen = pstCompatBuf[i].u32MsgLen;
            }
...
static HI_S32 DMXRecIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_DEMUX_REC_CHAN_ACQUIRE_DATA :
        {
            ...
            HI_UNF_DMX_REC_DATA_S RecData = {0};
...
static HI_S32 Compat_DMXRecIoctl(struct file *file, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        case CMD_DEMUX_COMPAT_REC_CHAN_ACQUIRE_DATA_INDEX :
        {
            ...
            HI_UNF_DMX_REC_DATA_INDEX_S *RecDataIdx = HI_NULL;
            HI_U32 index;

            RecDataIdx = HI_KMALLOC(HI_ID_DEMUX, sizeof(HI_UNF_DMX_REC_DATA_INDEX_S), GFP_KERNEL);
            if (!RecDataIdx)
            {
                ret = HI_ERR_DMX_ALLOC_MEM_FAILED;
                break;
            ...
            Param->RecDataIdx.u32IdxNum = RecDataIdx->u32IdxNum;
            memcpy(Param->RecDataIdx.stIndex, RecDataIdx->stIndex, RecDataIdx->u32IdxNum * sizeof(HI_UNF_DMX_REC_INDEX_S));
            Param->RecDataIdx.u32RecDataCnt = RecDataIdx->u32RecDataCnt;

            for (index = 0; index < RecDataIdx->u32RecDataCnt; index ++)
            {
                Param->RecDataIdx.stRecData[index].u32DataPhyAddr = RecDataIdx->stRecData[index].u32DataPhyAddr;
                Param->RecDataIdx.stRecData[index].u32Len = RecDataIdx->stRecData[index].u32Len;
            }
        ...
        case CMD_DEMUX_COMPAT_REC_CHAN_RELEASE_DATA_INDEX :
        {
            ...
            RecDataIdx->u32IdxNum = Param->RecDataIdx.u32IdxNum;
            memcpy(RecDataIdx->stIndex, Param->RecDataIdx.stIndex, RecDataIdx->u32IdxNum * sizeof(HI_UNF_DMX_REC_INDEX_S));
            RecDataIdx->u32RecDataCnt = Param->RecDataIdx.u32RecDataCnt;

            for (index = 0; index < RecDataIdx->u32RecDataCnt; index ++)
            {
                 RecDataIdx->stRecData[index].u32DataPhyAddr = Param->RecDataIdx.stRecData[index].u32DataPhyAddr;
                 RecDataIdx->stRecData[index].u32Len = Param->RecDataIdx.stRecData[index].u32Len;
            }
