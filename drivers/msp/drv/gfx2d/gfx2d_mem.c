...
static spinlock_t gs_stTmpBufLock;
static HI_U32 gs_u32TmpBufPhyaddr = 0;
static HI_S32 gs_s32TmpBufRef = 0;

#define GFX2D_MEM_TMPWIDTH 1920
#define GFX2D_MEM_TMPHEIGHT 1080
#define GFX2D_MEM_TMPSTRIDE (GFX2D_MEM_TMPWIDTH * 4)

/*临时buf大小按照1080p场景分配*/
#define GFX2D_MEM_TMPBUFSIZE (GFX2D_MEM_TMPSTRIDE * GFX2D_MEM_TMPHEIGHT)
...
HI_S32 GFX2D_MEM_Init(const HI_U32 u32Size)
{
	...
	spin_lock_init(&gs_stTmpBufLock);
...
HI_S32 GFX2D_MEM_Deinit(HI_VOID)
{
	...
	if (gs_u32TmpBufPhyaddr != 0)
	{
#ifdef CONFIG_GFX_MMU_SUPPORT
		HI_GFX_FreeMem(gs_u32TmpBufPhyaddr, HI_TRUE);
#else
		HI_GFX_FreeMem(gs_u32TmpBufPhyaddr, HI_FALSE);
#endif
		gs_u32TmpBufPhyaddr = 0x0;
	}
    return HI_SUCCESS;
}
...
HI_S32 GFX2D_Mem_UnRegister(const HI_U32 u32BlockSize)
{
    ...
    memset(&g_struMemBlock[i+1],0,sizeof(MemoryBlock));    
    /*内存块个数-1*/
    g_uRegisterCount--;
    return HI_SUCCESS;
}
...
HI_S32 GFX2D_MEM_GetTmpBuf(HI_U32 u32Size)
{
	unsigned long lockflags;
	HI_U32 u32Phyaddr;
	HI_BOOL bMmu;
	...
	if (u32Size > GFX2D_MEM_TMPBUFSIZE)
	{
		HI_GFX_COMM_LOG_ERROR(HIGFX_GFX2D_ID, "Alloc %d failed!\n", u32Size);
		return 0;
	}

	GFX2D_MEM_Lock(&gs_stTmpBufLock, &lockflags);
	
	if (gs_u32TmpBufPhyaddr != 0)
	{
		gs_s32TmpBufRef++;
		GFX2D_MEM_Unlock(&gs_stTmpBufLock, &lockflags);
		return gs_u32TmpBufPhyaddr;
	}

	GFX2D_MEM_Unlock(&gs_stTmpBufLock, &lockflags);

	u32Phyaddr = HI_GFX_AllocMem("GFX2D_TMP_BUF", pZoneName, GFX2D_MEM_TMPBUFSIZE, &bMmu);
	if (0 == u32Phyaddr)
	{
        HI_GFX_COMM_LOG_ERROR(HIGFX_GFX2D_ID, "Alloc %d failed!\n", GFX2D_MEM_TMPBUFSIZE);
		return u32Phyaddr;
	}
	...
	gs_s32TmpBufRef++;
 	gs_u32TmpBufPhyaddr = u32Phyaddr;
...
HI_VOID GFX2D_MEM_ReleaseTmpBuf(HI_U32 u32Phyaddr)
{
	unsigned long lockflags;
