...
{
    HWC_NODE_S *pstNode[GFX2D_HAL_MAX_NODE] = {NULL, NULL};
    HI_U32 i = 0, j = 0;
    HI_S32 s32Ret = HI_SUCCESS;
    HWC_LAYER_ID aenLayerId[HWC_LAYER_G5] = {0};
	HI_U32 u32NodeCnt = 1;
	HI_U32 u32ZmeLayerCnt = 0;
	HI_GFX2D_COMPOSE_LIST_S stComposeList[GFX2D_HAL_MAX_NODE] = {{0}, {0}};
	HI_GFX2D_COMPOSE_S *pstCompose[GFX2D_HAL_MAX_NODE] = {NULL,NULL};
	HI_GFX2D_SURFACE_S stDstSurface[GFX2D_HAL_MAX_NODE] = {{0},{0}};
	...
	if (u32ZmeLayerCnt > GFX2D_CONFIG_ZME_LAYER)
	{
        HI_GFX_COMM_LOG_ERROR(HIGFX_GFX2D_ID, "unsupport!\n");
        return HI_ERR_GFX2D_UNSUPPORT;
	}
	else if (u32ZmeLayerCnt > gs_stCapability.u32ZmeLayerNum)
	{
		u32NodeCnt += u32ZmeLayerCnt - gs_stCapability.u32ZmeLayerNum;
	}

	if (u32NodeCnt > GFX2D_HAL_MAX_NODE)
	{
        HI_GFX_COMM_LOG_ERROR(HIGFX_GFX2D_ID, "unsupport!\n");
        return HI_ERR_GFX2D_UNSUPPORT;
	}
	...
	for (i = 0; i < u32NodeCnt; i++)
	{
		if (i < u32NodeCnt - 1)
		{
			pstCompose[i] = HI_GFX_KMALLOC(HIGFX_GFX2D_ID,sizeof(HI_GFX2D_COMPOSE_S), GFP_KERNEL);
			if (pstCompose[i] == NULL)
			{
		...
		{
			pstCompose[i] = HI_GFX_KMALLOC(HIGFX_GFX2D_ID,sizeof(HI_GFX2D_COMPOSE_S) * pstComposeList->u32ComposeCnt, GFP_KERNEL);
			if (pstCompose[i] == NULL)
			{
				s32Ret = HI_ERR_GFX2D_NO_MEM;
				goto ERR;
	...
	for (i = 0; i < pstComposeList->u32ComposeCnt; i++)
	{
		if (j != (u32NodeCnt - 1) && pstComposeList->pstCompose[i].stOpt.stResize.bResizeEnable)
		{
			memcpy(&pstCompose[j][0], &pstComposeList->pstCompose[i], sizeof(HI_GFX2D_COMPOSE_S));
			pstCompose[j][0].stOutRect.s32XPos = 0;
			pstCompose[j][0].stOutRect.s32YPos = 0;
		
			/*alloc temp buf*/
			stDstSurface[j].u32Stride[0] = ((4 * pstCompose[j][0].stOutRect.u32Width + gs_stCapability.u8StrideAlign - 1) & (~(gs_stCapability.u8StrideAlign - 1)));

			/*todo:GFX2D_MEM_GetTmpBuf support more than two tmp operations*/
			stDstSurface[j].u32Phyaddr[0] = GFX2D_MEM_GetTmpBuf(stDstSurface[j].u32Stride[0] *  pstCompose[j][0].stOutRect.u32Height);
			if (0 == stDstSurface[j].u32Phyaddr[0])
			{
				s32Ret = HI_ERR_GFX2D_NO_MEM;
				goto ERR;
			}
			stDstSurface[j].enFmt = HI_GFX2D_FMT_ARGB8888;
			stDstSurface[j].u32Width = pstCompose[j][0].stOutRect.u32Width;
			stDstSurface[j].u32Height = pstCompose[j][0].stOutRect.u32Height;

			stComposeList[j].pstCompose = &pstCompose[j][0];
			stComposeList[j].u32ComposeCnt = 1;

			memcpy(&pstCompose[u32NodeCnt - 1][i], &pstComposeList->pstCompose[i], sizeof(HI_GFX2D_COMPOSE_S));
			pstCompose[u32NodeCnt - 1][i].stInRect.s32XPos = 0;
			pstCompose[u32NodeCnt - 1][i].stInRect.s32YPos = 0;
			pstCompose[u32NodeCnt - 1][i].stInRect.u32Width = pstCompose[u32NodeCnt - 1][i].stOutRect.u32Width;
			pstCompose[u32NodeCnt - 1][i].stInRect.u32Height = pstCompose[u32NodeCnt - 1][i].stOutRect.u32Height;

			memcpy(&pstCompose[u32NodeCnt - 1][i].stSurface, &stDstSurface[j], sizeof(HI_GFX2D_SURFACE_S));

			pstCompose[u32NodeCnt - 1][i].stOpt.stResize.bResizeEnable = HI_FALSE;
	...
	memcpy(&stDstSurface[u32NodeCnt - 1], pstDstSurface, sizeof(HI_GFX2D_SURFACE_S));
	stComposeList[u32NodeCnt - 1].pstCompose = &pstCompose[u32NodeCnt - 1][0];
	stComposeList[u32NodeCnt - 1].u32ComposeCnt = pstComposeList->u32ComposeCnt;
	stComposeList[u32NodeCnt - 1].u32BgColor = pstComposeList->u32BgColor;

	for (i = 0; i < u32NodeCnt; i++)
	{
	    s32Ret = CheckCompose(enDevId, &stComposeList[i], &stDstSurface[i],ppNode, penNodeType);
	    if (HI_SUCCESS != s32Ret)
	    	goto ERR;

    	AdjustCompose(enDevId, &stComposeList[i], &stDstSurface[i]);

	    /*选择对应的层ID*/
	    s32Ret = SelectLayerId(&stComposeList[i], aenLayerId);
		if (s32Ret < 0)
			goto ERR;
	    ...
	    for (j = 0; j < stComposeList[i].u32ComposeCnt; j++)
	    {
	    	SetSrc(pstNode[i], &(stComposeList[i].pstCompose[j]), j, aenLayerId[j]);
		}

    	SetDst(pstNode[i], &stDstSurface[i]);

    	SetBgColor(pstNode[i], stComposeList[i].u32BgColor);

#ifdef GFX2D_HISTOGRAM_SUPPORT
		if ((stDstSurface[i].enFmt == HI_GFX2D_FMT_SEMIPLANAR420UV)
        	&& (stDstSurface[i].u32HistogramPhyaddr != 0))
	    {
    ...
    *penNodeType = GFX2D_HAL_DEV_TYPE_HWC;
	...
	for (i = 0; i < u32NodeCnt; i++)
	{
		if (pstCompose[i] != NULL)
		{
			HI_GFX_KFREE(HIGFX_GFX2D_ID,pstCompose[i]);
		}	
	}
...
ERR:
	...
	for (i = 0; i < u32NodeCnt - 1; i++)
	{
		if (stDstSurface[i].u32Phyaddr != 0)
		{
			GFX2D_MEM_ReleaseTmpBuf(stDstSurface[i].u32Phyaddr[0]);
		}
	}
...
static inline HI_VOID SetZme(HWC_NODE_S* pstNode, HI_GFX2D_COMPOSE_S *pstCompose, HWC_LAYER_ID enLayerId)
{
	...
	{
		unVhdCtl.u32All = pstNode->unVhdCtl.u32All;
		unRes.u32All = pstNode->unVhdInRes.u32All;
		unVhdHsp.u32All = pstNode->unVhdHsp.u32All;
		unVhdLumHOffset.u32All = pstNode->unVhdLumHOffset.u32All;
		unVhdChromaHOffset.u32All = pstNode->unVhdChromaHOffset.u32All;
		unVhdVsp.u32All = pstNode->unVhdVsp.u32All;
		unVhdZmeOutRes.u32All = pstNode->unVhdZmeOutRes.u32All;
		unVhdZmeInRes.u32All = pstNode->unVhdZmeInRes.u32All;
#if HWC_VERSION == 0x400
		unVhdCropPos.u32All = pstNode->unVhdCropPos.u32All;
		unVhdCropInReso.u32All = pstNode->unVhdCropInReso.u32All;
		unVhdCropOutReso.u32All = pstNode->unVhdCropOutReso.u32All;
#endif
	}
#if HWC_VERSION == 0x400
	else if (enLayerId >= HWC_LAYER_G0)
	{
		...
		unCtrl.u32All = pstLayer->unCtrl.u32All;
		unRes.u32All = pstLayer->unRes.u32All;
		unVhdHsp.u32All = pstLayer->unG0Hsp.u32All;
		unVhdLumHOffset.u32All = pstLayer->unLumHOffset.u32All;
		unVhdChromaHOffset.u32All = pstLayer->unChromaHOffset.u32All;
		unVhdVsp.u32All = pstLayer->unVsp.u32All;
		unVhdZmeOutRes.u32All = pstLayer->unZmeOutRes.u32All;
		unVhdZmeInRes.u32All = pstLayer->unZmeInRes.u32All;
		unVhdCropPos.u32All = pstLayer->unCropPos.u32All;
		unVhdCropInReso.u32All = pstLayer->unCropInReso.u32All;
		unVhdCropOutReso.u32All = pstLayer->unCropOutReso.u32All;
	}
#endif
    ...
    if (u32HStep >= (16 << GFX2D_HAL_HWC_HSTEP_FLOATLEN))
    {
		...
		if (enLayerId == HWC_LAYER_VHD)
		{
        	unVhdCtl.stBits.bit1ZmehDwsm = HI_TRUE;
        	unVhdCtl.stBits.bit4ZmehDwsMode = HdwsmFactor - 1;
		}
#if HWC_VERSION == 0x400
		else
		{
        	unCtrl.stBits.bit1ZmehDwsm = HI_TRUE;
        	unCtrl.stBits.bit4ZmehDwsMode = HdwsmFactor - 1;
		}
#endif
    }
             ...
             || ((pstSurface->enFmt >= HI_GFX2D_FMT_YUV888) && (u32InWidth > 1920) && (pstCompose->stOutRect.u32Width > 960)))
    {
		...
		{
	        unVhdCtl.stBits.bit1ZmehDwsm = HI_TRUE;
	        unVhdCtl.stBits.bit4ZmehDwsMode = 1;
		}
#if HWC_VERSION == 0x400
		else
		{
	        unCtrl.stBits.bit1ZmehDwsm = HI_TRUE;
	        unCtrl.stBits.bit4ZmehDwsMode = 1;
		}
#endif
    }

    unVhdHsp.stBits.bit1IsLumEnable = HI_TRUE;
    unVhdHsp.stBits.bit1IsChromaEnable = HI_TRUE;
#if HWC_VERSION <= 0x300
    unVhdHsp.stBits.bit1IsNonLinear = 0x0;
#endif
    unVhdHsp.stBits.bit1LumMode = bFilter;
    unVhdHsp.stBits.bit1ChrmoaMode = bFilter;
    unVhdHsp.stBits.bit24Ratio = u32HStep;
    unVhdHsp.stBits.bit1IsLumMid = (u32HStep < GFX2D_HAL_HWC_NOSCALE_HSTEP)?0x1:0x0;
    unVhdHsp.stBits.bit1IsChromaMid = (u32HStep < GFX2D_HAL_HWC_NOSCALE_HSTEP)?0x1:0x0;

    if (pstSurface->enFmt <= HI_GFX2D_FMT_BGRA8888)
    {
        if (pstCompose->stOutRect.u32Width > 1920)
            unVhdHsp.stBits.bit1Order = 0x1;
        else
            unVhdHsp.stBits.bit1Order = 0x0;
    }
    else
    {
    ...
    unVhdLumHOffset.stBits.bit28Offset = 0x0;
    unVhdChromaHOffset.stBits.bit28Offset = 0x0;

    unVhdVsp.stBits.bit1IsLumEnable = 0x1;
    unVhdVsp.stBits.bit1IsChromaEnable = 0x1;
    unVhdVsp.stBits.bit1IsLumMid = (u32VStep < GFX2D_HAL_HWC_NOSCALE_VSTEP)?0x1:0x0;
    unVhdVsp.stBits.bit1IsChromaMid = (u32VStep < GFX2D_HAL_HWC_NOSCALE_VSTEP)?0x1:0x0;
    unVhdVsp.stBits.bit1LumMode = bFilter;
    unVhdVsp.stBits.bit1ChromaMode = bFilter;
    unVhdVsp.stBits.bit2OutFmt = 0x2;
    ...
    {
        unVhdVsp.stBits.bit2InFmt = 0x1;
    }
    else
    {
        unVhdVsp.stBits.bit2InFmt = 0x3;
    }

    unVhdZmeInRes.stBits.bit12Width = u32InWidth - 1;
    unVhdZmeInRes.stBits.bit12Height = u32InHeight - 1;
    unVhdZmeOutRes.stBits.bit12Width = pstCompose->stOutRect.u32Width - 1;
    unVhdZmeOutRes.stBits.bit12Height = pstCompose->stOutRect.u32Height - 1;
	unRes.stBits.bit12Width = u32InWidth * HdwsmFactor - 1;

#if HWC_VERSION == 0x400
	if (pstCompose->stOpt.stClip.enClipMode == HI_GFX2D_CLIP_INSIDE)
	{
		unVhdCropPos.stBits.bit13XPos = pstCompose->stOpt.stClip.stClipRect.s32XPos;
		unVhdCropPos.stBits.bit13YPos = pstCompose->stOpt.stClip.stClipRect.s32YPos;
		unVhdCropInReso.stBits.bit13Width = pstCompose->stOutRect.u32Width - 1;
		unVhdCropInReso.stBits.bit13Height = pstCompose->stOutRect.u32Height - 1;
		unVhdCropOutReso.stBits.bit13Width = pstCompose->stOpt.stClip.stClipRect.u32Width - 1;
		unVhdCropOutReso.stBits.bit13Height = pstCompose->stOpt.stClip.stClipRect.u32Height - 1;
	}
	else
	{
		unVhdCropPos.stBits.bit13XPos = 0;
		unVhdCropPos.stBits.bit13YPos = 0;
		unVhdCropInReso.stBits.bit13Width = pstCompose->stOutRect.u32Width - 1;
		unVhdCropInReso.stBits.bit13Height = pstCompose->stOutRect.u32Height - 1;
		unVhdCropOutReso.stBits.bit13Width = pstCompose->stOutRect.u32Width - 1;
		unVhdCropOutReso.stBits.bit13Height = pstCompose->stOutRect.u32Height - 1;
	}
#endif

	if (enLayerId == HWC_LAYER_VHD)
	{
		pstNode->unVhdCtl.u32All = unVhdCtl.u32All;
		pstNode->unVhdRes.u32All = unRes.u32All;
		pstNode->unVhdHsp.u32All = unVhdHsp.u32All;
		pstNode->unVhdLumHOffset.u32All = unVhdLumHOffset.u32All;
		pstNode->unVhdChromaHOffset.u32All = unVhdChromaHOffset.u32All;
		pstNode->unVhdVsp.u32All = unVhdVsp.u32All;
		pstNode->u32VhdVsr = u32VStep;
    	pstNode->u32VhdVOffset = 0x0;
		pstNode->unVhdZmeOutRes.u32All = unVhdZmeOutRes.u32All;
		pstNode->unVhdZmeInRes.u32All = unVhdZmeInRes.u32All;
#if HWC_VERSION == 0x400
		pstNode->unVhdCropPos.u32All = unVhdCropPos.u32All;
		pstNode->unVhdCropInReso.u32All = unVhdCropInReso.u32All;
		pstNode->unVhdCropOutReso.u32All = unVhdCropOutReso.u32All;
#endif
		if (bFilter)
	    {
	        ...
	        if (0x0 == unVhdVsp.stBits.bit1ChromaTap)
	        {
	...
	{
		HWC_GLAYER_S *pstLayer = &(pstNode->astGLayer[enLayerId - HWC_LAYER_G0]);
		pstLayer->unCtrl.u32All = unCtrl.u32All;
		pstLayer->unRes.u32All = unRes.u32All;
		pstLayer->unG0Hsp.u32All = unVhdHsp.u32All;
		pstLayer->unLumHOffset.u32All = unVhdLumHOffset.u32All;
		pstLayer->unChromaHOffset.u32All = unVhdChromaHOffset.u32All;
		pstLayer->unVsp.u32All = unVhdVsp.u32All;
		pstLayer->u32Vsr = u32VStep;
		pstLayer->u32VOffset = 0x0;
		pstLayer->unZmeOutRes.u32All = unVhdZmeOutRes.u32All;
		pstLayer->unZmeInRes.u32All = unVhdZmeInRes.u32All;
		pstLayer->unCropPos.u32All = unVhdCropPos.u32All;
		pstLayer->unCropInReso.u32All = unVhdCropInReso.u32All;
		pstLayer->unCropOutReso.u32All = unVhdCropOutReso.u32All;
...
static HI_S32 SelectLayerId(HI_GFX2D_COMPOSE_LIST_S *pstComposeList, HWC_LAYER_ID *penLayerId)
{
    ...
    for (i = 0; i < pstComposeList->u32ComposeCnt; i++)
    {
		...
		/*缩放，优先选择视频层，然后图形层*/
		else if ((HI_GFX2D_SURFACE_TYPE_MEM == pstCompose->stSurface.enType)
			&& (pstCompose->stOpt.stResize.bResizeEnable))
		{
			/*图形层不支持缩放，则选择视频层*/
			...
			{
				if (au8Ref[HWC_LAYER_G0] == 0)
					penLayerId[i] = HWC_LAYER_G0;
				else if (au8Ref[HWC_LAYER_VHD] == 0)
					penLayerId[i] = HWC_LAYER_VHD;
				else
					goto ERR;				
			}
        ...
        {
        	for (j = HWC_LAYER_G1; j <= HWC_LAYER_G5; j++)
    		{
    			if (au8Ref[j] == 0)
				{
					penLayerId[i] = j;
                    ...
                    HI_VOID **ppNode, GFX2D_HAL_DEV_TYPE_E *penNodeType)
{
    ...
    for (i = 0; i < pstComposeList->u32ComposeCnt; i++)
    {
        pstSurface = &(pstComposeList->pstCompose[i].stSurface);
        pstInRect = &(pstComposeList->pstCompose[i].stInRect);
        pstOutRect = &(pstComposeList->pstCompose[i].stOutRect);
		...
		if (pstSurface->enType != HI_GFX2D_SURFACE_TYPE_COLOR)
		{
        ...
        {
            s32Ret = CheckResize(&(pstComposeList->pstCompose[i]));
            if (HI_SUCCESS != s32Ret)
            {
                return s32Ret;
            }
            ...
            else if ((pstInRect->u32Width < gs_stCapability.u32MinVWidth)
                || (pstInRect->u32Height < gs_stCapability.u32MinVHeight))
            {
            	u32GLayerNum++;
            }
    ...
    if ((u32VhdLayerNum > gs_stCapability.u8MaxVLayerNum)
        ...
        || (u32ZmeLayerNum > GFX2D_CONFIG_ZME_LAYER))
    {
        HI_GFX_COMM_LOG_ERROR(HIGFX_GFX2D_ID, "unsupport!\n");
        return HI_ERR_GFX2D_UNSUPPORT;
    }
