...
extern VENC_BUFFER_S g_stKernelOmxmmz[OMX_OUTPUT_PORT_BUFFER_MAX];
 ...
 HI_VOID VENC_DRV_EflRcCalPFrmQp(VeduEfl_Rc_S *pRc, VeduEfl_EncPara_S *pEncPara, HI_S32 FrameIMBRatio)
{
...
#if 0	    //old version for Tanyixin
      if(pRc->StreamBuffer[29])
		{
		    if(pRc->BufferBits*10 > u32BitRate*11)
		    {
		        pRc->TargetBits = pRc->ConsAvePBits*9/10;
		    }
			else if(pRc->BufferBits*10 < u32BitRate*9)
			{
	...
	else
	{
...
#if 0	  //tyx don't let the bitrate increase when move scence.
	  ...
	  //if((pRc->StillMoveNum <= 0) && (Gop > 10) && (pRc->FrmNumInGop > Gop - 10))
	  #if 1
...
HI_S32 VENC_DRV_EflDestroyVenc( VeduEfl_EncPara_S* EncHandle )
{
    ...
    if (pEncPara->OMXChn)
    {
	    ...
	    {
            if ( 0 != g_stKernelOmxmmz[i].pu8StartVirAddr)
	        {
               sMBufVenc_temp.pu8StartVirAddr = g_stKernelOmxmmz[i].pu8StartVirAddr;
...
HI_S32 VENC_DRV_EflRequestIframe( VeduEfl_EncPara_S* EncHandle )
{
    VeduEfl_EncPara_S*  pEncPara = EncHandle;
...
#define VEDU_CLIP3(x, y, z) (((x) < (y)) ? (y) : (((x) > (z)) ? (z) : (x)))
    ...
    if( pEncPara->RcStart )
    {
        //pEncPara->StartQp  = pEncPara->Protocol == VEDU_H264 ? 36 : 12;    //lmq_0124
    }
    else
    {
#endif    
...
#if 0
           if(pEncPara->stRc.TotalBitsLeft > ( pEncPara->stRc.GopBits*12/10))
           {
                if(pEncPara->stRc.TotalBitsLeft <( pEncPara->stRc.GopBits*14/10))
                {
                     pEncPara->stRc.GopBitsLeft = pEncPara->stRc.TotalBitsLeft -  pEncPara->stRc.GopBits*2/10;
                 }
                else
                {
                     pEncPara->stRc.GopBitsLeft = pEncPara->stRc.GopBitsLeft*11/10;
                }
           }
...
#if 0  /*tanyixin old*/
    Updata_StreamBuffer(pEncPara->stRc.StreamBuffer, pEncPara->PicBits );
    if(pEncPara->stRc.StreamBuffer[29])
    {
        pEncPara->stRc.BufferBits = pEncPara->VoFrmRate * VENC_DRV_EflRcAverage(pEncPara->stRc.StreamBuffer, 30);
    }
    else
    {
        pEncPara->stRc.BufferBits = pEncPara->stRc.FrmNumSeq * VENC_DRV_EflRcAverage(pEncPara->stRc.StreamBuffer,pEncPara->stRc.FrmNumSeq);
    }
    if(pEncPara->stRc.StreamBuffer[29])
    {
        if(pEncPara->stRc.BufferBits*100 > pEncPara->BitRate *105)
        {
            pEncPara->stRc.CurQp++;
        }
        else if(pEncPara->stRc.BufferBits*100 < pEncPara->BitRate *95)
        {
            pEncPara->stRc.CurQp--;
        }
    }
#else  //change by liminqi
...
#ifdef HI_SMMU_SUPPORT
			 pEncIn->TunlCellAddr = pEncPara->stImage.u32TunnelPhyAddr;  //VeduEfl_SmmuToPhyAddr(pEncPara->u32SmmuPageBaseAddr,pEncPara->stImage.u32TunnelPhyAddr);
#else			 
   ...
   else
   {
        ...
        != VENC_DRV_EflRcFrmRateCtrl( g_stVencChn[u32ChnID].hVEncHandle, pEncPara->stStat.PutFrameNumOK/*pImagePriv->u32FrmCnt*/))
    {
		   ...
		   pEncPara->stStat.PutFrameNumOK++;
			...
			s32Ret = (pVpssFunc->pfnVpssGetImage)(g_stVencChn[u32ChnID].hVPSS,&stVpssFrame);
		    if (!s32Ret)
		    {
...
//////////////////////// add by w00283384
		...
		if(pEncPara->ICatchEnable)
        {
            ...
            if(ImbRatio > 833)
            {
                if(pEncPara->ICatchOn)
                {
                    pEncPara->ICatchOn = 0;
                }
                else if(pEncPara->FrmCnt > 30)
                {
                    pEncPara->InterFrmCnt = pEncPara->Gop - 1;
                    pEncPara->ICatchOn = 1;
                    g_flag_I_insert = 1;
