...
HI_S32 VENC_Ioctl(struct inode *inode, struct file *file, unsigned int cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_VENC_REQUEST_I_FRAME:
        {
            ...
            if (HI_INVALID_HANDLE == g_stVencChn[u32Index].hJPGE)
            {
        ...
        case CMD_VENC_MMZ_MAP:
        {
            ...
            if (HI_SUCCESS == Ret)
            {
                pMMZPhyInfo->vir2phy_offset = (HI_VIRT_ADDR_T)pMMBInfo.stVencBuf.kernel_viraddr - pMMBInfo.stVencBuf.phyaddr;
            }
...
#if 0
                {
                    HI_U32 diff = 0,targetKbps = 0,realKbps = 0;
                    char Sign[8];
                    targetKbps = g_stVencChn[i].stChnUserCfg.u32TargetBitRate / 1000U;
                    realKbps   = g_stVencChn[i].u32LastSecKbps * 8 / 1000U;

                    if (targetKbps > realKbps)
                    {
                        diff = targetKbps - realKbps;
                        memcpy(Sign,"-",3);
                    }
                    else
                    {
                        diff = realKbps - targetKbps;
                        memcpy(Sign,"+",3);
                    }

                    diff = diff * 100 / targetKbps;

                    HI_ERR_VENC("[chn %d]input frmeRate(%d/%d),out frmRate(%d/%d),Target bps = %d , Real bps = %d, diff = %d%%  %s\n",
                    i,StatInfo.u32RealSendInputRrmRate,g_stVencChn[i].u32LastSecInputFps,
                    StatInfo.u32RealSendOutputFrmRate,g_stVencChn[i].u32LastSecEncodedFps,
                    g_stVencChn[i].stChnUserCfg.u32TargetBitRate / 1000U,
                    g_stVencChn[i].u32LastSecKbps * 8 / 1000U,
                    diff,Sign
                    );
