...
VENC_BUFFER_S g_stKernelOmxmmz[OMX_OUTPUT_PORT_BUFFER_MAX] = {{0, 0, 0}};
...
******************************************************************************/
#if 1   
HI_S32 VENC_DRV_EflMMapToKernel_OMX(HI_MMZ_BUF_S* pMMZbuf)
{
    HI_S32 s32Ret = 0,i = 0;
    ...
    if (HI_SUCCESS == s32Ret)  
    {
       ...
       for(i = 0; i< OMX_OUTPUT_PORT_BUFFER_MAX;i++)
       {
          ...
          {
             g_stKernelOmxmmz[i].pu8StartVirAddr = sMBufVenc.pu8StartVirAddr;
             g_stKernelOmxmmz[i].u32StartPhyAddr = pMMZbuf->phyaddr;
             break;
          }
       }
...
HI_S32 VENC_DRV_EflUMMapToKernel_OMX(HI_MMZ_BUF_S* pMMZbuf)
{
    HI_S32 i = 0;
    VENC_BUFFER_S sMBufVenc   = {0};
    if(!pMMZbuf)
    ...
    for (i = 0;i < OMX_OUTPUT_PORT_BUFFER_MAX; i++)
    {
       if (g_stKernelOmxmmz[i].u32StartPhyAddr == sMBufVenc.u32StartPhyAddr)
       {
          sMBufVenc.pu8StartVirAddr = g_stKernelOmxmmz[i].pu8StartVirAddr;
          g_stKernelOmxmmz[i].u32StartPhyAddr = 0;
          g_stKernelOmxmmz[i].pu8StartVirAddr = 0;
          break;
       }
    }
    ...
    if( (OUTPUT_PORT == u32PortIndex) || (ALL_PORT == u32PortIndex) )
    {
       ...
       while((!VENC_DRV_MngQueueEmpty(pEncPara->StreamQueue_OMX)) && (!s32Ret1))
       {
		  ...
		  if (!s32Ret1 && QueueData.bToOMX)
		  {
             ...
             VENC_DRV_EflPutMsg_OMX(pEncPara->MsgQueue_OMX, VENC_MSG_RESP_OUTPUT_DONE, s32Ret1 , &(QueueData.unFrmInfo.msg_info_omx.buf)); 
	          ...
	          while((!VENC_DRV_MngQueueEmpty(pEncPara->FrameQueue_OMX)) && (!s32Ret2))
	          {
	              s32Ret2 |= VENC_DRV_MngDequeue(pEncPara->FrameQueue_OMX, &QueueData);
		   		  if (!s32Ret2 && QueueData.bToOMX)
		   		  {
		   			 ...
		   			 QueueData.unFrmInfo.msg_info_omx.buf.data_len = 0;
		                VENC_DRV_EflPutMsg_OMX(pEncPara->MsgQueue_OMX, VENC_MSG_RESP_INPUT_DONE, s32Ret2 , &(QueueData.unFrmInfo.msg_info_omx.buf)); 
		   			 pEncPara->stStat.MsgQueueNum++;
		   		  }
