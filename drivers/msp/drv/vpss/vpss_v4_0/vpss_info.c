        ...
        case DBG_SET_I2P:
            switch(pstCmd->hDbgPart)
            {
                case DEF_DBG_SRC_ID:
					...
					printk("set interlace to progress %s !!\n",
            ...
            switch(pstCmd->hDbgPart)
            {
                ...
                case DEF_DBG_PORT2_ID:
					...
					if (HI_DRV_VPSS_ROTATION_BUTT == pstCmd->hDbgValue)
					{
						...
						printk("set prot%d rotation debug disable!!\n", u32Count);
					}
					else
					{
		...
		case DBG_SET_OUTBITWIDTH:
			switch(pstCmd->hDbgPart)
			{
				...
				case DEF_DBG_PORT2_ID:
					...
					if (HI_DRV_PIXEL_BITWIDTH_BUTT == pstCmd->hDbgValue)
					{
						pstPortDbg->unInfo.bits.dbg_output_bitwidth_en = HI_FALSE;

						printk("set prot%d outbitwidth debug disable!!\n", u32Count);
					}
					else
					{
						...
						printk("set prot%d outbitwidth %d bit!!\n", u32Count, pstCmd->hDbgValue*2+8);
					}
            ...
            switch (u32DbgPart)
            {
                ...
                case DEF_DBG_PORT2_ID:
                    ...
                    if (pstPortDbg->unInfo.bits.writeyuv
                        ...
                        //|| (pstFrm->u32FrameIndex < 2))
                    {
                        HI_OSAL_Snprintf(chFile,
                                DEF_FILE_NAMELENGTH, "vpss_p%d_%dX%d_idx%d_%02d_%02d_%02d.yuv",u32Count,
