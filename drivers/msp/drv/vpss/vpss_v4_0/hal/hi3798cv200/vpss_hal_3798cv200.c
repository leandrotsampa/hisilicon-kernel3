...
SQO5jL3QmNV6rqg7A0WrfnKmviw7F2LG6XOJpbpXkr9eWx9JZxnEa2XNrn9vrkSN99TkHvUb
qPjoT99ww1rqZ7axAqTKmc+lE5aCMHjGYuFubXy0yJIfkztLxbjqKOa11g+a7w==#*/
    ...
    HI_U32* pu32AppVir, HI_U32 u32AppPhy)
{
    ...
    {
    	VPSS_REG_SetImgAddr(pu32AppVir,NEXT2_FIELD,
                        pstCur->u32PhyAddr_Y-pstCur->u32Stride_Y,
                        pstCur->u32PhyAddr_C-pstCur->u32Stride_Y);
    }
    else
    {
    	VPSS_REG_SetImgAddr(pu32AppVir,NEXT2_FIELD,
...
HI_S32 VPSS_HAL_SetHDRELNode(VPSS_IP_E enIP, VPSS_HAL_INFO_S *pstHalInfo,
    HI_U32 *pu32AppVir, HI_U32 u32AppPhy)
{
#if 1
    HI_DRV_VID_FRAME_ADDR_S *pstCur = HI_NULL;
...
HI_S32 VPSS_HAL_AllocDetileBuffer(VPSS_IP_E enIP)
{
    ...
    {
        s32Ret = HI_DRV_SMMU_AllocAndMap("VPSS_DETILE_BUF", 1920*1080*2, 0, &(pstHalCtx->stDeTileMMUBuf));
...
HI_VOID VPSS_HAL_FreeDetileBuffer(VPSS_IP_E enIP)
{
    ...
    if (pstHalCtx->stDeTileMMUBuf.u32Size != 0)
    {
        HI_DRV_SMMU_UnmapAndRelease(&(pstHalCtx->stDeTileMMUBuf));
    ...
    HI_U32* pu32AppVir,HI_U32 u32AppPhy)
{
    ...
    s32Ret = VPSS_HAL_AllocDetileBuffer(enIP);
    if (s32Ret != HI_SUCCESS)
    {
...
HI_S32 VPSS_HAL_GetIntState(VPSS_IP_E enIP, HI_U32* pu32IntState)
{
    ...
    if (u32SmmuSeIntState != 0 || u32SmmuNSIntState != 0)
    {
        HI_PRINT("SMMU_WRITE_ERR MODULE : %s  NSSTATE : %#x SSTATE : %#x RdAddr:%#x : WrAddr:%#x\n",
            HI_MOD_VPSS,u32SmmuNSIntState,u32SmmuSeIntState,
     ...
     VPSS_HAL_INFO_S *pstHalInfo,  VPSS_HAL_TASK_NODE_E enTaskNodeId)
{
	...
	//printk("pstHalInfo->enNodeType=%d\n",pstHalInfo->enNodeType);
	switch(pstHalInfo->enNodeType)
    {
        ...
        case VPSS_HAL_NODE_HDR_EL:
            s32Ret = VPSS_HAL_SetHDRELNode(enIP, pstHalInfo,  pu32AppVir,u32AppPhy);
            break; 
#ifdef ZME_2L_TEST
