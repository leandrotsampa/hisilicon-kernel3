...
SQO5jMRCtJrY7CicaL01Jq2XNxg2a3NDwuuwNiCjBLmKFESjvFNuCKU5StxP7RypzACgtDSM
aHZMYNiMLLhtgnVYztqtcLjVDEdSkF+L/+CL3SOZpbm/cO74EN9vUdPzA5tewQ==#*/
...
HI_S32 VPSS_IN_VFREE_V3(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    if (pstEntity->pstMcdeiInfo[0])
        VPSS_VFREE(pstEntity->pstMcdeiInfo[0]);
    if (pstEntity->pstMcdeiInfo[1])
        VPSS_VFREE(pstEntity->pstMcdeiInfo[1]);
...
HI_S32 VPSS_IN_VMALLOC_V3(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    pstEntity->pstMcdeiInfo[0] = (VPSS_MCDEI_INFO_S *)VPSS_VMALLOC(sizeof(VPSS_MCDEI_INFO_S));
    if (HI_NULL == pstEntity->pstMcdeiInfo[0])
    {
        VPSS_ERROR("malloc pstMcdeiInfo failed\n");
        goto V3_VMALLOC_ERROR;
    }
    memset(pstEntity->pstMcdeiInfo[0],0,sizeof(VPSS_MCDEI_INFO_S));

    pstEntity->pstMcdeiInfo[1] = (VPSS_MCDEI_INFO_S *)VPSS_VMALLOC(sizeof(VPSS_MCDEI_INFO_S));
    if (HI_NULL == pstEntity->pstMcdeiInfo[1])
    {
        VPSS_ERROR("malloc pstMcdeiInfo failed\n");
        goto V3_VMALLOC_ERROR;
    }
    memset(pstEntity->pstMcdeiInfo[1],0,sizeof(VPSS_MCDEI_INFO_S));
...
HI_S32 VPSS_IN_ReviseImage(VPSS_IN_ENTITY_S *pstEntity,HI_DRV_VIDEO_FRAME_S *pImage)
{
    ...
    pImage->u32Width  =  pImage->u32Width & 0xfffffffe;
...
HI_VOID VPSS_IN_ClearAllAlgBuffer(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    pstMcdeiInfo = pstEntity->pstMcdeiInfo[0];
    ...
    (HI_VOID)VPSS_INST_McDeiDeInit(pstEntity->pstMcdeiInfo[1]);
    ...
    */
    ...
    if (HI_TRUE != bSupport)
	{
		...
		else if (pstLastPriv->u32LastFlag == DEF_HI_DRV_VPSS_LAST_ERROR_FLAG)
		{
			...
			if (pstEntity->pstWbcInfo[0] != HI_NULL 
					&& pstEntity->pstWbcInfo[0]->bInit == HI_TRUE)
			{
    ...
    {   
        ...
        pstMcdeiInfo = (VPSS_MCDEI_INFO_S *)pstEntity->pstMcdeiInfo[0];
        ...
        else
        {
            if (HI_DRV_FT_NOT_STEREO == pstImage->eFrmType)
            {
                ...
                s32InitListRet |= VPSS_STTINFO_SttWbcInit(psttWbc);
//                (HI_VOID)VPSS_INST_McDeiDeInit(pstMcdeiInfo + 1); 
            }
            else
            {
                VPSS_OSAL_DownSpin(&(pstEntity->stSrcSpin),&flags);
				...
				s32InitListRet |= VPSS_INST_McDeiInit((pstEntity->pstMcdeiInfo[1]),pstImage); 
            }
        }
    ...
    switch(enType)
    {
        ...
        case VPSS_IN_INFO_BLEND_REF:
                ...
                || HI_DRV_BUF_ADDR_RIGHT == enLR )
            {
                ...
                ppstRef = (HI_DRV_VIDEO_FRAME_S **)pstInfo;
                pstBlend = &pstEntity->pstMcdeiInfo[enLR]->stBlendWbcInfo;
        ...
        case VPSS_IN_INFO_RGME:
                ...
                || HI_DRV_BUF_ADDR_RIGHT == enLR )
            {
                ...
                pstStRgme = &pstEntity->pstMcdeiInfo[enLR]->stRgmeInfo;
        ...
        case VPSS_IN_INFO_BLKMV:
                ...
                || HI_DRV_BUF_ADDR_RIGHT == enLR )
            {
                ...
                pstStBlkmv = &pstEntity->pstMcdeiInfo[enLR]->stBlkmvInfo;
        ...
        case VPSS_IN_INFO_PRJH:
                ...
                || HI_DRV_BUF_ADDR_RIGHT == enLR )
            {
                ...
                pstStPrjh = &pstEntity->pstMcdeiInfo[enLR]->stPrjhInfo;
        ...
        case VPSS_IN_INFO_PRJV:
                ...
                || HI_DRV_BUF_ADDR_RIGHT == enLR )
            {
                ...
                pstStPrjv = &pstEntity->pstMcdeiInfo[enLR]->stPrjvInfo;
		...
		{		
			...
			switch(pstHalInfo->enNodeType)
			{
				...
				case VPSS_HAL_NODE_2D_DETILE_STEP1:				
                    ...
                    pstHalInfo = (VPSS_HAL_INFO_S *)pstInfo;
                    pstMcdeiInfo = pstEntity->pstMcdeiInfo[enLR];
...
{   
    ...
    VPSS_CHECK_NULL(pstEntity->pstMcdeiInfo);
    ...
    if(pstEntity->stStreamInfo.u32StreamW <= 1920)
    {
        ...
        if(pstEntity->stStreamInfo.eStreamFrmType != HI_DRV_FT_NOT_STEREO)
        {
            ...
            VPSS_INST_McDeiComplete(pstEntity->pstMcdeiInfo[1]);
        }
    }
    else
    {
...
HI_S32 VPSS_IN_Reset_V3(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    if(pstEntity->stStreamInfo.u32StreamW <= 1920)
    {
        ...
        if(pstEntity->stStreamInfo.eStreamFrmType != HI_DRV_FT_NOT_STEREO)
        {
            ...
            VPSS_INST_McDeiReset(pstEntity->pstMcdeiInfo[1]);
        }
    }
    else
    {
        VPSS_STTINFO_SttWbcReset(pstEntity->pstSttWbc[0]);
    }
    ...
    if(pstEntity->stStreamInfo.u32StreamW <= 1920)
    {
        ...
        if(pstEntity->stStreamInfo.eStreamFrmType != HI_DRV_FT_NOT_STEREO)
        {
            ...
            VPSS_INST_McDeiDeInit(pstEntity->pstMcdeiInfo[1]);
        }
    }
	else
	{
...
HI_S32 VPSS_IN_DeInit(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    if (pstEntity->enVersion == VPSS_VERSION_V4_0)
    {
        ...
        VPSS_CHECK_NULL(pstEntity->pstMcdeiInfo[0]);
