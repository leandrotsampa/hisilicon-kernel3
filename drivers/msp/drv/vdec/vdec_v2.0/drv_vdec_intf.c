...
static HI_S32 VDEC_DRV_CtrlReadProc(struct seq_file *p, HI_VOID *v)
{
    ...
    if (stSpecialFrmProc.u32SpecialFrmNum != 0)
    {
         ...
         {
		   PROC_PRINT(p, "%-35s:0x%x/%p/%d\n", "PhyAddr/VirAddr/Size",  stSpecialFrmProc.frmBufRec[i].u32StartPhyAddr,stSpecialFrmProc.frmBufRec[i].pu8StartVirAddr,stSpecialFrmProc.frmBufRec[i].u32Size);
		 }
    }
#endif	
    PROC_PRINT(p, "\n");
                                 ...
                                 const char __user * buffer, size_t count, loff_t *ppos)
{
    ...
    else if(0==strncmp(dat1,"bypass",7))
    {
        ...
        {
            MaskCtrlWord = MaskCtrlWord | (MaskBit);
            HI_DRV_PROC_EchoHelper("Without vpss.\n");
        }
        else if(0==strncmp(dat2,"off",3))
        {
            MaskCtrlWord = MaskCtrlWord & (~MaskBit);
            HI_DRV_PROC_EchoHelper("With vpss.\n");
        }
        else
        {
        }
    ...
    else if(0==strncmp(dat1,"maskcmp",7))
    {
        ...
        {
            MaskCtrlWord = MaskCtrlWord | (MaskBit);
            HI_DRV_PROC_EchoHelper("Mask cmp.\n");
        }
        else if(0==strncmp(dat2,"off",3))
        {
            MaskCtrlWord = MaskCtrlWord & (~MaskBit);
            HI_DRV_PROC_EchoHelper("Open cmp.\n");
        }
...
static HI_S32 VDEC_DRV_ReadProc(struct seq_file *p, HI_VOID *v)
{
    ...
    HI_CHAR aszSrcFrmType[32];
    HI_CHAR aszSrcColorSpace[32];
    HI_CHAR aszSrcGammaType[32];
    ...
    if (pstChan)
    {
        ...
        switch (pstChan->stLastFrm.enSrcFrameType)
        {
        case HI_DRV_VIDEO_FRAME_TYPE_SDR:
            snprintf(aszSrcFrmType, sizeof(aszSrcFrmType), "SDR");
            break;
        case HI_DRV_VIDEO_FRAME_TYPE_DOLBY_BL:
            snprintf(aszSrcFrmType, sizeof(aszSrcFrmType), "DOLBY_BL");
            break;
        case HI_DRV_VIDEO_FRAME_TYPE_DOLBY_EL:
			snprintf(aszSrcFrmType, sizeof(aszSrcFrmType), "DOLBY_EL");
			break;
        case HI_DRV_VIDEO_FRAME_TYPE_HDR10:
            snprintf(aszSrcFrmType, sizeof(aszSrcFrmType), "HDR10");
            break;
        default:
            snprintf(aszSrcFrmType, sizeof(aszSrcFrmType), "UNKNOWN");
            break;
        }

        switch (pstChan->stLastFrm.unHDRInfo.stHDR10Info.enGammaType)
        {
        case HI_DRV_GAMMA_TYPE_1886:
            snprintf(aszSrcGammaType, sizeof(aszSrcGammaType), "GAMMA_1886");
            break;
        case HI_DRV_GAMMA_TYPE_2084:
            snprintf(aszSrcGammaType, sizeof(aszSrcGammaType), "GAMMA_2084");
            break;
        default:
            snprintf(aszSrcGammaType, sizeof(aszSrcGammaType), "UNKNOWN");
            break;
        }

        switch (pstChan->stLastFrm.unHDRInfo.stHDR10Info.enColorSpace)
        {
        case HI_DRV_CS_BT2020_YUV_LIMITED:
            snprintf(aszSrcColorSpace, sizeof(aszSrcColorSpace), "BT2020");
            break;
        default:
            snprintf(aszSrcColorSpace, sizeof(aszSrcColorSpace), "UNKNOWN");
            break;
        }
        ...
        if (HI_INVALID_HANDLE == pstChan->hDmxVidChn)
        {
           ...
           if(HI_SUCCESS == s32Ret)
           {
                u64UsedTmp = stBMStatus.u32Used*100;
                do_div(u64UsedTmp, (stBMStatus.u32Free+stBMStatus.u32Used));
                PROC_PRINT(p,
        ...
        PROC_PRINT(p,"--------------------------Other Information-------------------------\n"
                        "SrcFrameType                        : %s\n"
                        "SrcColorSpace                       : %s\n"
                        "SrcGammaType                        : %s\n",aszSrcFrmType, aszSrcColorSpace, aszSrcGammaType);
