...
HI_S32 BUFMNG_Create(HI_HANDLE *phBuf, BUFMNG_INST_CONFIG_S *pstConfig)
{
    ...
    if (BUFMNG_ALLOC_INNER == pstConfig->enAllocType)
    {
...
#endif
        {
            ...
            {
                s32Ret = HI_DRV_VDEC_Alloc_TVP(pstConfig->aszName, HI_NULL, pstConfig->u32Size, 0, &stVDECAllocBuf);
                ...
                pstConfig->u32PhyAddr = stVDECAllocBuf.u32StartPhyAddr;
                pstConfig->pu8KnlVirAddr = (HI_U8*)stVDECAllocBuf.u32StartPhyAddr;
                pstConfig->pu8UsrVirAddr = (HI_U8*)stVDECAllocBuf.u32StartPhyAddr;
            }
            else
#endif
            {
                s32Ret = HI_DRV_VDEC_AllocAndMap(pstConfig->aszName, HI_NULL, pstConfig->u32Size, 0, &stVDECAllocBuf);
                ...
                pstConfig->u32PhyAddr = stVDECAllocBuf.u32StartPhyAddr;
                pstConfig->pu8KnlVirAddr = (HI_U8 *)stVDECAllocBuf.pu8StartVirAddr;
            }
        }
    }
    ...
    if (HI_NULL == pstInst)
    {
        ...
        if (BUFMNG_ALLOC_INNER == pstConfig->enAllocType)
        {
            ...
            if (HI_TRUE == pstConfig->bTvp)
            {
                HI_DRV_VDEC_Release_TVP(&stVDECAllocBuf);
            }
            else
#endif
            {
                HI_DRV_VDEC_UnmapAndRelease(&stVDECAllocBuf);
            }
        }
        return HI_ERR_BM_NO_MEMORY;
    }
...
HI_S32 BUFMNG_SaveYuv(HI_S32 Handle, HI_DRV_VIDEO_FRAME_S *pstFrame, HI_UNF_VCODEC_TYPE_E enType)
{
    ...
    else
    {
        ...
        if (HI_SUCCESS != HI_DRV_VDEC_Map(&stFrmBuf))
        {
            BUFMNG_UNLOCK(stYuvSem);
            return -1;
        }
    ...
    if ((Y_Addr == 0) || (C_Addr == 0) || (DEF_HI_DRV_VPSS_LAST_ERROR_FLAG == pstVideoPriv->u32LastFlag))
    {
        len = -1;
        goto EXIT;
    }
    ...
    else
    {
        ...
        if (!bVcmpFlag)
        {
            if (HI_NULL == YUV_Array)
            {
                ...
                {
                    YUV_Array = HI_VMALLOC_BUFMNG(pstFrame->u32Width * pstFrame->u32Height * 3 / 2 * 2);
                }
                else
                {
            ...
            head_stride = (((((pstFrame->u32Width + 511) / 512) * 512) / 64) * 2);
            head_inf_size = ((pstFrame->u32Height + 63) / 64) * 32 * head_stride;
            YOffset_2b = (SlotWidth * SlotHeight) * 3 / 2 + head_inf_size * 3 / 2;
            ...
            for (i = 0; i < pstFrame->u32Height; i++)
            {
                for (j = 0; j < pstFrame->u32Width; j += 64)
                {
                    if (pstFrame->enBitWidth == HI_DRV_PIXEL_BITWIDTH_10BIT)
                    {
                        ...
                        src2b = Yaddress_2b + Stride_2b * (i / 32) + (dstIdx * 64 + (j / 64) * 64 * 32) / 4;
            ...
            if (pstFrame->enBitWidth == HI_DRV_PIXEL_BITWIDTH_10BIT)
            {
                len = VdecSaveYuvFile->f_op->write(VdecSaveYuvFile, YUV_Array, pstFrame->u32Width * pstFrame->u32Height * 2, &VdecSaveYuvFile->f_pos);
            
                if (len != pstFrame->u32Width * pstFrame->u32Height * 2)
                {
            ...
            for (i = 0; i < pstFrame->u32Height / 2; i++)
            {
                for (j = 0; j < pstFrame->u32Width; j += 64)
                {
                    if (pstFrame->enBitWidth == HI_DRV_PIXEL_BITWIDTH_10BIT)
                    {
                        ...
                        src2b = Caddress_2b + (Stride_2b / 2) * (i / 16) + (dstIdx * 64 + (j / 64) * 64 * 16) / 4;
                        memcpy(dst2b, src2b, 64 / 4);
            ...
            if (pstFrame->enBitWidth == HI_DRV_PIXEL_BITWIDTH_10BIT)
            {
                len = VdecSaveYuvFile->f_op->write(VdecSaveYuvFile, U_Array, pstFrame->u32Width * pstFrame->u32Height / 2, &VdecSaveYuvFile->f_pos);
                if (len != pstFrame->u32Width * pstFrame->u32Height / 2)
                {
                    HI_ERR_BUFMNG("%s write U failed.len : %d,W*H :%d\n", __func__, len, pstFrame->u32Width * pstFrame->u32Height / 2);
                    {
                        filp_close(VdecSaveYuvFile, HI_NULL);
                        HI_ERR_BUFMNG("Close yuv file of inst%d.\n", VdecYuvChanNum);
                        VdecSaveYuvFile = HI_NULL;
                ...
                len = VdecSaveYuvFile->f_op->write(VdecSaveYuvFile, V_Array, pstFrame->u32Width * pstFrame->u32Height / 2, &VdecSaveYuvFile->f_pos);
                if (len != pstFrame->u32Width * pstFrame->u32Height / 2)
                {
            ...
            HI_ERR_BUFMNG("Saving YUV(%dx%d), cmpFlag=%d, enBitWidth=%d...\n", pstFrame->u32Width, pstFrame->u32Height, bVcmpFlag, pstFrame->enBitWidth);
        }
        else
        {
            if (pstFrame->enBitWidth == HI_DRV_PIXEL_BITWIDTH_10BIT)
            ...
            *(HI_U32 *)(u8PicInfo + 24) = head_inf_size;
            ...
            WriteSize = 32;
            len = VdecSaveYuvFile->f_op->write(VdecSaveYuvFile, u8PicInfo, WriteSize, &VdecSaveYuvFile->f_pos);

            if (len != WriteSize)
            {
                HI_ERR_BUFMNG("%s write u8PicInfo failed.len : %d, %d\n", __func__, len, WriteSize);
                {
                    filp_close(VdecSaveYuvFile, HI_NULL);
                    HI_ERR_BUFMNG("Close yuv file of inst%d.\n", VdecYuvChanNum);
                    VdecSaveYuvFile = HI_NULL;
            ...
            len = VdecSaveYuvFile->f_op->write(VdecSaveYuvFile, Y_Addr, WriteSize, &VdecSaveYuvFile->f_pos);

            if (len != WriteSize)
            {
                HI_ERR_BUFMNG("%s write Ydata failed.len : %d, %d\n", __func__, len, WriteSize);
                {
                    filp_close(VdecSaveYuvFile, HI_NULL);
                    HI_ERR_BUFMNG("Close yuv file of inst%d.\n", VdecYuvChanNum);
                    VdecSaveYuvFile = HI_NULL;
            ...
            len = VdecSaveYuvFile->f_op->write(VdecSaveYuvFile, C_Addr, WriteSize, &VdecSaveYuvFile->f_pos);

            if (len != WriteSize)
            {
                HI_ERR_BUFMNG("%s write Cdata failed.len : %d, %d\n", __func__, len, WriteSize);
                {
                    filp_close(VdecSaveYuvFile, HI_NULL);
                    HI_ERR_BUFMNG("Close yuv file of inst%d.\n", VdecYuvChanNum);
                    VdecSaveYuvFile = HI_NULL;
            ...
            if (pstFrame->enBitWidth == HI_DRV_PIXEL_BITWIDTH_10BIT)
            {
                ...
                len = VdecSaveYuvFile->f_op->write(VdecSaveYuvFile, Yaddress_2b, WriteSize, &VdecSaveYuvFile->f_pos);
                ...
                len = VdecSaveYuvFile->f_op->write(VdecSaveYuvFile, Caddress_2b, WriteSize, &VdecSaveYuvFile->f_pos);
