...
#define VDEC_WaitEvent(WaitQueue, Condition, TimeOut)\
    do\
    {\
        wait_event_interruptible_timeout(WaitQueue, Condition, TimeOut);\
    }while(0)
...
static HI_S32 BUFMNG_VPSS_RelBuffer(HI_HANDLE hVpss, HI_HANDLE hPort, HI_DRV_VIDEO_FRAME_S* pstImage)
{
    ...
    HI_DRV_VIDEO_FRAME_S stPrivFrmInfo;
...
static HI_S32 VDEC_EventHandle(HI_S32 s32ChanID, HI_S32 s32EventType, HI_VOID* pArgs)
{
    ...
    switch (s32EventType)
    {
        ...
        case EVNT_NEW_IMAGE:
            /*move to here report the normal change*/
        {
            IMAGE* pstImage;
            pstImage = (IMAGE*)(pArgs); //@f00241306 64bit
            if ((pstImage->disp_height != pstChan->stLastFrm.stDispRect.s32Height) ||
                (pstImage->disp_width != pstChan->stLastFrm.stDispRect.s32Width))
            {
        ...
        case EVNT_ARRANGE_FRAME_BUF:
            ...
            case EVNT_ALLOC_FRAME_BUF_ONLY: //仅申请帧存
            VDEC_ChanControlStateSpinLockIRQ(&(pstChan->stAllocMemLock));
            ...
            VDEC_ChanControlStateSpinUnlockIRQ(&(pstChan->stAllocMemLock));
            break;
    case EVNT_RELSE_FRAME_BUF_ONLY: //仅释放帧存
        ...
        case EVNT_POST_MODULE_OCCUPY_FRAMES:
            ...
            if (((HI_U64*)pArgs)[1] != 0)
            {
                ...
                for (i = 0; i < specialFrmNum; i++, pSpecialFrmRec++)
                {
                    ...
                    if (HI_SUCCESS == VDEC_FindSpecialFrmToNormalIndex(pstChan, stSpecialFrmInfo.frmBufRec.u32StartPhyAddr, &index))
                    {
#ifdef HI_TEE_SUPPORT
                    ...
                    {
                        HI_ERR_VDEC("BUFMNG_VDEC_List_Add return failed! 0x%x\n", s32Ret);
                    }
                }
            }
            break;
        ...
        case EVNT_POST_MODULE_OCCUPY_ONE_FRAME:
            if (((HI_U64*)pArgs)[2] == 1)
            {
                ...
                pstChan->specialFramePhyAddr    = ((HI_U64*)pArgs)[0];
                pstChan->specialFrameVirAddr    = ((HI_U64*)pArgs)[1];
            }
            else
            {
                pstChan->specialFrameExist = HI_FALSE;
            }
            pstChan->bSpecialFrameCanRls = HI_FALSE;
...
/**************************************************************/
static HI_VOID VDEC_FastbackwardHandle(HI_HANDLE hVdec, HI_HANDLE hHandle, VDEC_VPSSCHANNEL_S* pstVpssChan)
{
    HI_S32 i;
    HI_S32 s32Ret;
...
static HI_S32 VDEC_VpssEventHandle(HI_HANDLE hVdec, HI_DRV_VPSS_EVENT_E enEventID, HI_VOID* pstArgs)
{
    ...
    switch (enEventID)
    {
        ...
        case VPSS_EVENT_REL_FRMBUFFER:
            if (HI_NULL != (pstFrame = HI_VMALLOC_BUFMNG(sizeof(*pstFrame))))
            {
                hPort = ((HI_DRV_VPSS_FRMBUF_S*)pstArgs)->hPort;
                hVpss = (HI_HANDLE)PORTHANDLE_TO_VPSSID(hPort);
        ...
        case  VPSS_EVENT_TASK_COMPLETE:
        {
            ...
            HI_DRV_VPSS_PORT_AVAILABLE_S* pstCanGetFrm = (HI_DRV_VPSS_PORT_AVAILABLE_S*)pstArgs;

            if (s_stVdecDrv.astChanEntity[hHandle].stVpssChan.s32Speed < 0)
            {
                VDEC_FastbackwardHandle(hVdec, hHandle, pstVpssChan);
            }
            for (i = 0; i < VDEC_MAX_PORT_NUM; i++)
            {
                if (   (HI_TRUE == pstVpssChan->stPort[i].bEnable)
...
static HI_S32 VDEC_SetAttr(VDEC_CHANNEL_S* pstChan)
{
    ...
    {
        stVdecChanCfg.s8BeHDRChan = 1;
        if (HI_VDEC_HDR_CHAN_TYPE_BL == pstChan->stVdecHdrAttr.enVdecHDRType)
        {
            stVdecChanCfg.dv_codec_capacity = DV_BL_DECODING_SUPPORT;
        }
        else if (HI_VDEC_HDR_CHAN_TYPE_EL == pstChan->stVdecHdrAttr.enVdecHDRType)
        {
            stVdecChanCfg.dv_codec_capacity = DV_EL_DECODING_SUPPORT;
        }
        else
        {
        ...
        {
            stVdecChanCfg.layer_imp = DV_DUAL_LAYER_IMP;
        }
        else
        {
            stVdecChanCfg.layer_imp = DV_MAX_LAYER_IMP;
        }

    }
    else
    {
        stVdecChanCfg.s8BeHDRChan = 0;
    }
#endif
    /* Set to VFMW */
...
static HI_S32 VDEC_Chan_RecvVpssFrmBuf(HI_HANDLE hVpss, HI_DRV_VIDEO_FRAME_PACKAGE_S* pstFrm, HI_U32 TimeOut)
{
    ...
    for (j = 0; j < VDEC_MAX_PORT_NUM; j++)
    {
               ...
               && (HI_DRV_VPSS_BUF_VPSS_ALLOC_MANAGE == pstVpssChan->stPort[j].bufferType) )
        {
            ...
            if (VDEC_PORT_TYPE_VIRTUAL == pstVpssChan->stPort[j].enPortType)
            {
                hVIRTUAL = pstVpssChan->stPort[j].hPort;
            }
        }
    }
...
static HI_VOID VDEC_GetHDR10_Info(IMAGE* pstImage,HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    if (1 == pstImage->hdr10_info.colour_description_present_flag)
    {
        if(BT2020 == pstImage->hdr10_info.colour_primaries &&
           SMPTE_ST_2084 == pstImage->hdr10_info.transfer_characteristics)/*==9:BT2020.==16:SMPTE ST 2084*/
        {
            pstFrame->unHDRInfo.stHDR10Info.bValid = HI_TRUE;
            pstFrame->enSrcFrameType = HI_DRV_VIDEO_FRAME_TYPE_HDR10;
        }
        else
        {
            pstFrame->unHDRInfo.stHDR10Info.bValid = HI_FALSE;
            pstFrame->enSrcFrameType = HI_DRV_VIDEO_FRAME_TYPE_SDR;
        }
        if(BT2020 == pstImage->hdr10_info.colour_primaries)
        {
            pstFrame->unHDRInfo.stHDR10Info.enColorSpace = HI_DRV_CS_BT2020_YUV_LIMITED;
            ((HI_DRV_VIDEO_PRIVATE_S*)(pstFrame->u32Priv))->eColorSpace  = HI_DRV_CS_BT2020_YUV_LIMITED;
        }
        else
        {
            pstFrame->unHDRInfo.stHDR10Info.enColorSpace = HI_DRV_CS_UNKNOWN;
            ((HI_DRV_VIDEO_PRIVATE_S*)(pstFrame->u32Priv))->eColorSpace = HI_DRV_CS_UNKNOWN;
        }
        if(SMPTE_ST_2084 == pstImage->hdr10_info.transfer_characteristics)
        {
            pstFrame->unHDRInfo.stHDR10Info.enGammaType = HI_DRV_GAMMA_TYPE_2084;
        }
        else
        {
            pstFrame->unHDRInfo.stHDR10Info.enGammaType = HI_DRV_GAMMA_TYPE_1886;
        }

        if(BT2020_NON_CONSTANT_LUMA_SYS == pstImage->hdr10_info.matrix_coeffs)
        {
            pstFrame->unHDRInfo.stHDR10Info.enLuminaceSys = HI_DRV_BT2020_NON_CONSTANCE_LUMINACE_SYS;
        }
        else if(BT2020_CONSTANT_LUMA_SYS == pstImage->hdr10_info.matrix_coeffs)
        {
            pstFrame->unHDRInfo.stHDR10Info.enLuminaceSys = HI_DRV_BT2020_CONSTANCE_LUMINACE_SYS;
        }
        else
        {
            pstFrame->unHDRInfo.stHDR10Info.enLuminaceSys = HI_DRV_BT2020_LUMINACE_SYS_BUTT;
        }
    }
    else
    {
    ...
    pstFrame->unHDRInfo.stHDR10Info.stMetadata.u16DispPrimariesX0 = pstImage->hdr10_info.stMasteringDisplayColourVolume.display_primaries_x[0];
    pstFrame->unHDRInfo.stHDR10Info.stMetadata.u16DispPrimariesX1 = pstImage->hdr10_info.stMasteringDisplayColourVolume.display_primaries_x[1];
    pstFrame->unHDRInfo.stHDR10Info.stMetadata.u16DispPrimariesX2 = pstImage->hdr10_info.stMasteringDisplayColourVolume.display_primaries_x[2];
    pstFrame->unHDRInfo.stHDR10Info.stMetadata.u16DispPrimariesY0 = pstImage->hdr10_info.stMasteringDisplayColourVolume.display_primaries_y[0];
    pstFrame->unHDRInfo.stHDR10Info.stMetadata.u16DispPrimariesY1 = pstImage->hdr10_info.stMasteringDisplayColourVolume.display_primaries_y[1];
    pstFrame->unHDRInfo.stHDR10Info.stMetadata.u16DispPrimariesY2 = pstImage->hdr10_info.stMasteringDisplayColourVolume.display_primaries_y[2];
    pstFrame->unHDRInfo.stHDR10Info.stMetadata.u16WhitePointX     = pstImage->hdr10_info.stMasteringDisplayColourVolume.white_point_x;
    pstFrame->unHDRInfo.stHDR10Info.stMetadata.u16WhitePointY     = pstImage->hdr10_info.stMasteringDisplayColourVolume.white_point_y;
    pstFrame->unHDRInfo.stHDR10Info.stMetadata.u32MaxDispMasteringLuminance = pstImage->hdr10_info.stMasteringDisplayColourVolume.max_display_mastering_luminance;
    pstFrame->unHDRInfo.stHDR10Info.stMetadata.u32MinDispMasteringLuminance = pstImage->hdr10_info.stMasteringDisplayColourVolume.min_display_mastering_luminance;
                               ...
                               IMAGE* pstImage, HI_DRV_VIDEO_FRAME_S* pstFrame)
{
   ...
   {
        VDEC_GetHDR10_Info(pstImage,pstFrame);
   }
...
HI_S32 VDEC_ResetChanFrmBuf(HI_S32 hHandle)
{
    ...
    for (i = 0; i < VDEC_MAX_BUFFER_RECORD; i++)
    {
        ...
        {
            pstChan->stFrameBufferRecord[i].enPmvBufferNodeStatus   = DRV_VDEC_BUFFER_STATUS_BUTT;//即使有特殊帧存，后级模块也不使用pmv内存，直接将pmv节点的状态reset
        }
        else
        {
...
#else
        if (HI_TRUE == pstChan->specialFrameExist && pstChan->stFrameBufferRecord[i].stFrameBuffer.u32StartPhyAddr == pstChan->specialFramePhyAddr)
        {
            pstChan->stFrameBufferRecord[i].enPmvBufferNodeStatus   = DRV_VDEC_BUFFER_STATUS_BUTT;//即使有特殊帧存，后级模块也不使用pmv内存，直接将pmv节点的状态reset
        }
        else
...
static HI_S32 VDEC_RlsFrm(VDEC_CHANNEL_S* pstChan, HI_DRV_VIDEO_FRAME_S* pstFrame)
{
...
#ifdef VFMW_VPSS_BYPASS_EN
#if 0
    if (0 != s_stVdecDrv.stRemainFrmList.s32Num)
    {
        /*this channel have special frame*/
        HI_INFO_VDEC("try to mark special frame can be release! Phyaddr = 0x%x\n", pstFrame->stBufAddr[0].u32PhyAddr_Y);
    ...
    if (HI_TRUE == pstChan->specialFrameExist)
    {
        if (stImage.luma_phy_addr == pstChan->specialFramePhyAddr)
        {
            pstChan->bSpecialFrameCanRls = HI_TRUE;
        }
    }
#endif
...
static HI_S32 VDEC_RecvFrm(HI_HANDLE hHandle, VDEC_CHANNEL_S* pstChan, HI_DRV_VIDEO_FRAME_S* pstFrame)
{
...
#if 0  /*move to vfmw report EVNT_NEW_IMAGE event*/
    ...
    if (stImage.is_SecureFrame != HI_TRUE)
    {
        for (u32Index = 0 ; u32Index < 4; u32Index++)
        {
            ...
            pstUsrData     = (VOID*)(HI_SIZE_T)(stImage.p_usrdat[u32Index]);
...
{
    HI_S32 s32Ret;
    VDEC_VIDEO_USERDATA_S stVdecUsrData;
...
static HI_S32 VDEC_IFrame_SetAttr(VDEC_CHANNEL_S* pstChan, HI_UNF_VCODEC_TYPE_E type)
{
    ...
    pstChan->stOption.s32ExtraFrameStoreNum = 0;
...
HI_S32 VDEC_Ioctl(struct inode* inode, struct file*  filp, unsigned int cmd, void* arg)
{
    ...
    switch (cmd)
    {
...
#ifdef HI_HDR_SUPPORT
        case UMAPC_VDEC_CHAN_SET_HDR_ATTR:
#endif
        ...
        case UMAPC_VDEC_CHAN_SETRESOLUTION:
            ...
            break;
#ifdef VFMW_VPSS_BYPASS_EN
        case UMAPC_VDEC_CHAN_GLOBALREL:
            if (HI_NULL == arg)
            {
                HI_ERR_VDEC("CMD %d Bad arg!\n", cmd);
                return HI_ERR_VDEC_INVALID_PARA;
            ...
            break;
        case UMAPC_VDEC_CHAN_GETVIDEOBYPASS:
            if (HI_NULL == arg)
            {
                HI_ERR_VDEC("CMD %d Bad arg!\n", cmd);
                return HI_ERR_VDEC_INVALID_PARA;
...
#endif
        default:
            HI_ERR_VDEC("CMD %d unsupport now!\n", cmd);
    ...
    switch (cmd)
    {
        ...
        case UMAPC_VDEC_CHAN_GETVIDEOBYPASS:
        {
            ...
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_VDEC("VDEC_Chan_SetResolution err!:%d\n", s32Ret);
            }
    ...
    for (i = 0; i < VDEC_MAX_BUFFER_RECORD; i++)
    {
...
#endif
        {
            ...
            if (pstChan->stVdecHdrAttr.enVdecHDRType != HI_VDEC_HDR_CHAN_TYPE_BUTT)
            {
                ...
                {
                    HI_DRV_VDEC_ReleaseMem(&pstChan->stFrameBufferRecord[i].stHdrBuffer, pstChan->bTvp, HI_TRUE);
                }
            }
        #endif
...
static HI_S32 VDEC_AllocMemForDecoder(HI_HANDLE hHandle)
{
    ...
    HI_U32        u32NeededPmvNum = 0;
    HI_U32        u32NeededHdrMetadataNum = 0;
    HI_BOOL       bAllocFrameBuffer = HI_FALSE;
    HI_BOOL       bAllocPmvBuffer = HI_FALSE;
    HI_BOOL       bAllocHdrMetadataBuffer = HI_FALSE;
    ...
    {
        u32NeededFrameNum = pstChan->stVFMWReportFrameInfo.u32ImageNum + pstChan->stOption.s32ExtraFrameStoreNum - pstChan->stVFMWConfiguredBufferNum.u32ImageNum;
    }
    else
    {
        ...
        {
            u32NeededFrameNum = pstChan->stVFMWReportFrameInfo.u32ImageNum - pstChan->stVFMWConfiguredBufferNum.u32ImageNum;
        }
        else
        {
            u32NeededFrameNum = 0;
    ...
    u32NeededPmvNum          = pstChan->stVFMWReportFrameInfo.u32PmvNum - pstChan->stVFMWConfiguredBufferNum.u32PmvNum;

#ifdef HI_HDR_SUPPORT
    u32NeededHdrMetadataNum = (pstChan->stVdecHdrAttr.enVdecHDRType != HI_VDEC_HDR_CHAN_TYPE_BUTT) ? u32NeededFrameNum : 0;
#else
    u32NeededHdrMetadataNum = 0;
#endif
    VDEC_ChanControlStateSpinUnlockIRQ(&(pstChan->stAllocMemLock));
    ...
    if ((1 == pstChan->stOption.u32DynamicFrameStoreAllocEn) && (u32NeededFrameNum > 0) && (pstChan->stVFMWReportFrameInfo.u32ImageNum > 0))
    {
        ...
        for (i = 0; i < u32NeededFrameNum; i++)
        {
            ...
            stMemBuffer.u32Size = pstChan->stVFMWReportFrameInfo.u32ImageSize;
            ...
            {
                //预分配分配不到的话直接从系统里面分配
                s32Ret = HI_DRV_VDEC_AllocMem("VFMW_FRM_BUF", NULL, stMemBuffer.u32Size, 0, &stMemBuffer, pstChan->bTvp, HI_FALSE);
                if (HI_SUCCESS != s32Ret)
                {
                    HI_WARN_VDEC("no memory !\n");
                    //状态为BUTT表示此节点记录的帧存信息无效
                    stFrameBuf.enBufNodeStatus = DRV_VDEC_BUFFER_STATUS_BUTT;
                    bAllocFrameBuffer = HI_FALSE;
                    continue; //防止Frame分配失败，继续分配metadata与pmv，而该内存又不会配置下去，导致metadata与pmv无法释放。
                }
                else
                {
                    //状态为SYSALLOC表示此节点记录的帧存信息有效，且内存来自动态分配
                    stFrameBuf.enBufNodeStatus = DRV_VDEC_BUFFER_STATUS_SYSALLOC;
                    bAllocFrameBuffer = HI_TRUE;
                }
            }
            else
            {
                //状态为PREALLOC表示节点记录的帧存信息有效，且内存来自预分配
                stFrameBuf.enBufNodeStatus = DRV_VDEC_BUFFER_STATUS_PREALLOC;
                bAllocFrameBuffer = HI_TRUE;
            }
            //记录帧存信息到局部变量stFrameBuf中，配置给vfmw
            stFrameBuf.u32StartPhyAddr = stMemBuffer.u32StartPhyAddr;
            stFrameBuf.u64StartVirAddr = (HI_U64)(HI_SIZE_T)stMemBuffer.pu8StartVirAddr;
            stFrameBuf.u32Size         = stMemBuffer.u32Size;

            //2:分配hdr metadata buffer
            if (u32NeededHdrMetadataNum != 0)
            {

                memset(&stMemBuffer, 0, sizeof(VDEC_BUFFER_S));
                stMemBuffer.u32Size = pstChan->stVFMWReportFrameInfo.u32HdrMetadataSize;
                //hdr metadata信息直接从系统里面动态分配，因为vo要用此片buffer映射用户态虚拟地址，如果传递的物理地址不是直接从系统里面分配的话，会映射失败
                s32Ret = HI_DRV_VDEC_AllocMem("VFMW_HDR_BUF", NULL, stMemBuffer.u32Size, 0, &stMemBuffer, pstChan->bTvp, HI_TRUE);
                if (HI_SUCCESS != s32Ret)
                {
                    HI_WARN_VDEC("no memory !\n");
                    stFrameBuf.enBufHdrMetadataNodeStatus = DRV_VDEC_BUFFER_STATUS_BUTT;
                    bAllocHdrMetadataBuffer = HI_FALSE;

                    if( stFrameBuf.enBufNodeStatus == DRV_VDEC_BUFFER_STATUS_SYSALLOC && bAllocFrameBuffer == HI_TRUE)
                    {
                        stMemBuffer.u32StartPhyAddr = stFrameBuf.u32StartPhyAddr;
                        stMemBuffer.pu8StartVirAddr = (HI_U8*)(HI_SIZE_T)stFrameBuf.u64StartVirAddr;
                        stMemBuffer.u32Size = stFrameBuf.u32Size;
                        HI_DRV_VDEC_ReleaseMem(&stMemBuffer, pstChan->bTvp, HI_FALSE);
                    }

                    continue;//防止metadata分配失败，继续分配pmv，而该内存又不会配置下去，导致frame与pmv无法释放。
                }
                else
                {

                    stFrameBuf.enBufHdrMetadataNodeStatus = DRV_VDEC_BUFFER_STATUS_SYSALLOC;
                    bAllocHdrMetadataBuffer = HI_TRUE;
                }
                //记录metadata信息到局部变量stFrameBuf中，配置给vfmw
                stFrameBuf.u32HdrMetadataStartPhyAddr = stMemBuffer.u32StartPhyAddr;
                stFrameBuf.u64HdrMetadataStartVirAddr = (HI_U64)(HI_SIZE_T)stMemBuffer.pu8StartVirAddr;
                stFrameBuf.u32HdrMetadataSize         = stMemBuffer.u32Size;
            ...
            if (u32NeededPmvNum != 0)
            {
                memset(&stMemBuffer, 0, sizeof(VDEC_BUFFER_S));
                stMemBuffer.u32Size = pstChan->stVFMWReportFrameInfo.u32PmvSize;
                //有预分配的话先从预分配里面拿帧存数据，没有的话采用动态分配
        #ifdef HI_TEE_SUPPORT
                if (HI_TRUE == pstChan->bTvp)
                {
                ...
                {
                    //预分配分配不到的话直接从系统里面分配
                    s32Ret = HI_DRV_VDEC_AllocMem("VFMW_PMV_BUF", NULL, stMemBuffer.u32Size, 0, &stMemBuffer, pstChan->bTvp, HI_FALSE);
                    if (HI_SUCCESS != s32Ret)
                    {
                        HI_WARN_VDEC("no memory !\n");
                        //无效节点
                        stFrameBuf.enBufPmvNodeStatus = DRV_VDEC_BUFFER_STATUS_BUTT;
                        bAllocPmvBuffer = HI_FALSE;
                        ...
                        if( stFrameBuf.enBufHdrMetadataNodeStatus == DRV_VDEC_BUFFER_STATUS_SYSALLOC && bAllocHdrMetadataBuffer == HI_TRUE)
                        {
                            stMemBuffer.u32StartPhyAddr = stFrameBuf.u32HdrMetadataStartPhyAddr;
                            stMemBuffer.pu8StartVirAddr = (HI_U8*)(HI_SIZE_T)stFrameBuf.u64HdrMetadataStartVirAddr;
                            stMemBuffer.u32Size = stFrameBuf.u32HdrMetadataSize;
                            HI_DRV_VDEC_ReleaseMem(&stMemBuffer, pstChan->bTvp, HI_TRUE);
                        }
                    }
                    else
                    {
                        //有效节点，内存来自系统动态分配
                        stFrameBuf.enBufPmvNodeStatus = DRV_VDEC_BUFFER_STATUS_SYSALLOC;
                        bAllocPmvBuffer = HI_TRUE;
                    }
                }
                else
                {
                    //有效节点，内存来自预分配
                    stFrameBuf.enBufPmvNodeStatus = DRV_VDEC_BUFFER_STATUS_PREALLOC;
                    bAllocPmvBuffer = HI_TRUE;
                }
                //记录pmv信息到局部变量stFrameBuf中，配置给vfmw
                stFrameBuf.u32PmvStartPhyAddr = stMemBuffer.u32StartPhyAddr;
                stFrameBuf.u64PmvStartVirAddr = (HI_U64)(HI_SIZE_T)stMemBuffer.pu8StartVirAddr;
                stFrameBuf.u32PmvSize         = stMemBuffer.u32Size;
            ...
            if ((HI_TRUE == bAllocFrameBuffer)
                && ((u32NeededHdrMetadataNum > 0 && HI_TRUE == bAllocHdrMetadataBuffer) || (u32NeededHdrMetadataNum == 0))
                && ((u32NeededPmvNum > 0 && HI_TRUE == bAllocPmvBuffer) || (u32NeededPmvNum == 0)))
            {
                s32Ret = VDEC_AddFrmBufRec(hHandle, &stFrameBuf, &u32Index);
                if (HI_SUCCESS != s32Ret)
                {
...
#ifdef VFMW_VPSS_BYPASS_EN   //specialFrameExist->specialFrameNum    bSpecialFrameCanRls
#if 0
                if (pstChan->specialFrameNum != 0)
                {
                    /*this channel have special frame,and try to release the special Frame which can be release.
                      be careful:do this function once,may be release more than one special frame.
                      change by l00228308
                     */
                    s32Ret = VDEC_TryToRlsSpecialFrame(pstChan);
                    if (HI_SUCCESS != s32Ret)
                    {
                        HI_INFO_VDEC("VDEC_TryToRlsSpecialFrame chan %d  err, return 0x%x\n", i, s32Ret);
                    }
                }
#endif
#else
                if ((HI_TRUE == pstChan->bSpecialFrameCanRls) && (HI_TRUE == pstChan->specialFrameExist))
                {
                    s32Ret = VDEC_RlsSpecialFrame(pstChan);
                    if (HI_SUCCESS != s32Ret)
                    {
                        HI_ERR_VDEC("VDEC_RlsSpecialFrame chan %d  err\n", i);
                    }
                }
#endif
...
HI_S32 VDEC_DRV_GlobalRelFrmInter(HI_U32 u32Index)
{
    ...
    if (HI_TRUE == pSpecialFrmRec->bIsSecFrm)
    {
        ...
        if (DRV_VDEC_BUFFER_STATUS_SYSALLOC == pSpecialFrmRec->enhdrNodeStatus)  /*动态申请*/
        {
            ...
            HI_DRV_VDEC_Release_TVP(&pSpecialFrmRec->hdrBufRec);
        }
    }
    else
#endif
...
HI_S32 HI_DRV_VDEC_GlobalRelFrm(HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    ...
    if (HI_TRUE == pSpecialFrmInfo->bIsSecFrm)
    {
        ...
        {
            HI_DRV_VDEC_Release_TVP(&pSpecialFrmInfo->hdrBufRec);
        }
    }
    else
#endif
