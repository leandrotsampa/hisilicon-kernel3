...
/5koJPsX6LzmZA6hNoH347lCOqFGFrF5gnYGSTVGPScBenwdeK7oIJHFH3PRsE+ibAd8yXfl
gKv4z2EKra0CKhZYXoOaSYV6Lf9GrHalmRbs/gIzPdiLxE8rjj5tsjtB9HyXCg==#*/
...
static HI_S32 Dsp_ResetBoot(HI_VOID)
{
...
#elif defined(CHIP_TYPE_hi3751v600) || defined(CHIP_TYPE_hi3751lv500) || defined(CHIP_TYPE_hi3751av500)
    //3. reset dsp
    U_DSP_CTRL  uTmpVal;
    uTmpVal.u32 = g_pstRegPeri->DSP_CTRL.u32;
    ...
    uTmpVal1.u32 = g_pstRegCrg->PERI_CRG52.u32;
#if !defined(CHIP_TYPE_hi3751av500)    //av500 have no clock select
...
#if 0  //defined (HI_SND_AOE_SWSIMULATE_SUPPORT)
...
HI_VOID ADSP_PrivDebug_Proc(HI_U32 u32DebugId, HI_U32 u32DebugVal)
{
#ifdef HI_SND_DSP_HW_SUPPORT
    if(DSP_PRIVDEBUG_REG_NUM <= u32DebugId)
    {
        return;
    }

    if(HI_NULL == g_pstRegMapAdspChan)
    {
        return;
    }

    g_pstRegMapAdspChan->ADSP_PRIVDEBUG[u32DebugId] = u32DebugVal;
            ...
            && pEneReg->ENGINE_CTRL.bits.cmd_done == 1)
        {
            ...
            for (j = 0; j < AOE_MAX_AIP_NUM; j++)
            {
                if ((pEneReg->ENGINE_MIX_SRC.bits.aip_fifo_ena) & (1L << j))
                {
                    ...
                    PROC_PRINT( p,
                                ...
                                "Aip(%d)      :Status(%s), Mute(%s), Vol(%ddB/%ddB), ChMode(%d), SpeedAdjCnt(%d)\n",
                                j,
            ...
            for (j = 0; j < AOE_MAX_AOP_NUM; j++)
            {
                if ((pEneReg->ENGINE_ROU_DST.bits.aop_buf_ena) & (1L << j))
                {
                    ...
                    {
                        PROC_PRINT( p,
                                    "Aop(%d,Cast) :Status(%s), AddMute(%s), AefBypass(%s), Vol(%ddB/%ddB), Mute(%s), Latency(%dms)\n",
                                    j,
                    ...
                    {
                        PROC_PRINT( p,
                                    "Aop(%d)      :Status(%s), AefBypass(%s), Vol(%s%d.%ddB/%s%d.%ddB), Balance(%d), mute(%s) , Latency(%dms), AD(%s)\n",
                                    j,
                                    ADSP_AOE_StatusName(pAopReg->AOP_CTRL.bits.cmd, pAopReg->AOP_CTRL.bits.cmd_done),
                                    (0 == pAopReg->AOP_CTRL.bits.ena_aef_bypass) ? "Off" : "On",
