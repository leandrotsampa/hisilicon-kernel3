...
static HI_VOID PQ_MNG_InitSoftWareAlgParm(HI_Mc_DeiParam_S* mParm, HI_DEI_Variable_S* deiVariable)
{
    ...
    mParm->FORCE_YUV444 = 0; //1;  // not used   //no register
    ...
    mParm->MODE_MEP1 = 0; //1; //fixed  //no register
    ...
    mParm->RGSIZEV = 8;   //16;  //fixed   //no register
    mParm->RGSTATH = 128;        //fixed   //no register
    mParm->RGSTATV = 16;  //32;  //fixed   //no register
...
HI_S32 PQ_MNG_DEI_ParaInit(HI_VOID)
{
    ...
    */
    ...
    stDeiPara->u32EdgeSmoothRatio = 128; //64;
    stDeiPara->bStinfoStop        = 0;
    stDeiPara->bDieRst            = 0;
    stDeiPara->u8DirMchC          = 0; //fmd相关
    stDeiPara->u8DirMchL          = 0; //fmd相关
    stDeiPara->bEdgeSmoothEn      = 0;
    stDeiPara->u8DeiLmmode        = 1;
    stDeiPara->u8DeiChmmode       = 1;
    stDeiPara->bDeiOutSelChm      = 0; //fmd相关
    stDeiPara->bDeiOutSelLum      = 0; //fmd相关
    ...
    stDeiPara->s32MinVerStrength = 320;//-320;

    stDeiPara->u32ScaleRange    = 8;//2;
    ...
    stDeiPara->u32BcGain      = 64;//32;
    stDeiPara->u32DirThd      = 0; //4;
    ...
    //for recursive mode
    stDeiPara->au8RecModeFldMotionStep[0] = 2; //1;
    stDeiPara->au8RecModeFldMotionStep[1] = 2;
    stDeiPara->au8RecModeFrmMotionStep[0] = 0;
    stDeiPara->au8RecModeFrmMotionStep[1] = 0; //1;
    ...
    stDeiPara->au16FrameMotionSmoothRatio[1] =  ALG_CLIP3(0, 127, stDeiPara->au16FrmFldBlendRatio[0] + //64;
    ...
    stDeiPara->mcdeibaseParams.stINTERPINFO.EDGE_NORM[0]  = /*65535/(stDeiPara->au8DirRatio[0]* 2 *3 *3)*/ 0;
...
#endif
    /*
        s32Ret = PQ_HAL_SetDeiRegist(u32HandleNo, stDeiPara);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_PQ("error in set dei register\n");
...
HI_S32 PQ_MNG_ALG_GetMcDeiParam(MOTION_INPUT_S* pstMotionInput)
{
    ...
    HI_S32 nAdjK_STAT = 0, nAdjK_STAT_P;
    HI_S32 nVmvSum    = 0;
    HI_S32 nHmvSum    = 0;
    ...
    nGlobalMotion = PQ_MNG_ALG_API_GMD_MA(au32MotionData);
...
#if 0
    PQ_MNG_GetFilmMode(&u32FilmMode);
#endif
...
HI_S32 PQ_MNG_InitDei(PQ_PARAM_S* pstPqParam, HI_BOOL bDefault)
{
    ...
    s32Ret = PQ_MNG_InitPhyList(u32HandleNo, HI_PQ_MODULE_DEI, SOURCE_MODE_NO, OUTPUT_MODE_NO);
    if (HI_SUCCESS != s32Ret)
    {
