...
static HI_S32 PQ_MNG_FindAcmGain(HI_VOID)
{
    ...
    PQ_MNG_FindBinPos(HI_PQ_MODULE_COLOR, &u32StartPos, &u32EndPos);
...
HI_S32 PQ_MNG_InitColor(PQ_PARAM_S* pstPqParam, HI_BOOL bDefault)
{
    ...
    s32Ret = PQ_MNG_InitPhyList(u32ID, HI_PQ_MODULE_COLOR, SOURCE_MODE_HD, OUTPUT_MODE_HD);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 PQ_MNG_SetACMGain(HI_VOID)
{
    ...
    {
        PQ_MNG_SetColorGainOffset(g_u32SourceMode);

        u32GainLuma = sg_u32ColorGain[g_u32SourceMode][sg_enFleshLevel][ACM_EH_GAIN_LUMA];
        u32GainHue  = sg_u32ColorGain[g_u32SourceMode][sg_enFleshLevel][ACM_EH_GAIN_HUE];
        u32GainSat  = sg_u32ColorGain[g_u32SourceMode][sg_enFleshLevel][ACM_EH_GAIN_SAT];
    }
    else
    {
        u32GainLuma = sg_u32ColorGain[g_u32SourceMode][COLOR_GAIN_MID][ACM_EH_GAIN_LUMA];
        u32GainHue  = sg_u32ColorGain[g_u32SourceMode][COLOR_GAIN_MID][ACM_EH_GAIN_HUE];
        u32GainSat  = sg_u32ColorGain[g_u32SourceMode][COLOR_GAIN_MID][ACM_EH_GAIN_SAT];
    }
...
HI_S32 PQ_MNG_SetFleshToneLevel(HI_PQ_FLESHTONE_E enGainLevel)
{
    ...
    if (g_u32SourceMode >= SOURCE_MODE_ALL)
    {
        HI_ERR_PQ("[%d]Source mode unknown!\n", g_u32SourceMode);
        g_u32SourceMode = SOURCE_MODE_NO;
    }
