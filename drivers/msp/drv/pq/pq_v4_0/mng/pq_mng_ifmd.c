...
static PQ_TUN_MODE_E  sg_enIfmdTunMode   = PQ_TUN_NORMAL;
...
{
    if ((bInit == HI_FALSE) && (PQ_TUN_NORMAL == sg_enIfmdTunMode))
    {
        return HI_FAILURE;
    }
...
HI_S32 PQ_MNG_InitIfmd(PQ_PARAM_S* pstPqParam, HI_BOOL bDefault)
{
    ...
    s32Ret = PQ_MNG_InitPhyList(u32HandleNo, HI_PQ_MODULE_FMD, SOURCE_MODE_NO, OUTPUT_MODE_NO);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 PQ_MNG_SetIfmdTunMode(PQ_TUN_MODE_E enTunMode)
{
    sg_enIfmdTunMode = enTunMode;

    return HI_SUCCESS;
}

HI_S32 PQ_MNG_GetIfmdTunMode(PQ_TUN_MODE_E* penTunMode)
{
    PQ_CHECK_NULL_PTR(penTunMode);
    *penTunMode = sg_enIfmdTunMode;
    return HI_SUCCESS;
}
...
static stPQAlgFuncs stFmdFuncs =
{
    ...
    .SetTunMode           = PQ_MNG_SetIfmdTunMode,
    .GetTunMode           = PQ_MNG_GetIfmdTunMode,
};
