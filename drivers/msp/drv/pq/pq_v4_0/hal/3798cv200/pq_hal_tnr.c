...
HI_S32 PQ_HAL_SetTNRCbCrLut(HI_U32 u32HandleNo, HI_U8 TLUT_CbCr[][32])
{
    ...
    if (REG_TYPE_VPSS == PQ_COMM_GetAlgTypeID(HI_PQ_MODULE_TNR))
    {
        ...
        for (i = 0; i < 16; i++)
        {
            for (j = 0; j < 32;)
            {
                ...
                memcpy((HI_U32*)(&(pstVpssReg->VPSS_TNR_CLUT20.u32) + u32TmpAddr), &VPSS_TNR_CLUT20, sizeof(HI_U32));
                u32TmpAddr += 1;
                j += 4;
            }
        }
    ...
    for (i = 0; i < 16; i++)
    {
        for (j = 0; j < 32;)
        {
            ...
            memcpy((HI_U32*)(&(g_pstIPSelVdpAlg->V0_TNR_CLUT20.u32) + u32TmpAddr), &V0_TNR_CLUT20, sizeof(HI_U32));
            u32TmpAddr += 1;
            j += 4;
        }
    }
