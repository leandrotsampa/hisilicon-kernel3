...
HI_U32 PQ_HAL_FUNC_SendCoef(PQ_COEF_SEND_CFG* stCfg)
{
    HI_U32 ii, kk, nn, mm;
...

    {
        ...
        if (nRet != 0)
        {
            HI_PQ_ERR("Get dm_cvm_buf failed\n");
            return HI_FAILURE;
        }
    }
    VDP_HDR_SetVCvmVcvmCoefAddr(stMBuf.u32StartPhyAddr);
    pqprint(PQ_PRN_CSC, "dm_cvm_coef phy addr stMBuf.u32StartPhyAddr is %08x \n", stMBuf.u32StartPhyAddr);
    pqprint(PQ_PRN_CSC, "dm_cvm_coef vir addr stMBuf.u32StartVirAddr is %08x \n", stMBuf.u32StartVirAddr);
    ...
    else
    {
        ...
        for (kk = 0; kk < total_burst_cycle_num; kk++)
        {
            ...
            for (nn = 0; nn < (cycle_num * stCfg->lut_num); nn++)
            {
                ...
                PQ_HAL_FUNC_Push128(&stData128, tmp_data, stCfg->coef_bit_length[lut_cnt]);
