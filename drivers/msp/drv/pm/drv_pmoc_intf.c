...
static HI_U32 g_u32IRPowerKeyLow = 0;
static HI_U32 g_u32IRPowerKeyHigh = 0;
...
static HI_S32  c51_loadCode(HI_VOID)
{
    ...
    g_pstRegSysCtrl->SC_GENm[12] = (enChipType | (enChipID << 16));
#else
    g_pstRegSysCtrl->SC_GEN12 = (enChipType | (enChipID << 16));
#endif
...
static HI_S32 c51_resume(PM_BASEDEV_S *pdev)
{
    ...
    if (HI_NULL == pWakeup)
    {
        HI_ERR_PM("ioremap_nocache c51 wake up mode  err! \n");
        return HI_FAILURE;
    }
    
    HI_REG_READ32(pWakeup + 2, ret);
    ...
    if (C51_PMOC_WAKEUP_IR == wakeupMode)
    {
        HI_REG_READ32(pWakeup, g_u32IRPowerKeyLow);
        HI_REG_READ32(pWakeup + 1, g_u32IRPowerKeyHigh);
    }
    else
    {
        g_u32IRPowerKeyLow = 0;
        g_u32IRPowerKeyHigh = 0;
    }

    HI_REG_READ32(pWakeup + 3, g_u32StandbyPeriod);
    iounmap(pWakeup);
...
#if defined (HI_TEMP_CTRL_CONFIG)
    /* In fast power up, only call resume function */
    c51_loadCode();
    HI_REG_WRITE32(pMcuStart, 0x1); //start MCU
#endif
...
static long c51_Ioctl(struct file *file, HI_U32 cmd, HI_SIZE_T arg)
{
    ...
    case HI_PMOC_GET_WAKEUPMODE_CMD:
    {
        ...
        sWakeUpMode.enWakeUpMode = wakeupMode;

        /* get power key */
        sWakeUpMode.u64IRPowerKey = g_u32IRPowerKeyHigh;
        sWakeUpMode.u64IRPowerKey = (g_u32IRPowerKeyLow | (sWakeUpMode.u64IRPowerKey << 32));
...
#if defined(CHIP_TYPE_hi3798cv200) || defined(CHIP_TYPE_hi3798hv100)
    u32RegVal = g_pstRegSysCtrl->SC_GENx[3] & 0x400000; //bit[22]
    if (u32RegVal == 0)
    {
        *ps16Temp += ((g_pstRegSysCtrl->SC_GENx[3] >> 18) & 0xf);//bit[21:18]
    }
    else
    {
        *ps16Temp -= ((g_pstRegSysCtrl->SC_GENx[3] >> 18) & 0xf);
    }
#else
...
static HI_S32 CPUProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    PROC_PRINT(p, "CPU: AVS = %s, hpm offset = %d hpm target = 0x%x \n",
               (u32CpuAVSStatus != 0xff) ? ((u32CpuAVSStatus == 1) ? "On" : "Off") : "None", u8CpuHpmOffset, cur_cpu_hpm);
#else
...
|| defined(CHIP_TYPE_hi3798hv100)
    ...
    /* get trim data */
#if defined(CHIP_TYPE_hi3798cv200) || defined(CHIP_TYPE_hi3798hv100)
    g_pstRegSysCtrl->SC_GENx[3] = HAL_OTP_V200_Read(0x258);
#else
...
#if defined(CHIP_TYPE_hi3798cv200) || defined(CHIP_TYPE_hi3798hv100)
    u32VoltMin = g_pstRegSysCtrl->SC_GENm[17] >> 16;
#else
    u32VoltMin = g_pstRegSysCtrl->SC_GEN17 >> 16;
#endif
    u32VoltMin = CORE_VMAX - ((u32VoltMin - 1) * PWM_STEP) / PWM_CLASS;
	   
#if defined(CHIP_TYPE_hi3798cv200) || defined(CHIP_TYPE_hi3798hv100)
    u32VoltMax = g_pstRegSysCtrl->SC_GENm[17] & 0xffff;
#else
    u32VoltMax = g_pstRegSysCtrl->SC_GEN17 & 0xffff;
#endif
    u32VoltMax = CORE_VMAX - ((u32VoltMax - 1) * PWM_STEP) / PWM_CLASS;

#if defined(CHIP_TYPE_hi3798cv200) || defined(CHIP_TYPE_hi3798hv100)
    u32VoltMid = g_pstRegSysCtrl->SC_GENm[18] & 0xffff;
#else
    u32VoltMid = g_pstRegSysCtrl->SC_GEN18 & 0xffff;
#endif
