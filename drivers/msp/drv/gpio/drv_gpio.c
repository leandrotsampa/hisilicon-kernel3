...
HI_S32 gpio_pm_suspend(PM_BASEDEV_S *pdev, pm_message_t state)
{
    HI_PRINT("GPIO suspend OK\n");
    return 0;
}

HI_S32 gpio_pm_resume(PM_BASEDEV_S *pdev)
{
...
static GPIO_EXT_FUNC_S g_stGpioExportFuncs =
{
    ...
    .pfnGpioSuspend              = gpio_pm_suspend,
    .pfnGpioResume               = gpio_pm_resume
};

static GPIO_GET_GPIONUM_S g_GpioNum;

static HI_U8 *g_GpioUsrAddr[HI_GPIO_MAX_GROUP_NUM];

#if  defined(CHIP_TYPE_hi3718mv100)

static HI_U32 g_GpioPhyAddr[HI_GPIO_MAX_GROUP_NUM]={HI_GPIO_0_ADDR,HI_GPIO_1_ADDR,HI_GPIO_2_ADDR,HI_GPIO_3_ADDR,HI_GPIO_4_ADDR,\
...
HI_S32 HI_DRV_GPIO_GetGpioNum(GPIO_GET_GPIONUM_S* GpioNum)
{
    ...
    GpioNum->u8GpioGrpNum = HI_GPIO_GROUP_NUM;
    GpioNum->u8GpioMaxNum = HI_GPIO_MAX_BIT_NUM;
...
HI_S32 HI_DRV_GPIO_Init(void)
{
    ...
    {
        g_GpioUsrAddr[i] = (HI_U8 *)ioremap_nocache(g_GpioPhyAddr[i], HI_GPIO_ADDR_SIZE);
        if (g_GpioUsrAddr[i] == HI_NULL)
        {
            HI_ERR_GPIO("ioremap_nocache err!\n");
            return HI_FAILURE;
        }
    }
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_GPIO("GPIO disable INT failed 0x%x.\n", ret);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_GPIO(" GPIO clear all INT failed 0x%x.\n", ret);
        return HI_FAILURE;
    }
    ...
    for (i = 0; i < g_GpioNum.u8GpioGrpNum; i++)
    {
        ...
        if (HI_SUCCESS != ret)
        {
            HI_ERR_GPIO("register GPIO INT failed 0x%x, irq:%d.\n", ret, HI_GPIO_IRQ_NO + i);
            return HI_FAILURE;
        }
		else
		{
			if(HI_SUCCESS != HI_DRV_SYS_SetIrqAffinity(HI_ID_GPIO, HI_GPIO_IRQ_NO + i))
			{
            	HI_ERR_GPIO("HI_DRV_SYS_SetIrqAffinity failed.\n");
        	}	
		}		
    }
