    ...
    else
    {
		...
		else if (u32DispId == 1)
		{
			...
			thd = ((pstCfg->stTiming.u32Vfb 
				    ...
				    + pstCfg->stTiming.u32Vact) * 50) / 100;
		}
		else
		{
			thd = 0;
...
HI_S32 PF_AddVDacToVenc(HI_DRV_DISPLAY_E enDisp,DISP_VENC_E eVenc, HI_U32 uVdac, HI_DRV_DISP_VDAC_SIGNAL_E signal)
{
    ...
    if (v0 < HI_DISP_VDAC_MAX_NUMBER)
    {
        SDATE_VDAC_SetLink(eVenc, v0, signal);
...
HI_S32 VDP_SetDACDET(DISP_VENC_E enDate,HI_DRV_DISP_FMT_E enFmt)
{
        ...
        )
    {
        ...
        SDATE_SetDACDET(enDate, u32DetLine, u32DetPixelSta);
    }
    else // VGA Timing
    {
    }
...
HI_VOID DISP_HAL_SetSyncInv(HI_DRV_DISPLAY_E enDisp,HI_DRV_DISP_INTF_ID_E eID,HI_DRV_DISP_FMT_E enFmt,HI_DRV_DISP_TIMING_S *pstCustomTimg)
{
    ...
    switch (eID)
    {
        ...
        case HI_DRV_DISP_INTF_HDMI1:
        case HI_DRV_DISP_INTF_HDMI2:
			/*98cv200  hdmi inv is need not  reversal*/
            enInv.u32HsInv= 0;
            enInv.u32VsInv = 0;
    ...
    */
    ...
    if ( pstIf->bLinkVenc)
    {
        ...
        else
        {
            ...
            SDATE_HDATE_ResetFmt(pstIf->eVencId, eFmt);
        }
#else
    ...
    {
        SDATE_HDATE_SetSignal(pstIf->stIf.eID,pstIf->eVencId,pstIntf->bDacSync);
    }
    switch (pstIf->stIf.eID)
    {
        case HI_DRV_DISP_INTF_YPBPR0:
...
HI_S32 PF_SetIntfEnable2(HI_DRV_DISPLAY_E enDisp, DISP_INTF_S *pstIt, HI_BOOL bEnable)
{
    ...
    */
    ...
    if (pstIt->bLinkVenc)
    {
		...
		/*closed DAC Det*/
        SDATE_SetDACDetEn(enVencId, 0);
#else
        if (g_stVDACAttr.bDACDetEn)
        {
            SDATE_SetDACDetEn(enVencId, (HI_U32)bEnable);
        }
        else
        {
            SDATE_SetDACDetEn(enVencId, 0);
						       ...
						       HI_RECT_S *out)
{
    ...
    if (in->s32Width == 3840 && in->s32Height == 2160)
    {
    ...
    if (in->s32Width == 3840 && in->s32Height == 2160)
    {
        stRect.u32DXL = (HI_U32)in->s32Width/2;
    }
    else
    {
...
#if !defined(DAC_TYPE_HISI_VOLTAGE)
	/*closed DAC Det*/
    SDATE_SetDACDetEn(DISP_VENC_HDATE0, 0);
    SDATE_SetDACDetEn(DISP_VENC_SDATE0, 0);
#else
    SDATE_SetDACDetEn(DISP_VENC_HDATE0, bDACDetEn);
    SDATE_SetDACDetEn(DISP_VENC_SDATE0, bDACDetEn);
#endif
...
HI_S32 PF_DATE_SetCoef(HI_DRV_DISPLAY_E enDisp, HI_S32 u32Index)
{
    ...
    SDATE_HDATE_SetSrcCoef(DISP_VENC_SDATE0, VDP_Get_SDateSrcCoef(u32Index));
...
/************HDR hal start******************/
HI_VOID TransferDrvReso2CbbReso(HI_RECT_S *pstDrvReso,VDP_RECT_S *pstCbbReso)
{
...
HI_VOID TransferDrvAddr2CbbAddr(DRV_DISP_HDR_FRAME_ADDR_S *pstDrvAddr,
...
HI_VOID TransferDrvPixelFmt2CbbPixelFmt(HI_DRV_PIX_FORMAT_E enInputPixelFmt,VDP_VID_IFMT_E  *peOutPixelFmt)
{
...
HI_VOID TransferHdrCfgPara2CBB(HI_DRV_DISP_DOLBY_HDR_CFG_S *pstHalPara,VDP_HDR_CFG_S *pstCbbHdrCfg)
{
    pstCbbHdrCfg->bGfxEn  = pstHalPara->bGfxEn;
    pstCbbHdrCfg->bMuteEn = pstHalPara->bMuteEn;
    pstCbbHdrCfg->bSmmuEn = pstHalPara->bSmmu;
    pstCbbHdrCfg->bPreMult = 0;
    
    switch (pstHalPara->enHdrMode)
    {
        case DRV_DOVI_BC_IN_SDR_OUT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_BC_IN_SDR_OUT;
            break;
        }
        case DRV_DOVI_BC_IN_SDR_OUT_CERT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_BC_IN_SDR_OUT_CERT;
            break;
        }
        case DRV_DOVI_BC_IN_HDR10_OUT:
        {
            /* !!!!NOTECE!!!HAL NEED TO ADD NEW BRANCH TO OUTPUT 444 */
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_BC_IN_HDR10_OUT;
            break;
        }
        case DRV_DOVI_BC_IN_HDR10_OUT_CERT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_BC_IN_HDR10_OUT_CERT;
            break;
        }
        case DRV_DOVI_BC_IN_DOVI_YUV_OUT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_BC_IN_DOVI_YUV_OUT;
            break;
        }
        case DRV_DOVI_BC_IN_DOVI_IPT_OUT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_BC_IN_DOVI_IPT_OUT;
            break;
        }
        case DRV_DOVI_NBC_IN_SDR_OUT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_NBC_IN_SDR_OUT;
            break;
        }
        case DRV_DOVI_NBC_IN_SDR_OUT_CERT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_NBC_IN_SDR_OUT_CERT;
            break;
        }
        case DRV_DOVI_NBC_IN_HDR10_OUT:
        {
            /* !!!!NOTECE!!!HAL NEED TO ADD NEW BRANCH TO OUTPUT 444 */
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_NBC_IN_HDR10_OUT;
            break;
        }
        case DRV_DOVI_NBC_IN_HDR10_OUT_CERT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_NBC_IN_HDR10_OUT_CERT;
            break;
        }
        case DRV_DOVI_NBC_IN_DOVI_YUV_OUT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_NBC_IN_DOVI_YUV_OUT;
            break;
        }
        case DRV_DOVI_NBC_IN_DOVI_IPT_OUT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_NBC_IN_DOVI_IPT_OUT;
            break;
        }
        case DRV_HDR10_IN_SDR_OUT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_HDR10_IN_SDR_OUT;
            break;
        }
        case DRV_HDR10_IN_SDR_OUT_CERT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HDR_HDR10_IN_SDR_OUT_CERT;
            break;
        }
        case DRV_HDR10_IN_HDR10_OUT:
        {
            /* OUTPUT 10BIT 444 */
            pstCbbHdrCfg->enHdrMode = VDP_HDR_HDR10_IN_HDR10_OUT ;
            break;
        }
        case DRV_HDR10_IN_HDR10_OUT_CERT:
        {
            /* OUTPUT 10BIT 422 */
            pstCbbHdrCfg->enHdrMode = VDP_HDR_HDR10_IN_HDR10_OUT_CERT;
            break;
        }
        default:
            pstCbbHdrCfg->enHdrMode = VDP_HDR_DOVI_NBC_IN_SDR_OUT;
    ...
    switch (pstHalPara->stBLFrame.enBitWidth)
    {
        case HI_DRV_PIXEL_BITWIDTH_8BIT :
        {
            pstCbbHdrCfg->u32DataWidth = VDP_DATA_WTH_8;
            break;
        }
        case HI_DRV_PIXEL_BITWIDTH_10BIT :
        {
            pstCbbHdrCfg->u32DataWidth = VDP_DATA_WTH_10;
            break;
        }
        default:
            pstCbbHdrCfg->u32DataWidth = VDP_DATA_WTH_10;
    ...
    TransferDrvPixelFmt2CbbPixelFmt(pstHalPara->stBLFrame.enPixFormat, 
                                    &pstCbbHdrCfg->enVdpBlVidIFmt);
    TransferDrvPixelFmt2CbbPixelFmt(pstHalPara->stELFrame.enPixFormat, 
                                    &pstCbbHdrCfg->enVdpElVidIFmt);

    TransferDrvReso2CbbReso(&pstHalPara->stBLResoIn,&pstCbbHdrCfg->stVidReso);
    TransferDrvReso2CbbReso(&pstHalPara->stELResoIn,&pstCbbHdrCfg->stElReso);
    TransferDrvReso2CbbReso(&pstHalPara->stBLResoOut,&pstCbbHdrCfg->stBlReso);
    
    TransferDrvAddr2CbbAddr(&pstHalPara->stBLFrame.stAddress[0],
                            &pstCbbHdrCfg->stBlAddr,
                            HI_FALSE,
                            pstHalPara->stBLFrame.bSecurity,
                            pstHalPara->bSmmu);
    TransferDrvAddr2CbbAddr(&pstHalPara->stBLFrame.stAddress_lb[0],
                            &pstCbbHdrCfg->stBlAddr_2LowBit,
                            HI_FALSE,
                            pstHalPara->stBLFrame.bSecurity,
                            pstHalPara->bSmmu);
    TransferDrvAddr2CbbAddr(&pstHalPara->stELFrame.stAddress[0],
                            &pstCbbHdrCfg->stElAddr,
                            HI_FALSE,
                            pstHalPara->stELFrame.bSecurity,
                            pstHalPara->bSmmu);   

    if(pstHalPara->stBLFrame.bDcmpEn)
    {
        TransferDrvAddr2CbbAddr(&pstHalPara->stBLFrame.stAddress[0],
                                &pstCbbHdrCfg->stHeadAddr,
                                HI_TRUE,
                                pstHalPara->stBLFrame.bSecurity,
                                pstHalPara->bSmmu);
    }
	pstCbbHdrCfg->bDcmpEn = pstHalPara->stBLFrame.bDcmpEn;
...
{
    HI_S32 s32Ret;
    VDP_HDR_CFG_S stVdpHdrCfg;
    
    memset(&stVdpHdrCfg,0,sizeof(VDP_HDR_CFG_S));
    
    TransferHdrCfgPara2CBB(pstHdrCfg,&stVdpHdrCfg);
    
    s32Ret = VDP_DRV_SetHdrMode(&stVdpHdrCfg,pstCompCfg,pstDmKs);
    if(HI_SUCCESS != s32Ret)
    {
        DISP_ERROR("VDP set HdrMode fail!\n");
        return s32Ret;
    }
    return HI_SUCCESS;
}
...
{
    HI_S32 s32Ret;
    s32Ret = VDP_DRV_SetHdrHdmiMetadata(pu8HeadData, u32MdLen, pu8MdBuf);
    return s32Ret;
}

HI_VOID PF_CleanHdr(HI_BOOL bclear)
{
    VDP_DRV_CleanHdr(bclear);
    return;
}

//===========================add Hisi-self solution=============================
HI_VOID TransferHisiHdrCfgPara2CBB(HI_DRV_DISP_HISI_HDR_CFG_S *pstHalPara,VDP_HISI_HDR_CFG_S *pstCbbHdrCfg)
{
    pstCbbHdrCfg->bGfxEn  = pstHalPara->bGfxEn;
    pstCbbHdrCfg->bMuteEn = pstHalPara->bMuteEn;
    pstCbbHdrCfg->bSmmuEn = pstHalPara->bSmmu;
    pstCbbHdrCfg->bPreMult = HI_FALSE;
    
    switch (pstHalPara->enHdrMode)
    {            
        case DRV_HISI_HDR10_IN_SDR_OUT:
        {
            pstCbbHdrCfg->enHdrMode = VDP_HIHDR_HDR10_IN_SDR_OUT;
            break;
        }
        case DRV_HISI_HDR10_IN_HDR10_OUT:
        {
            /* OUTPUT 10BIT 444 */
            pstCbbHdrCfg->enHdrMode = VDP_HIHDR_HDR10_IN_HDR10_OUT;
            break;
        }
        default:
            pstCbbHdrCfg->enHdrMode = VDP_HIHDR_HDR10_IN_SDR_OUT;
    ...
    switch (pstHalPara->stFrame.enBitWidth)
    {
        case HI_DRV_PIXEL_BITWIDTH_8BIT :
        {
            pstCbbHdrCfg->u32DataWidth = VDP_DATA_WTH_8;
    ...
    TransferDrvPixelFmt2CbbPixelFmt(pstHalPara->stFrame.enPixFormat, 
                                    &pstCbbHdrCfg->enVidIFmt);

    TransferDrvReso2CbbReso(&pstHalPara->stResoIn,&pstCbbHdrCfg->stVidInReso);
    TransferDrvReso2CbbReso(&pstHalPara->stResoOut,&pstCbbHdrCfg->stVidOutReso);
    
    TransferDrvAddr2CbbAddr(&pstHalPara->stFrame.stAddress[0],
                            &pstCbbHdrCfg->stVidAddr,
                            HI_FALSE,
                            pstHalPara->stFrame.bSecurity,
                            pstHalPara->bSmmu);
    TransferDrvAddr2CbbAddr(&pstHalPara->stFrame.stAddress_lb[0],
                            &pstCbbHdrCfg->stVidAddr_2LowBit,
                            HI_FALSE,
                            pstHalPara->stFrame.bSecurity,
                            pstHalPara->bSmmu);   

    if(pstHalPara->stFrame.bDcmpEn)
    {
        TransferDrvAddr2CbbAddr(&pstHalPara->stFrame.stAddress[0],
                                &pstCbbHdrCfg->stHeadAddr,
                                HI_TRUE,
	...
	pstCbbHdrCfg->bDcmpEn = pstHalPara->stFrame.bDcmpEn;

    return ;
}

/* add for Hisi-self Hdr path */
HI_S32 PF_SetHisiHdrMode(HI_DRV_DISP_HISI_HDR_CFG_S *pstHalHdrCfg,HiHDRDmTMCfg *pstPqHdrCfg)
{
    HI_S32 s32Ret;
    VDP_HISI_HDR_CFG_S stVdpHisiHdrCfg;
    
    memset(&stVdpHisiHdrCfg,0,sizeof(VDP_HISI_HDR_CFG_S));
    TransferHisiHdrCfgPara2CBB(pstHalHdrCfg,&stVdpHisiHdrCfg);

    s32Ret = VDP_DRV_SetHisiHdrMode(&stVdpHisiHdrCfg,pstPqHdrCfg);
    if(HI_SUCCESS != s32Ret)
    {
        DISP_ERROR("VDP set HdrMode fail!\n");
        return s32Ret;
...
HI_VOID PF_SetHisiHdrMute(HI_BOOL bEnble)
{
    //HI_ERR_DISP("---mute Hisi hdr---\n");
    VDP_DRV_HiSiHdrMute(bEnble);
    return ;
}

HI_VOID PF_CleanHisiHdr(HI_BOOL bclear)
{
    //HI_ERR_DISP("--clean Hisi hdr---\n");
    VDP_DRV_HiSiHdrClean(bclear);
    return ;
}
...
#ifndef __DISP_PLATFORM_BOOT__	
    s_stIntfOps.PF_SetHdrMode = PF_SetHdrMode;
    s_stIntfOps.PF_SetHdrHdmiMetadata = PF_SetHdrHdmiMetadata;
    s_stIntfOps.PF_CleanHdr   = PF_CleanHdr;
    /* add for Hisi-self Hdr */
    s_stIntfOps.PF_SetHisiHdrMode = PF_SetHisiHdrMode;
    s_stIntfOps.PF_SetHisiHdrMute = PF_SetHisiHdrMute;
    s_stIntfOps.PF_CleanHisiHdr   = PF_CleanHisiHdr;
#endif	
	s_DispIntfFlag++;
