...
HI_VOID VDP_RST_FILE(char * fpName)
{
	...
	strcpy(fpSumRst, FILE_PATH);
	strcat(fpSumRst, fpName);
	strcat(fpSumRst,".rst");
...
HI_VOID  VDP_DISP_ClearIntSta(HI_U32 u32intmask )
{
	...
	volatile U_VOCTRL  VOCTRL;
		...
		if((u32intmask & VDP_INTMSK_DHD0_VTTHD)  == VDP_INTMSK_DHD0_VTTHD)
		{
			...
			strcpy(cSrcFile , "[frame number :  ");		
			sprintf(cSrcTmp, "%x]",gu32FrmCnt+1);
			strcat(cSrcFile,cSrcTmp);
...
//volatile U_VOMSKINTSTA VOMSKINTSTA;
	...
	else
	{
			...
			strcpy(cSrcFile , "[reg frame ");		
			sprintf(cSrcTmp, "%d]",gu32FrmCnt+1);
			strcat(cSrcFile,cSrcTmp);
