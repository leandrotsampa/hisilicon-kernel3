...
HI_U8 *g_pDispHdrCfgModeString[DRV_DISP_HDR_CFG_MODE_BUTT] = {
    "DRV_DOVI_BC_IN_DOVI_YUV_OUT" , 
...
HI_U8 *g_pDispHisiHdrCfgModeString[DRV_HISI_HDR_CFG_MODE_BUTT] = {
    "DRV_HISI_HDR10_IN_HDR10_OUT", 
...
HI_U8 *g_pDispWorkModeString[HI_DRV_DISP_WORKING_MODE_BUTT] = {
    "SDR Path",
    "Dolby HDR Path",
    "LastGfxDolby HDR Path",
    "Dolby HDR10 Path",
    "HDR10 Path",
    "BBC HDR Path",
};

HI_U8 *g_pDispHdrPlayModeString[HI_DRV_HDR_PLAY_MODE_BUTT] = {
    "SDR Out",
    "SDR CERT Out",    
    "Dolby Out",
    "Last Dolby Out",
...
static HI_VOID DISP_DolbyHdrProcRead(struct seq_file *p,DISP_PROC_INFO_S *pstDispAttr)
{
    ...
    PROC_PRINT(p, "%-20s:%s\n", "WorkMode",
      g_pDispWorkModeString[pstDispAttr->stHdrProcInfo.enWorkMode]);

    PROC_PRINT(p, "%-20s:%s\n", "PlayMode",
      g_pDispHdrPlayModeString[pstDispAttr->stHdrProcInfo.enHdrPlayMode]);

    PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->Mode",
      g_pDispHdrCfgModeString[pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDolbyHdrCfg.enHdrMode]);

    PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->ELValid",
      g_pDispYNString[pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.bELVaild]);

    PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->BL bit width",
      g_pDispHdrCfgDataWidthString[pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDolbyHdrCfg.stBLFrame.enBitWidth]);

    PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->bGfx",
      g_pDispYNString[pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDolbyHdrCfg.bGfxEn]);

    PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->BL bDcmp",
      g_pDispYNString[pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDolbyHdrCfg.stBLFrame.bDcmpEn]);
    ...
    PROC_PRINT(p, "%-20s:%d/%d/%d/%d\n", "HdrCfg->EL ResoIn",
      ...
      pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDolbyHdrCfg.stELResoIn.s32Height);

    PROC_PRINT(p, "%-20s:%d\n", "CompCfg->disable EL",
      pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stCompCfg.disable_residual_flag);

    PROC_PRINT(p, "%-20s:%d\n", "CompCfg->VDR bit",
      pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stCompCfg.rpu_VDR_bit_depth);

    PROC_PRINT(p, "%-20s:%d\n", "CompCfg->BL_bit",
      pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stCompCfg.rpu_BL_bit_depth);

    PROC_PRINT(p, "%-20s:%d\n", "CompCfg->EL_bit",
      pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stCompCfg.rpu_EL_bit_depth);

    PROC_PRINT(p, "%-20s:%d\n", "DmCfg->ext blk num",
      pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDmCfg.base.num_ext_blocks);

    PROC_PRINT(p, "%-20s:%d\n", "DmCfg->bit",
      pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDmCfg.base.signal_bit_depth);

    PROC_PRINT(p, "%-20s:%s\n", "DmCfg->chrm",
      g_pDispHdrCfgChrmFormat[(HI_U32)pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDmCfg.base.signal_chroma_format]);

    PROC_PRINT(p, "%-20s:%s\n", "DmCfg->color space",
      g_pDispHdrCfgCsString[(HI_U32)pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDmCfg.base.signal_color_space]);

    PROC_PRINT(p, "%-20s:%s\n", "DmKs->IMap.clr",
      g_pDispHdrCfgClrString[(HI_U32)pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDmKs.ksIMap.clr]);

    PROC_PRINT(p, "%-20s:%s\n", "DmKs->OMap.clr",
      g_pDispHdrCfgClrString[(HI_U32)pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDmKs.ksOMap.clr]);

    PROC_PRINT(p, "%-20s:%s\n", "DmKs->Uds.chrmIn",
      g_pDispHdrCfgChrmFormat[(HI_U32)pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDmKs.ksUds.chrmIn]);

    PROC_PRINT(p, "%-20s:%s\n", "DmKs->Uds.chrmOut",
      g_pDispHdrCfgChrmFormat[(HI_U32)pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDmKs.ksUds.chrmOut]);

    PROC_PRINT(p, "%-20s:%s\n", "DmKs->Src.eotf",
      g_pDispHdrCfgEotfType[(HI_U32)pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDmKs.ksIMap.eotfParam.eotf]);

    PROC_PRINT(p, "%-20s:%s\n", "DmKs->Tag.eotf",
      g_pDispHdrCfgEotfType[(HI_U32)pstDispAttr->stHdrProcInfo.stDolbyHdrProcInfo.stDmKs.ksOMap.oetfParam.oetf]);
    return ;
}

/* Hisi-self Hdr solution proc debug info. */
static HI_VOID DISP_HisiHdrProcRead(struct seq_file *p,DISP_PROC_INFO_S *pstDispAttr)
{
    if((HI_NULL == p) || (HI_NULL == pstDispAttr))
    {
        return ;
    ...
    PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->Mode",
      g_pDispHisiHdrCfgModeString[pstDispAttr->stHdrProcInfo.stHisiHdrProcInfo.stHisiHdrCfg.enHdrMode]);

    PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->bit width",
      g_pDispHdrCfgDataWidthString[pstDispAttr->stHdrProcInfo.stHisiHdrProcInfo.stHisiHdrCfg.stFrame.enBitWidth]);

    PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->bGfx",
      g_pDispYNString[pstDispAttr->stHdrProcInfo.stHisiHdrProcInfo.stHisiHdrCfg.bGfxEn]);

    PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->bDcmp",
      g_pDispYNString[pstDispAttr->stHdrProcInfo.stHisiHdrProcInfo.stHisiHdrCfg.stFrame.bDcmpEn]);
    
    PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->bSmmu",
      g_pDispYNString[pstDispAttr->stHdrProcInfo.stHisiHdrProcInfo.stHisiHdrCfg.bSmmu]);

    PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->bMute",
      g_pDispYNString[pstDispAttr->stHdrProcInfo.stHisiHdrProcInfo.stHisiHdrCfg.bMuteEn]);
    ...
    PROC_PRINT(p, "%-20s:%d/%d/%d/%d\n", "HdrCfg->ResoIn",
      ...
      pstDispAttr->stHdrProcInfo.stHisiHdrProcInfo.stHisiHdrCfg.stResoIn.s32Height);
    return ;
}
...
static HI_S32 DISP_ProcRead(struct seq_file *p, HI_VOID *v)
{
	...
	PROC_PRINT(p,"---------CFG_VDP_MMU_SUPPORT---------\n");
	#endif
    ...
    {
        if ((HI_DRV_DISP_WORKING_MODE_SDR < s_DispAttr.stHdrProcInfo.enWorkMode)
            && (HI_DRV_DISP_WORKING_MODE_BUTT > s_DispAttr.stHdrProcInfo.enWorkMode))
        {
            PROC_PRINT(p, "-----------------------HDR Info-------------------------\n");

            PROC_PRINT(p, "%-20s:%s\n", "DispType",
                       g_pDispOutputTypeString[s_DispAttr.enDispOutput]);
            switch(s_DispAttr.stHdrProcInfo.enWorkMode)
            {
                case HI_DRV_DISP_WORKING_MODE_HISI_HDR10:
                {
                    DISP_HisiHdrProcRead(p,&s_DispAttr);
                    break;
                }
                case HI_DRV_DISP_WORKING_MODE_HISI_BBC:
                {
                    PROC_PRINT(p, "Hisi BBC HDR will support soon.\n");
                    break;
                }
        ...
        else if (HI_DRV_DISP_WORKING_MODE_SDR == s_DispAttr.stHdrProcInfo.enWorkMode)
        {            
            PROC_PRINT(p, "%-20s:%s\n", "DispType set",
                       g_pDispOutputTypeString[s_DispAttr.enDispOutput]);
            PROC_PRINT(p, "%-20s:%s\n", "WorkMode",
                       g_pDispWorkModeString[s_DispAttr.stHdrProcInfo.enWorkMode]);
        }
    }
...
#ifdef	HI_VDP_WATERMARK
extern HI_BOOL bEnableNexguard ;
extern HI_BOOL bEnableVerimatrix;

extern HI_U32 u32VerStartX;
extern HI_U32 u32VerStartY;
extern HI_U32 u32Verscale ;

extern HI_U32 u32SymbolSize ;
...
HI_S32 DispProcCmdProcee(HI_DRV_DISPLAY_E enDisp, HI_CHAR *pArg1,HI_CHAR *pArg2)
{
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "bottom", strlen("bottom")))
    {
	 ...
	 else if (0 == HI_OSAL_Strncmp(pArg1, "top", strlen("top")))
    {
        HI_DRV_DISP_OFFSET_S screenoffset;
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "output", strlen("output")))
    {
        if (0 == HI_OSAL_Strncmp(pArg2, "sdr", strlen("sdr")))
        {
            DISP_SetDispOutput(enDisp,HI_DRV_DISP_TYPE_NORMAL);
        }
        else if (0 == HI_OSAL_Strncmp(pArg2, "cert_sdr", strlen("cert_sdr")))
        {
            DISP_SetDispOutput(enDisp,HI_DRV_DISP_TYPE_SDR_CERT);
        }
        else if (0 == HI_OSAL_Strncmp(pArg2, "dolby", strlen("dolby")))
        {
            DISP_SetDispOutput(enDisp,HI_DRV_DISP_TYPE_DOLBY);
        }
        else if (0 == HI_OSAL_Strncmp(pArg2, "hdr10", strlen("hdr10")))
        {
            DISP_SetDispOutput(enDisp,HI_DRV_DISP_TYPE_HDR10);
        }
		else if (0 == HI_OSAL_Strncmp(pArg2, "cert_hdr10", strlen("cert_hdr10")))
        {
            DISP_SetDispOutput(enDisp,HI_DRV_DISP_TYPE_HDR10_CERT);
        }
    }
	 else if (0 == HI_OSAL_Strncmp(pArg1, "bt2020", strlen("bt2020")))
    {
        if (0 == HI_OSAL_Strncmp(pArg2, "on", strlen("on")))
        {
             DISP_SetDispBT2020Enable(enDisp,1);
			 printk("DISP_SetDispBT2020Enable =  on    \n");
        }
        else if (0 == HI_OSAL_Strncmp(pArg2, "off", strlen("off")))
        {
             DISP_SetDispBT2020Enable(enDisp,0);
			 printk("DISP_SetDispBT2020Enable =  off    \n");
        }
    }


#ifdef	HI_VDP_WATERMARK	
	else if (0 == HI_OSAL_Strncmp(pArg1, "vmx", strlen("vmx")))
    {
            bEnableVerimatrix= (HI_U32)simple_strtol(pArg2, NULL, 10);
			printk("bEnableNexguard =%d   \n",bEnableVerimatrix);
    }
	else if (0 == HI_OSAL_Strncmp(pArg1, "nxg", strlen("nxg")))
    {
            bEnableNexguard= (HI_U32)simple_strtol(pArg2, NULL, 10);
			printk("bEnableVerimatrix =%d   \n",bEnableNexguard);
    }
	else if (0 == HI_OSAL_Strncmp(pArg1, "startx", strlen("startx")))
    {
            u32VerStartX= (HI_U32)simple_strtol(pArg2, NULL, 10);
			printk("u32VerStartX =%d   \n",u32VerStartX);
    }
	else if (0 == HI_OSAL_Strncmp(pArg1, "starty", strlen("starty")))
    {
            u32VerStartY= (HI_U32)simple_strtol(pArg2, NULL, 10);
			printk("u32VerStartY =%d   \n",u32VerStartY);
    }
	else if (0 == HI_OSAL_Strncmp(pArg1, "scale", strlen("scale")))
    {
            u32Verscale= (HI_U32)simple_strtol(pArg2, NULL, 10);
			printk("u32Verscale =%d   \n",u32Verscale);
    }
	else if (0 == HI_OSAL_Strncmp(pArg1, "size", strlen("size")))
    {
            u32SymbolSize= (HI_U32)simple_strtol(pArg2, NULL, 10);
			printk("u32Verscale =%d   \n",u32SymbolSize);
    }
#endif	
    else
...
*/
    ...
    HI_DRV_PROC_EchoHelper("echo output sdr/cert_sdr/dolby/hdr10/cert_hdr10 > /proc/msp/disp1\n");
}
...
HI_S32 DRV_DISP_Compat_ProcessCmd(unsigned int cmd, HI_VOID *arg, DRV_DISP_STATE_S *pDispState, HI_BOOL bUser)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_DISP_COMPAT_SEND_VBI:
            {
                DISP_COMPAT_VBI_S  *pDispCptVbi;
                DISP_VBI_S    stDispVbi;
                pDispCptVbi = (DISP_COMPAT_VBI_S *)arg;
                
                stDispVbi.hVbi = pDispCptVbi->hVbi;
                stDispVbi.stVbiData.eType = pDispCptVbi->stVbiData.eType;
                stDispVbi.stVbiData.pu8DataAddr = (HI_U8*)(HI_SIZE_T)pDispCptVbi->stVbiData.u32DataAddr;
                stDispVbi.stVbiData.u32DataLen = pDispCptVbi->stVbiData.u32DataLen;
                
                Ret = DISP_SendVbiData(stDispVbi.hVbi, &(stDispVbi.stVbiData));
                break;
            }
        ...
        case CMD_DISP_GET_OUTPUT_TYPE:
		  ...
		  case CMD_DISP_SET_BT2020_ENABLE:
            {
                DISP_BT2020_ENABLE_S *pstDispBt2020 = (DISP_BT2020_ENABLE_S *)arg;
                Ret = DISP_SetDispBT2020Enable(pstDispBt2020->enDisp,pstDispBt2020->bBT2020Enable);
                break;
            }
		    case CMD_DISP_GET_BT2020_ENABLE:
            {
                DISP_BT2020_ENABLE_S *pstDispBt2020 = (DISP_BT2020_ENABLE_S *)arg;
                Ret = DISP_GetDispBT2020Enable(pstDispBt2020->enDisp,&pstDispBt2020->bBT2020Enable);
                break;
            }
        default:
            // ????
...
HI_S32 fpga_test_disp_Init(char *filenametmp)
{
 ...
 stIntf[0].eID = HI_DRV_DISP_INTF_HDMI0;
    ...
    Ret = DISP_OS_MMZ_AllocAndMap((const char *)"fpga_test_vdp", VDP_MEM_TYPE_MMZ, 0x2000000, VDP_MEM_ALIGN, &stMBuf);
    if (Ret)
    {
                          ...
                          HI_U32 u32Stride)
{
    ...
    HI_U32  u32Length = 0 , u32ReadLen = 0;
    HI_S32  s32Ret = HI_SUCCESS;
    HI_U32  i = 0;
    HI_DRV_VIDEO_PRIVATE_S *pstPrivInfo = HI_NULL;


    pfYUV = vdp_k_fopen("/tmp/stream/005_M720p50.yuv", O_RDWR, 0);
    if (!pfYUV)
    {
	...
	s32Ret = DISP_OS_MMZ_Map(&stMBuf);
	if (s32Ret)
	{
		HI_ERR_DISP("map mem failed!\n");
		return s32Ret;
	}
	
	framebuf  = (HI_U8 *)stMBuf.pu8StartVirAddr;

    u32ReadLen = vdp_k_fread(framebuf,u32Length * 5, pfYUV);
    if (u32ReadLen < u32Length)
    {
        HI_ERR_WIN("read file '%s' fail:%d, %x!\n", file_name, u32ReadLen, u32Length);
        return HI_FAILURE;
    }
    
	s32Ret = DISP_OS_MMZ_UnMap(&stMBuf);
	if (s32Ret)
	{
		HI_ERR_DISP("unmap mem failed!\n");
		return s32Ret;
	}

    for (i = 0; i < u32ReadLen/u32Length; i ++)
    {
