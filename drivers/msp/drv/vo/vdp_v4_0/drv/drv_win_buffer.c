...
HI_S32 WinBuf_RlsFrameWithHandle(WB_POOL_S * pstWinBP,HI_DRV_VIDEO_FRAME_S *pstFrm)
{
    ...
    if ((HI_DRV_VIDEO_FRAME_TYPE_SDR == pstFrm->enSrcFrameType) ||
        (HI_DRV_VIDEO_FRAME_TYPE_HDR10 == pstFrm->enSrcFrameType))
    {
    ...
    else if (HI_DRV_VIDEO_FRAME_TYPE_DOLBY_BL == pstFrm->enSrcFrameType) //Dolby Frame release branch.Note:here need NOT judge Dolby EL,because BL and EL are store in node as BL/EL in sequence.
    {
        Ret = WinBuf_RlsDolbyFrame(pstWinBP, pstFrm);//pstDispFrame indicated Dolby BL frame here.
    }
    else
    {
        HI_ERR_WIN("Cannot release frame.NOT support frame type.\n");
        Ret = HI_ERR_VO_FRAME_RELEASE_FAILED;
    }
