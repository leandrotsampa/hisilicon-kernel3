...
#endif
extern HI_VOID DISP_ISR_SourceProcess(HI_VOID* hHandle, const HI_DRV_DISP_CALLBACK_INFO_S* pstInfo);
...
HI_S32 DispProduceDisplayInfo(DISP_S* pstDisp, HI_DISP_DISPLAY_INFO_S* pstInfo)
{
    ...
    if (eFmt < HI_DRV_DISP_FMT_CUSTOM)
    {
		...
		if (HI_TRUE == pstInfo->bBT2020Enable)
		{

			if ((HI_DRV_CS_BT709_RGB_LIMITED == pstInfo->eColorSpace )
				||(HI_DRV_CS_BT601_RGB_LIMITED == pstInfo->eColorSpace )
				||(HI_DRV_CS_BT709_RGB_FULL == pstInfo->eColorSpace )
				||(HI_DRV_CS_BT601_RGB_FULL == pstInfo->eColorSpace )
				||(HI_DRV_CS_BT2020_RGB_FULL == pstInfo->eColorSpace )
				||(HI_DRV_CS_BT2020_RGB_LIMITED== pstInfo->eColorSpace )
				)
			{
				pstInfo->eColorSpace = HI_DRV_CS_BT2020_RGB_FULL;
			}
			else
			{
				pstInfo->eColorSpace = HI_DRV_CS_BT2020_YUV_LIMITED;
			}
		}
		else
		{
...
HI_VOID DispInitDisplay(HI_DRV_DISPLAY_E enDisp)
{
    ...
    {
        if (stDefSetting.stIntf[t].eID < HI_DRV_DISP_INTF_ID_MAX)
        {
            //todo
            DispAddIntf(pstDisp, &stDefSetting.stIntf[t]);
            if (( !pstIntfOpt) || (!pstIntfOpt->PF_GetChnEnable) )
            {
                DISP_ERROR(" %s has null ptr!\n", __FUNCTION__);
                return ;
            }
            pstIntfOpt->PF_InitDacDetect(&stDefSetting.stIntf[t]);
        }
    }
...
HI_S32 DISP_Init(HI_VOID)
{
   ...
   DISP_MEMSET(&s_stDisplayDevice, 0, sizeof(DISP_DEV_S));
	...
	if (HI_SUCCESS != HI_DRV_SYS_SetIrqAffinity(HI_ID_DISP, DISP_INT_NUMBER))
    {
...
HI_U32 Disp_GetFastbootupFlag(HI_VOID)
{
	...
	{
		return *g_pu32DispUboot2KernelFlag & 0xffff0000;
	}
	else
	{
...
HI_U32 Disp_SetFastbootupFlag(HI_U32 u32Value)
{
	if (g_pu32DispUboot2KernelFlag != 0x0)
	{
		/*only change high 16 bit.*/
		*g_pu32DispUboot2KernelFlag = (*g_pu32DispUboot2KernelFlag & 0xffff) | (u32Value & 0xffff0000);
	}
	else
	{
		DISP_ERROR("g_pu32DispUboot2KernelFlag is NULL\n");
...
HI_BOOL bEnableNexguard = 0;
HI_BOOL bEnableVerimatrix = 0;

HI_U32 u32VerStartX = 10;
HI_U32 u32VerStartY = 10;
HI_U32 u32Verscale = 10;
HI_U32 u32SymbolSize = 10;
...
HI_VOID Coef_load(char *cName, HI_U32 PhyAddr, HI_U32 VirAddr,HI_U32 filesize)
{
    ...
    HI_S32 result;
    HI_U32 coef_addr,coef_size;

    coef_addr = PhyAddr;
    coef_size	= filesize;

    pOsdVirAddr = (HI_U8*) VirAddr;

    memset(pOsdVirAddr, 0x0, coef_size);

    if ( NULL == (fpOSD = SV_klib_fopen(cName, O_RDONLY, 0)))
    {
        printk("ERROR: Open File, LINE:%d\n", __LINE__);
        return ;
    }

    result = kfile_read(pOsdVirAddr, filesize, fpOSD);
    if (result < (filesize))
    {
        printk("read error,result:%d, line:%d\n", result, __LINE__);
    }

    for (i = 0; i < 32; i++)
    {
        printk("value:%x ", pOsdVirAddr[i]);
    }
...
HI_U32 u32SymbolAddr;//symbol¡§|¡§o??|¨¬?|¨¬???¡è

HI_S32 FREQUENCY_DISTANCE[3][3] = {{1, 2, 8}, {1, 2, 8}, {1, 2, 8}};
HI_S32 EMBEDDING_STRENGTH_THRESHOLD_10   [12] = {16, 22, 26, 32,  72, 138, 192,  248, 3072, 3072, 3072, 3072};
HI_S32 EMBEDDING_STRENGTH_THRESHOLD_BG_10[12] = {80, 86, 90, 96, 122, 556, 744, 3072, 3072, 3072, 3072, 3072};
HI_S32 EMBEDDING_STRENGTH_THRESHOLD_12   [48] = { 48,  60,  68,  80,  88,  96, 100, 108, 112, 116, 120, 124, 128, 160, 192, 224, 256, 320,  384,  448,  512,  576,  640,  704,  768,  824,  880,  936,  992,  992,  992,  992, 16383, 16383, 16383, 16383, 16383, 16383, 16383, 16383, 16383, 16383, 16383, 16383, 16383, 16383, 16383, 16383};
HI_S32 EMBEDDING_STRENGTH_THRESHOLD_BG_12[48] = {320, 328, 332, 340, 344, 348, 352, 356, 360, 368, 372, 380, 384, 412, 436, 464, 488, 924, 1356, 1792, 2224, 2412, 2600, 2788, 2976, 3072, 3072, 3072, 3072, 3072, 3072, 3072,  3072,  3072,  3072,  3072,  3072,  3072,  3072,  3072,  3072,  3072,  3072,  3072,  3072,  3072,  3072,  3072};
HI_S32 DIRECTION_MAX_DEFAULT = 396;

HI_VOID DISP_Set_Verimatrix( HI_BOOL bEnable)
{
	vmx_hw_soc_rend_t hwParameters = {0};

    HI_S32 i,j;

	HI_S32 s32Ret;

        hwParameters.symbol_phy_addr = u32SymbolAddr;

        hwParameters.watermark_on           = 1;
        hwParameters.symbols_rows           = 4;
        hwParameters.symbols_cols           = 4;
        hwParameters.symbols_xpos           = u32VerStartX;
        hwParameters.symbols_ypos           = u32VerStartY;
        hwParameters.symbol_size            = u32SymbolSize;
        hwParameters.spacing_vert           = 12;
        hwParameters.spacing_horz           = 10;
        hwParameters.symbol_scale_control   = u32Verscale;
        hwParameters.background_embedding_on = 1;
        hwParameters.direction_max          = DIRECTION_MAX_DEFAULT;
        hwParameters.strength_multiply = 100;

        // TODO: TA¡ä?8bit¦Ì?embedding_strength_threshold ¨°a¡Á¡é¨°a!!!
        // Parameters for symbol embedding of rendered symbols
        for (i = 0; i < 3; i++)
        {
            for (j = 0; j < 3; j++)
            {
                hwParameters.frequency_distance[i][j] = FREQUENCY_DISTANCE[i][j];
            }
        }

        for (i = 0; i < 12; i++)
        {
            hwParameters.embedding_strength_threshold_10[i]    = EMBEDDING_STRENGTH_THRESHOLD_10[i];
            hwParameters.embedding_strength_threshold_bg_10[i] = EMBEDDING_STRENGTH_THRESHOLD_BG_10[i];
        }

        for (i = 0; i < 48; i++)
        {
            hwParameters.embedding_strength_threshold_12[i]    = EMBEDDING_STRENGTH_THRESHOLD_12[i];
            hwParameters.embedding_strength_threshold_bg_12[i] = EMBEDDING_STRENGTH_THRESHOLD_BG_12[i];
        }
	...
	{
        hwParameters.watermark_on           = 0;

	}
	VM_HW_SetParameters_Rend(1, &hwParameters);
...
{
    NXG_HW_SetParameters(bEnable);
}


HI_VOID DISP_CB_WaterMarkProcess(HI_VOID* hHandle, const HI_DRV_DISP_CALLBACK_INFO_S* pstInfo)
{
		DISP_Set_Nexguard(bEnableNexguard);
	DISP_Set_Verimatrix(bEnableVerimatrix);
}
...
HI_S32 DISP_Open(HI_DRV_DISPLAY_E enDisp)
{
    ...
    HI_S32 s32Ret;
    MMZ_BUFFER_S stMBuf;
    char *dci_filename = "/tmp/vdp_vmx_coef_test.b";
    HI_DRV_DISP_CALLBACK_S  stWaterMarkCB;
#endif
	...
	if (HI_DRV_DISPLAY_1 == enDisp)
    {
        ...
        stHdrCB.pfDISP_Callback = DISP_ISR_SourceProcess;
    ...
    if (enDisp ==  HI_DRV_DISPLAY_1)
    {
        s32Ret = HI_DRV_MMZ_AllocAndMap("verimatrix_symbol", HI_NULL, 1920 , 0, &stMBuf);
        if (s32Ret != HI_SUCCESS)
        {
            DISP_ERROR("HI_DRV_MMZ_AllocAndMap  verimatrix_symbol faild !! %d\n", enDisp);
            return s32Ret;
        }

    	Coef_load(dci_filename, stMBuf.u32StartPhyAddr, stMBuf.pu8StartVirAddr,1920);

    	u32SymbolAddr = stMBuf.u32StartPhyAddr;

    	stWaterMarkCB.hDst            = (HI_VOID*)pstDisp;
        stWaterMarkCB.pfDISP_Callback = DISP_CB_WaterMarkProcess;
        stWaterMarkCB.eCallBackPrior  = HI_DRV_DISP_CALLBACK_PRIORTY_LOW;
        nRet = DISP_ISR_RegCallback(enDisp, HI_DRV_DISP_C_INTPOS_0_PERCENT, &stWaterMarkCB);
        DISP_ASSERT(HI_SUCCESS == nRet);
...
HI_S32 DISP_Close(HI_DRV_DISPLAY_E enDisp)
{
	...
	if (HI_DRV_DISPLAY_1 == enDisp)
	{
        ...
        stWaterMarkCB.pfDISP_Callback = DISP_CB_WaterMarkProcess;
        stWaterMarkCB.eCallBackPrior  = HI_DRV_DISP_CALLBACK_PRIORTY_LOW;
        nRet = DISP_ISR_UnRegCallback(enDisp, HI_DRV_DISP_C_INTPOS_0_PERCENT, &stWaterMarkCB);
		stMBuf.u32StartPhyAddr = u32SymbolAddr;
...
HI_S32 DISP_GetProcInto(HI_DRV_DISPLAY_E enDisp, DISP_PROC_INFO_S* pstInfo)
{
    ...
    pstInfo->stHdrProcInfo = pstDisp->stHdrProcInfo;
...
HI_S32 DISP_SetDispOutput( HI_DRV_DISPLAY_E enDisp, const HI_DRV_DISP_OUT_TYPE_E enDispType)
{
...
#if 0
    HI_DRV_HDMI_VIDEO_CAPABILITY_S stSinkDispCap;
#endif
...
#if 0
    memset(&stSinkDispCap, 0x0, sizeof(HI_DRV_HDMI_VIDEO_CAPABILITY_S));

    if ((pstDisp->stSetting.stIntf[HI_DRV_DISP_INTF_HDMI0].bOpen) && (!DispGetHdmiFunction()))
    {
        if (s_pstHDMIFunc->pfnHdmiGetVideoCapability)
        {
            if (s_pstHDMIFunc->pfnHdmiGetVideoCapability(HI_UNF_HDMI_ID_0, &stSinkDispCap))
            {
                DISP_ERROR("pfnHdmiGetVideoCapability fail!\n");
                return HI_ERR_DISP_INVALID_PARA;
            }
        }
        else
        {
            DISP_ERROR("pfnHdmiGetVideoCapability is NULL!\n");
            return HI_ERR_DISP_NULL_PTR;
        }
    }
#endif
    ...
    switch (enDispType)
    {
        ...
        {
#if 0
            if (HI_FALSE == stSinkDispCap.bDolbySupport)
            {
                DISP_ERROR("Sink CAN NOT support DolbyVision HDR.\n");
                return HI_ERR_DISP_NOT_SUPPORT;
            }
            else
#endif
            {
                if (DISP_OutputTypeAndFormatMatch(enDispType, pstDisp->stSetting.enFormat))
                {
                    ...
                    enHdmiUserMode = HI_DRV_HDMI_HDR_USERMODE_DOLBY;
                    //DISP_TransferDoviUnfCap2DoviDrvAttr(&stSinkCapability.stDolby,
                    //                                    &stDispHdrAttr.unSinkAttr.stDolbySinkAttr);
                }
                else
                {
                    DISP_ERROR("Disp format cannot be interlace while output is set Dolby!\n");
        ...
        {
#if 0
            if (HI_FALSE == stSinkDispCap.bHdrSupport)
            {
                DISP_ERROR("Sink CAN NOT support HDR10.\n");
                return HI_ERR_DISP_NOT_SUPPORT;
            }
            else
#endif
            {
                ...
                //DISP_TransferHDR10UnfCap2HDR10DrvAttr(&stSinkCapability.stHdr,
                //                                      &stDispHdrAttr.unSinkAttr.stHDR10SinkAttr);
            }
            break;
        }
...
HI_S32 DISP_SetDispBT2020Enable(HI_DRV_DISPLAY_E enDisp, HI_BOOL bEnable)
{
    DISP_S* pstDisp;

	if (HI_DRV_DISPLAY_1 != enDisp)
	{
		DISP_ERROR("the chanel not support Bt2020!\n");
		return HI_FAILURE;
	}
    DispCheckDeviceState();
    DispCheckID(enDisp);
    DispGetPointerByID(enDisp, pstDisp);
	  ...
	  pstDisp->bDispSettingChange = HI_FALSE;
	  pstDisp->stSetting.bBT2020Support = bEnable;
	  pstDisp->bDispSettingChange = HI_TRUE;
    return HI_SUCCESS;
...
HI_S32 DISP_GetDispBT2020Enable(HI_DRV_DISPLAY_E enDisp,  HI_BOOL *pbEnable)
{
    DISP_S* pstDisp;
    ...
    *pbEnable = pstDisp->stSetting.bBT2020Support;
