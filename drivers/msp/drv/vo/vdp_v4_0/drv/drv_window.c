...
{	
	...
	else
	{
		if ((HI_NULL != pstSlaveWin) && (HI_FALSE == pstSlaveWin->bEnable))
		{
		   //WIN_ERROR("u32ChnHd_time:%d, u32Chnsd_time:%d!\n", u32ChnHd_time, u32Chnsd_time);
		}
        ...
        for (s32Index = 0; s32Index < WINDOW_MAX_NUMBER; s32Index++)
        {
            ...
            if ((HI_NULL != pstWin) 
                && ((HI_DRV_WINDOW_MODE_DOLBY == pstWin->enWinMode)
                    ||(HI_DRV_WINDOW_MODE_HDR10 == pstWin->enWinMode)))
            {
...
HI_S32 WinRegCallback(WINDOW_S *pstWin)
{
    ...
    else
    {
        if (WinCheckWhetherWbcIsogenyMode())
        {
            ...
            stCB.hDst  = (HI_VOID*)pstWin;
            stCB.pfDISP_Callback = ISR_CallbackForWinSlaveOutput;
            stCB.eCallBackPrior  =  HI_DRV_DISP_CALLBACK_PRIORTY_MIDDLE;
                                          ...
                                          WINDOW_S **pWindowReturn)
{
	...
	{
	    nRet = DISP_OS_MMZ_AllocAndMap((const char *)"PQ_DBM_MEM", 
										VDP_MEM_TYPE_PHY, 
										WIN_ALG_MMZ_SIZE, 
										VDP_MEM_ALIGN,
...
{    
	...
	for (i = 0; i < WIN_ALG_MMZ_NUM; i++)
    {
		...
		pstWin->stWinAlgMMZ[i].bReady = HI_TRUE;		
    }
    ...
    /*the wbc's resouces is attached to slave window, since the mems
        *  are share bettween slave window and Wbc.
        */
    if (pstWin->enType == HI_DRV_WIN_ACTIVE_SLAVE)
	{   
		/*if wbc-isogeny mode, should stop wbc.*/
	    WinWbc_Deinitialize();
	}
...
HI_S32 WIN_SetAttr(HI_HANDLE hWin, HI_DRV_WIN_ATTR_S *pWinAttr)
{
    ...
    else
    {
		...
		WIN_FATAL("Vir window do not support set width or height!\n");
...
HI_S32 WIN_SetEnable_Seperate(WINDOW_S *pstWin, HI_BOOL bEnable)
{
    ...
    *pstWinTmp = *pstWin;
	...
	/*if slave window closed, stop wbc at the same time.*/
	if ((HI_FALSE == bEnable) 
		 && (HI_DRV_WIN_ACTIVE_SLAVE == pstWin->enType)
		 && (HI_TRUE == WinCheckWhetherWbcIsogenyMode()))
	{
		WIN_Stop_Wbc();
	}
	else if ((HI_TRUE == bEnable) 
			 && (HI_DRV_WIN_ACTIVE_SLAVE == pstWin->enType)
			 && (HI_TRUE == WinCheckWhetherWbcIsogenyMode()))
	{		
        WIN_Start_Wbc();
...
#define WIN_DELAY_TIME_MAX_CIRCLE 64
...
HI_S32 WinQueueFrame(HI_HANDLE hWin, HI_DRV_VIDEO_FRAME_S *pFrameInfo)
{
    ...
    if ((HI_DRV_VIDEO_FRAME_TYPE_SDR != pFrameInfo->enSrcFrameType) &&
        (HI_DRV_VIDEO_FRAME_TYPE_HDR10 != pFrameInfo->enSrcFrameType))
    {
        WIN_ERROR("Frame is NOT SDR or HDR10 frame!\n");
    ...
    /*set win mode to hdr10 mode when receive hdr10 frame. */
    if(HI_DRV_VIDEO_FRAME_TYPE_HDR10 == pFrameInfo->enSrcFrameType)
    {
        nRet = WIN_SetMode(hWin,HI_DRV_WINDOW_MODE_HDR10);
        if(HI_SUCCESS != nRet)
        {
			WIN_ERROR("Set HDR10 WinMode Fail!\n");
            return nRet;
        }
    }
    else
    {
        nRet = WIN_SetMode(hWin,HI_DRV_WINDOW_MODE_NORMAL);
        if(HI_SUCCESS != nRet)
        {
		    WIN_ERROR("Set NORMAL WinMode Fail!\n");
            return nRet;
        }
    }
    ...
    if (!bVirtual)
    {
        ...
        {  
            ...
            {                              
                //HI_BOOL bDolby = (HI_TRUE == WIN_GetDolbyLibStatus()) && (HI_FALSE == pstWin->stUsingAttr.bHisiHdr);
                /* always go through Hisi-self hdr. */
                nRet = WinBuf_PutNewHdr10Frame(&pstWin->stBuffer.stWinBP, pFrameInfo, HI_FALSE);
                if (HI_SUCCESS != nRet)
                {
                    return HI_ERR_VO_BUFQUE_FULL;
                }
    ...
    while(!nRet)
    {
        ...
        else if (pstSource->pfRlsFrame)
        {
                ...
                (HI_DRV_VIDEO_FRAME_TYPE_DOLBY_EL == stRlsFrm.enSrcFrameType))
            {
                ...
                ((HI_DRV_VIDEO_FRAME_TYPE_SDR      == stRlsFrm.enSrcFrameType) ||
                 (HI_DRV_VIDEO_FRAME_TYPE_HDR10    == stRlsFrm.enSrcFrameType) ||
                 (HI_DRV_VIDEO_FRAME_TYPE_DOLBY_BL == stRlsFrm.enSrcFrameType)))
            {
...
HI_VOID ISR_WinConfigFrameMute(WINDOW_S *pstWin, HI_DRV_VIDEO_FRAME_S *pstFrame, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    if ((pstWin->u32VideoLayer == VDP_LAYER_VID0)
        && WinCheckWhetherWbcIsogenyMode()
        && (pstWin->enWinMode == HI_DRV_WINDOW_MODE_NORMAL))
    {
	                         ...
	                         const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
	...
	stTimingInfo.stChannelTiming[(HI_PQ_DISPLAY_E)pstWin->enDisp].stFmtRect = pstInfo->stDispInfo.stFmtResolution;
	stTimingInfo.stChannelTiming[(HI_PQ_DISPLAY_E)pstWin->enDisp].u32RefreshRate = pstInfo->stDispInfo.u32RefreshRate;
	stTimingInfo.stChannelTiming[(HI_PQ_DISPLAY_E)pstWin->enDisp].bProgressive = pstInfo->stDispInfo.bInterlace ? HI_FALSE:HI_TRUE;
    stTimingInfo.stChannelTiming[(HI_PQ_DISPLAY_E)pstWin->enDisp].enOutColorSpace = pstInfo->stDispInfo.eColorSpace;


	if ( (HI_DRV_DISPLAY_1 == pstWin->enDisp )
		&& ((pstInfo->stDispInfo.eFmt == HI_DRV_DISP_FMT_1440x576i_50) 
		|| (pstInfo->stDispInfo.eFmt == HI_DRV_DISP_FMT_1440x480i_60))
		)
	{
	    stTimingInfo.stVideo.s32X      = pstLayerPara->stVideo.s32X * 2;
	    stTimingInfo.stVideo.s32Y      = pstLayerPara->stVideo.s32Y;
		stTimingInfo.stVideo.s32Width  = pstLayerPara->stVideo.s32Width * 2;    
	    stTimingInfo.stVideo.s32Height = pstLayerPara->stVideo.s32Height;
		
		stTimingInfo.stChannelTiming[(HI_PQ_DISPLAY_E)pstWin->enDisp].stFmtRect.s32Width *= 2;
	}
	else
	{
	...
	//printk("pq--para--out-->(%d,%d),video ->(%d,%d)\n",pstAttr->stOutRect.s32Width,pstAttr->stOutRect.s32Height,pstLayerPara->stVideo.s32Width,pstLayerPara->stVideo.s32Height);
                               ...
                               WIN_HAL_PARA_S *pstLayerPara)
{
    ...
    (HI_VOID)Win_PqProcess(pstWin, pstFrame,pstLayerPara, pstInfo);
...
HI_VOID ISR_WinConfigFrameNormal(WINDOW_S *pstWin, HI_DRV_VIDEO_FRAME_S *pstFrame, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    /*here we delete all the rwzb content, just rebuild it  in 98cv200 SB.*/
                                    ...
                                    const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    if (pstFrame == HI_NULL)
    { 
        /* when last work mode is HDR,go through HDR mute path;
         * otherwise,just go through normal mute path.
        */
    ...
    else if ((HI_DRV_VIDEO_FRAME_TYPE_SDR == pstFrame->enSrcFrameType) ||
             (pstWin->stUsingAttr.bCloseHdrPath))
    {
        ISR_WinConfigFrameNormal(pstWin, pstFrame, pstInfo);
    }
    else if ((HI_DRV_VIDEO_FRAME_TYPE_HDR10 == pstFrame->enSrcFrameType) ||
             (HI_DRV_VIDEO_FRAME_TYPE_DOLBY_BL == pstFrame->enSrcFrameType))
    {
        /* NOTE:HDR frame will config in DISP_ISR_SourceProcess function.
         * If now support HDR ,just go through normal path instead of HDR path.
         */
    ...
    {
        WIN_ERROR("NOT Support frame type.\n");
    }
