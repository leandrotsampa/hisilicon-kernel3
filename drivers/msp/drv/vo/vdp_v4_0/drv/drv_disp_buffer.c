...
HI_VOID BP_SetFrameTunnulAddr(HI_U32 *pu32TunnelAddr, DISP_MMZ_BUF_S *pstMem)
{
	...
	if (pstMem->u32Size % DISP_CAST_LOWDELAY_MEM_LENGTH)
	{
		DISP_ERROR("Cast size not 0x%x aligned!\n",DISP_CAST_LOWDELAY_MEM_LENGTH);		 
	}
	
	u32OffsetAddr = pstMem->u32StartPhyAddr + pstMem->u32Size - DISP_CAST_LOWDELAY_MEM_LENGTH;
		
#ifdef HI_SMMU_SUPPORT
	*pu32TunnelAddr = DISP_OS_SmmuToPhyAddr(u32OffsetAddr);
#else
	*pu32TunnelAddr = u32OffsetAddr;
#endif
...
HI_S32 BP_Create(HI_U32 u32BufNum, BUF_ALLOC_S *pstAlloc, BUF_POOL_S *pstBP)
{
    ...
    if (pstBP->enMemType == BUF_MEM_FB_SUPPLY)
    {
		...
		BufSize = ((BufSize + DISP_CAST_LOWDELAY_MEM_LENGTH - 1) /DISP_CAST_LOWDELAY_MEM_LENGTH + 1)
			      * DISP_CAST_LOWDELAY_MEM_LENGTH;
...
HI_S32 BP_Reset(BUF_POOL_S *pstBP)
{
    ...
    for (u = 0; u < pstBP->u32BufNum; u++)
    {
        ...
        if ( (pstBP->enMemType == BUF_MEM_FB_SUPPLY) || (pstBP->enMemType == BUF_MEM_USER_SUPPLY))
        {
            ...
            BP_SetFrameTunnulAddr(&pstBP->pstBufQueue[u].stFrame.u32TunnelPhyAddr,
			                      &pstBP->pstBufQueue[u].stMem);
...
HI_S32 BP_ReAllocBuf(BUF_POOL_S *pstBP, HI_U32 u32BufId)
{
    ...
    if ( (pstBP->enMemType == BUF_MEM_FB_SUPPLY) || (pstBP->enMemType == BUF_MEM_USER_SUPPLY))
    {
        ...
        BP_SetFrameTunnulAddr(&pstBP->pstBufQueue[index].stFrame.u32TunnelPhyAddr,
			                  &pstBP->pstBufQueue[index].stMem);
...
HI_S32 BP_CreateBlackFrame(HI_VOID)
{
	...
	nRet = DISP_OS_MMZ_UnMap(&s_stBlackFrame.stMem);
	if (HI_SUCCESS != nRet)
	{
		DISP_ERROR("UnMap black frame memory failed\n");
		DISP_OS_MMZ_Release(&s_stBlackFrame.stMem);
		return HI_FAILURE;
	}
