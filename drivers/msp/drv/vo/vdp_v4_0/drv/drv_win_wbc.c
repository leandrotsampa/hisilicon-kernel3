...
HI_DISP_WBC_S stWbcInfor   = {0};
DISP_MMZ_BUF_S stMBuf_WBC  = {0};
DISP_MMZ_BUF_S stMBuf_Secure_WBC  = {0};
Linker_INFOR_S g_stLinker  = {0};
...
extern HI_VOID VDP_DISP_GetVtThd(HI_U32 u32hd_id, HI_U32 u32uthdnum, HI_U32 *pu32vtthd);


HI_VOID WinSd_GetPrivSecAddr(HI_DRV_VIDEO_FRAME_S *pstFrame,HI_U32 *pu32PrivSecAddr)
{
	HI_DRV_VIDEO_PRIVATE_S *pstFramePrivate = HI_NULL;
	WBC_FRAME_PRIVATE_S  *pstWbcFramePrivate = HI_NULL;
	
    pstFramePrivate = (HI_DRV_VIDEO_PRIVATE_S *)pstFrame->u32Priv;
    pstWbcFramePrivate =  (WBC_FRAME_PRIVATE_S  *)(&pstFramePrivate->u32Reserve[0]);
    *pu32PrivSecAddr = pstWbcFramePrivate->u32SecureAddr;
}

HI_VOID WinSd_SetPrivSecAddr(HI_DRV_VIDEO_FRAME_S *pstFrame,HI_U32 u32PrivSecAddr)
{
	HI_DRV_VIDEO_PRIVATE_S *pstFramePrivate = HI_NULL;
	WBC_FRAME_PRIVATE_S  *pstWbcFramePrivate = HI_NULL;
    ...
    pstFramePrivate = (HI_DRV_VIDEO_PRIVATE_S *)pstFrame->u32Priv;
    pstWbcFramePrivate =  (WBC_FRAME_PRIVATE_S  *)(&pstFramePrivate->u32Reserve[0]);
    pstWbcFramePrivate->u32SecureAddr = u32PrivSecAddr;
}
...
HI_S32 Win_AllocateFrameBuf(HI_BOOL bAllocateFrameSize)
{
	...
	if (DISP_OS_MMZ_Alloc("VDP_WBC_V3",
					  VDP_MEM_TYPE_MMZ,
					  u32AllocateSize,
					  VDP_MEM_ALIGN,
					  &stMBuf_WBC))
...
#ifdef HI_TEE_SUPPORT
	{
        ...
        HI_DRV_SECSMMU_Alloc("VDP_WBC_V3_SE", 
                    u32AllocateSize,
                    VDP_MEM_ALIGN, 
                    &stTeeMem);
        stMBuf_Secure_WBC.bSmmu = HI_TRUE;
        stMBuf_Secure_WBC.pu8StartVirAddr = stTeeMem.pu8StartVirAddr;
        stMBuf_Secure_WBC.u32StartPhyAddr = stTeeMem.u32StartSmmuAddr;
        stMBuf_Secure_WBC.u32Size = stTeeMem.u32Size;
    }
#else
	memcpy(&stMBuf_Secure_WBC,&stMBuf_WBC,sizeof(DISP_MMZ_BUF_S));
#endif	
	return HI_SUCCESS;
}
...
HI_S32 WinWbc_Initialize(HI_VOID)
{
	...
	for (i = 0;i < WBC_BUF_NUM; i++)
	{
		...
		WinSd_SetPrivSecAddr(pstFrame_wbc[i],
								stMBuf_Secure_WBC.u32StartPhyAddr 
								+ i * WBC_MAX_STRIDE * WBC_RECT_HEIGHT * 2);
	...
	Win_SetWbcInitFlag(WBC_INITFLAG);
	...
	/*init the spinlock.*/
	spin_lock_init(&stWbcConfigInfor.spinlock_HardWareUse);	
...
HI_VOID WinWbc_Deinitialize(HI_VOID)
{
	...
	Win_SetWbcInitFlag(WBC_NOT_INIT_FLAG);	
	WinWbc_ResetBuffer();
		
	/*release the real frame mem.*/
	DISP_OS_MMZ_Release(&stMBuf_WBC);
	
	#ifdef HI_TEE_SUPPORT
	{
		SMMU_BUFFER_S stTeeMem;
        stTeeMem.pu8StartVirAddr = stMBuf_Secure_WBC.pu8StartVirAddr;
        stTeeMem.u32StartSmmuAddr = stMBuf_Secure_WBC.u32StartPhyAddr;
        stTeeMem.u32Size = stMBuf_Secure_WBC.u32Size;
        HI_DRV_SECSMMU_Release(&stTeeMem); 
	}
	#endif
	memset((HI_VOID*)&stMBuf_WBC, 0, sizeof(DISP_MMZ_BUF_S));
	
	/*release the  frame struct node.*/
	for (i = 0; i < WBC_BUF_NUM; i++)
	{	
		DISP_FREE(pstFrame_wbc[i]);
		pstFrame_wbc[i] = HI_NULL;
	}
...
HI_VOID Win_SetWbcInitFlag(WBC_INIT_FLAG eInitalizeFlag)
{
	stWbcConfigInfor.eInitalizeFlag = eInitalizeFlag;
	return;
}
...
HI_S32 WinLinker_GetNewNodeToWrite(HI_BOOL bLowDelayMode)
{
	...
	else
	{
		...
		s32Ret = DispBuf_GetEmptyNode(&stWbcConfigInfor.stWinBP.stBuffer,
		...
		s32Ret = DispBuf_DelEmptyNode(&stWbcConfigInfor.stWinBP.stBuffer,
									  stWbcConfigInfor.stWinBP.pstConfig);
...
HI_VOID  WinHd_Decide_WbcInputOutput_FieldMode(HI_DRV_FIELD_MODE_E *peWbcInFieldMode_Out)
{
	...
	{		
        ...
        {
            WIN_ERROR("stFunction.FP_GetChnBottomFlag is NULL!\n");
            return ;
        }
...
HI_S32 WinHd_GenarateFrameInfor(HI_DRV_VIDEO_FRAME_S **pstConfigFrame,
...
/*the current scene will take effect in wbc's policy.*/
HI_BOOL  Wbc_CheckComplexScene(HI_U32 u32WinNum)
{	
	if (u32WinNum >= 2)
	{
		return HI_TRUE;
	}
	else if (0) /*water mark*/
	{
		return HI_TRUE;
	}
...
/*to set the wbc's infor.*/
HI_VOID Wbc_GatherWbcInfor(WIN_WBC_CONFIG_S *pstWbcConfigInfor,
                           HI_RECT_S *pstHdWinInputSize,
							HI_RECT_S *pstHdWinOutputSize,
							HI_RECT_S *pstSdWinOutputSize,
							HI_BOOL   bLowdelayMode,
							HI_U32    u32WinNum)
{
	/*give v0's input/output value and v3's output value.*/
	pstWbcConfigInfor->stHdWinInputSize = *pstHdWinInputSize;
	pstWbcConfigInfor->stHdWinOutputSize = *pstHdWinOutputSize;
	pstWbcConfigInfor->stSdWinOutputSize = *pstSdWinOutputSize;	

    /*when single buffer,  frame mode forbidden , we will give a revise to PQ. */
	if (bLowdelayMode)
	{		
		pstWbcConfigInfor->bForceFieldMode = HI_TRUE;
	}
	else
	{
		pstWbcConfigInfor->bForceFieldMode = HI_TRUE;
	...
	/*if complex scene, set wbc point to vp.*/
	if (Wbc_CheckComplexScene(u32WinNum))
	{
		pstWbcConfigInfor->bComplexMode = HI_TRUE;
	}
	else
	{
		pstWbcConfigInfor->bComplexMode = HI_FALSE;
	}
...
HI_S32  WinHd_GenarateWbcInfo(WINDOW_S *pstWin,
										...
										HI_U32 u32WinNum)
{
	HI_DRV_FIELD_MODE_E  eWbcFieldMode_Out = HI_DRV_FIELD_BUTT;
	HI_BOOL   bLowdelay = HI_TRUE;
	HI_RECT_S stSdEffectiveDispRect;	
	...
	/*if isr can not grab the lock, just leave the wbc alone. in this scene,
	  * no effective frame comes into being, so the wbc will be disabled.
	  */
	spin_lock_irqsave(&stWbcConfigInfor.spinlock_HardWareUse, irqflag);
	
	/*if wbc not initialized or wbc is working, just return.*/
	if ((HI_FALSE == Win_GetWbcInitFlag()) || (HI_FALSE == stWbcConfigInfor.stConfInfo.bWbcEnable))
	{
		spin_unlock_irqrestore(&stWbcConfigInfor.spinlock_HardWareUse, irqflag);
		return HI_SUCCESS;	
	}
	
	/*clear the wbc buffer effective flag.*/
	...
	(HI_VOID)Win_Get_WbcPointAndOutSize(pstHdWinOutputSize,
							   (HI_RECT_S *)&pstInfo->stDispInfo.stFmtResolution,
							   &pstDispSd->stFmtResolution,
							   &stSdEffectiveDispRect);

	/*according the input size etc,  calculate out the wbc's output policy. */
	...
	Wbc_GatherWbcInfor(&stWbcConfigInfor.stConfInfo, 
						pstHdWinInputSize,
						pstHdWinOutputSize,
						&stSdEffectiveDispRect,
						bLowdelay,
	...
	stWbcConfigInfor.stConfInfo.eOutPutPixelFmt = HI_DRV_PIX_FMT_NV61_2X1;	

	if (HI_DRV_CS_UNKNOWN == pstWin->stCfg.enFrameCS)
	{		
		stWbcConfigInfor.stConfInfo.enHdWinSrcColorSpace = HI_DRV_CS_BT709_YUV_LIMITED;
    }
	else
	{
		stWbcConfigInfor.stConfInfo.enHdWinSrcColorSpace = pstWin->stCfg.enFrameCS;
    }

	if (HI_DRV_CS_UNKNOWN == pstWin->stCfg.enOutCS)
	{
		stWbcConfigInfor.stConfInfo.enHdWinDestColorSpace = HI_DRV_CS_BT709_YUV_LIMITED;
    }
	else
	{
        stWbcConfigInfor.stConfInfo.enHdWinDestColorSpace = pstWin->stCfg.enOutCS;
    }
	
	
	/*get the output policy of wbc, only pass parts of the WIN_WBC_CONFIG_S,
	  *others are for config.*/
	pstFunc->PF_GetWbcPolicy(&stWbcConfigInfor.stConfInfo, &stWbcPolicy);
	WinHd_Decide_WbcInputOutput_FieldMode(&eWbcFieldMode_Out);
	
	/*get the field of wbc's output.*/
	stWbcConfigInfor.stConfInfo.eFieldMode    = stWbcPolicy.bWbcOutputFieldMode ? eWbcFieldMode_Out:HI_DRV_FIELD_ALL;
	
	/*genarate the frame infor according the given wbc's infor.
	 * this frame is for v3's configuration.
	 */
	s32Ret = WinHd_GenarateFrameInfor(&pstConfigFrame,
	...
	if (HI_SUCCESS != s32Ret)
	{
		WIN_ERROR("Genarate frameinfor failed!\n");
		spin_unlock_irqrestore(&stWbcConfigInfor.spinlock_HardWareUse, irqflag);
		return HI_FAILURE;
	}
	
	if (stWbcPolicy.bWbcOutputFieldMode == HI_TRUE)
	{
		pstConfigFrame->bProgressive = HI_FALSE;
	}
	else
	{
		pstConfigFrame->bProgressive = HI_TRUE;
	}
		
	/*update the wbc's config infor.*/
	pstSlvWin->stUsingAttr.stOutRect = stWbcPolicy.stWbcOutputSize;	
...
{
	DISP_INTF_OPERATION_S stFunction = {0};
	HI_DRV_VID_FRAME_ADDR_S stAddr = {0};
	...
	if ((pstWbcLinker->stConfInfo.bWbcEnable == HI_FALSE)  || (HI_NULL == pstWbcLinker->pstCurrentFrame))
	{
		WIN_WBC_CONFIG_S st_tmpWbcConfig;

		/*when come here, set the tmp wbcConfInfor disabled.*/
		memcpy((HI_VOID*)&st_tmpWbcConfig, &pstWbcLinker->stConfInfo, sizeof(WIN_WBC_CONFIG_S));
	...
	if (pstWbcLinker->pstCurrentFrame->bSecure)
	{
        HI_U32 u32YSize;
		HI_U32 u32Yaddr;
		
        u32YSize = pstWbcLinker->pstCurrentFrame->stBufAddr[0].u32PhyAddr_C
                -pstWbcLinker->pstCurrentFrame->stBufAddr[0].u32PhyAddr_Y;
		WinSd_GetPrivSecAddr(pstWbcLinker->pstCurrentFrame,&u32Yaddr);	
        
        stAddr.u32PhyAddr_Y  =  u32Yaddr;
        stAddr.u32PhyAddr_C  =  stAddr.u32PhyAddr_Y + u32YSize;
	...
	pstWbcLinker->stConfInfo.stAddr = stAddr;
	...
	pstFunc->PF_SetWbcParam(&pstWbcLinker->stConfInfo);
...
HI_VOID WIN_Start_Wbc(HI_VOID)
{
	...
	spin_lock_irqsave(&stWbcConfigInfor.spinlock_HardWareUse, irqflag);
	stWbcConfigInfor.stConfInfo.bWbcEnable  = HI_TRUE;
	spin_unlock_irqrestore(&stWbcConfigInfor.spinlock_HardWareUse, irqflag);
...
HI_VOID WIN_Stop_Wbc(HI_VOID)
{
	...
	/*to get the lock to deinit and disable the wbc.
	  * if get lock here, it means that the isr func can't not open the wbc after operations below.
	  * if isr func grap the lock, this func will  spin until isr give up. 
	  */
	spin_lock_irqsave(&stWbcConfigInfor.spinlock_HardWareUse, irqflag);
	
	stWbcConfigInfor.stConfInfo.bWbcEnable  = HI_FALSE;
	
	spin_unlock_irqrestore(&stWbcConfigInfor.spinlock_HardWareUse, irqflag);
	
    pstFunc->PF_SetWbcParam(&stWbcConfigInfor.stConfInfo);	
    Win_SetSingleBufMode_WbcEffectiveFlag (HI_FALSE);	
	return;
}
...
{
	return stWbcConfigInfor.stConfInfo.bWbcEnable;
...
{	
	...
	/*for wbc producer,  insert a  frame into it.*/
	nRet = DispBuf_GetFullNode(&pstWinBP->stBuffer, &pstWinBP->pstConfig);
    if(HI_SUCCESS != nRet)
    {
        return HI_NULL;
    }
	...
	{		
		...
		nRet = DispBuf_DelFullNode(&pstWinBP->stBuffer, pstWinBP->pstConfig);
...
{	
	...
	else if (pstFieldMatch)
	{
		...
		while (!DispBuf_GetFullNode(&pstWinBP->stBuffer,&pstCurrentNode))
		{
			...
			s32Ret = DispBuf_DelFullNode(&pstWinBP->stBuffer, pstCurrentNode);
            DISP_ASSERT(s32Ret == HI_SUCCESS);
...
HI_DRV_VIDEO_FRAME_S * WinSd_Search_NewestFrame(WINDOW_S *pstWin)
{
	...
	{		
		s32Ret = DispBuf_DelFullNode(&pstWin->stBuffer.stWinBP.stBuffer, pstCurrentNode);
        DISP_ASSERT(s32Ret == HI_SUCCESS);
...
HI_DRV_VIDEO_FRAME_S g_stTmpSdSecFrm;
HI_VOID ISR_WinConfigFrameSlave(WINDOW_S *pstWin, HI_DRV_VIDEO_FRAME_S *pstFrame, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
	...
	stLayerPara.bDolbyMode = (pstWin->enWinMode == HI_DRV_WINDOW_MODE_DOLBY) ? HI_TRUE:HI_FALSE;
	stLayerPara.en3Dmode = pstInfo->stDispInfo.eDispMode;
	stLayerPara.bRightEyeFirst = pstInfo->stDispInfo.bRightEyeFirst;

    memcpy(&g_stTmpSdSecFrm,pstFrame,sizeof(HI_DRV_VIDEO_FRAME_S));
	
    if (g_stTmpSdSecFrm.bSecure)
    {
		HI_U32 u32Ysize;
		HI_U32 u32Yaddr;
		
        u32Ysize = g_stTmpSdSecFrm.stBufAddr[0].u32PhyAddr_C 
                - g_stTmpSdSecFrm.stBufAddr[0].u32PhyAddr_Y;
		WinSd_GetPrivSecAddr(&g_stTmpSdSecFrm,&u32Yaddr);	
					
        g_stTmpSdSecFrm.stBufAddr[0].u32PhyAddr_Y = u32Yaddr;
        g_stTmpSdSecFrm.stBufAddr[0].u32PhyAddr_C = u32Yaddr + u32Ysize;           
    }
		
	stLayerPara.pstFrame = &g_stTmpSdSecFrm;
	...
	stLayerPara.eOutPutColorSpace = pstInfo->stDispInfo.eColorSpace;
	stLayerPara.pstDispInfo = (HI_DISP_DISPLAY_INFO_S *)&(pstInfo->stDispInfo);
	
    (HI_VOID)pstWin->stVLayerFunc.PF_SetFramePara(pstWin->u32VideoLayer, &stLayerPara);

	(HI_VOID)Win_PqProcess(pstWin, pstFrame,&stLayerPara, pstInfo);
												...
												const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
	...
	{
        WIN_WARN("stFunction.PF_GetIsoWbcPartFinishPoint is NULL!\n");
        return ;
    }
	...
	if (pstFrame)
		ISR_WinConfigFrameSlave(pstWin, pstFrame,pstInfo);	
