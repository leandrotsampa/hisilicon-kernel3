...
HI_S32 HIFB_DRV_QuickCopy(HIFB_BUFFER_S *pSrcImg, HIFB_BUFFER_S *pDstImg)
{
    ...
    s32Ret = ps_TdeExportFuncs->pfnTdeQuickCopy(handle,&stSrcSur,&stSrcRect,&stDstSur,&stDstRect);
    if(s32Ret != HI_SUCCESS){
    ...
    s32Ret = ps_TdeExportFuncs->pfnTdeEndJob(handle,HI_TRUE,100,HI_FALSE,HI_NULL,HI_NULL);
    if(s32Ret != HI_SUCCESS){
...
HI_S32 HIFB_DRV_QuickCopyEx(HIFB_BUFFER_S *pSrcImg, HIFB_BUFFER_S *pDstImg)
{
    ...
    s32Ret = ps_TdeExportFuncs->pfnTdeQuickCopyEx(handle,&stSrcSur,&stSrcRect,&stDstSur,&stDstRect,HI_TRUE,HI_FALSE);
    if(s32Ret != HI_SUCCESS){
        HIFB_ERROR("tde quick copy failed\n");    
        ps_TdeExportFuncs->pfnTdeCancelJob(handle);
        return s32Ret;
...
HI_VOID HIFB_DRV_WaitAllTdeDone(HI_BOOL bSync)
{
    ...
    ps_TdeExportFuncs->pfnTdeWaitAllDone(bSync);
}
...
{
	Ops->HIFB_DRV_QuickCopy      = HIFB_DRV_QuickCopy;
    Ops->HIFB_DRV_QuickCopyEx    = HIFB_DRV_QuickCopyEx;
