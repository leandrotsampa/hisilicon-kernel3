...
{
	HI_U32 *regAddr = CIPHER_MMU_GLOBAL_CTR_ADDR;
	HI_U32 regValue = 0;

    (HI_VOID)HAL_CIPHER_ReadReg(regAddr, &regValue);
    regValue &= ~0x01; //Global bypass disable
    regValue |=  0x08; //Init enable
    (HI_VOID)HAL_CIPHER_WriteReg(regAddr, regValue);
	return;
}
...
HI_S32 HAL_CIPHER_ProcGetStatus(CIPHER_CHN_STATUS_S *pstCipherStatus)
{
	...
	/* get cipher ctrl */
		if(0 != i)
		{
			pu32RegAddr = CIPHER_REG_CHANn_CIPHER_CTRL(i);
		}
		else
		{
			pu32RegAddr = CIPHER_REG_CHAN0_CIPHER_CTRL;
		}
		...
		if(0 != i)
		{
		    pu32RegAddr = CIPHER_REG_CHANn_SRC_LST_SADDR(i);
		    (HI_VOID)HAL_CIPHER_ReadReg(pu32RegAddr, &u32RegValue);
			pstCipherStatus[i].u32DataInAddr = u32RegValue;
		}
		else
		{
			pstCipherStatus[0].u32DataInAddr = CIPHER_CIPHER_REG_BASE_ADDR_PHY + 0x1014;
		}

		/* get data out */
		if(0 != i)
		{
		    pu32RegAddr = CIPHER_REG_CHANn_DEST_LST_SADDR(i);
		    (HI_VOID)HAL_CIPHER_ReadReg(pu32RegAddr, &u32RegValue);
			pstCipherStatus[i].u32DataOutAddr = u32RegValue;
		}
		else
		{
			pstCipherStatus[0].u32DataOutAddr = CIPHER_CIPHER_REG_BASE_ADDR_PHY + 0x0000;
		}

	/* get INT RAW status */
	    pu32RegAddr = CIPHER_REG_INT_RAW;
	    (HI_VOID)HAL_CIPHER_ReadReg(pu32RegAddr, &u32RegValue);
		pstCipherStatus[i].bInINTRaw = ( u32RegValue >> i ) & 0x1;
		pstCipherStatus[i].bOutINTRaw = ( u32RegValue >> (i + 8) ) & 0x1;

	/* get INT EN status */
	    pu32RegAddr = CIPHER_REG_INT_EN;
	    (HI_VOID)HAL_CIPHER_ReadReg(pu32RegAddr, &u32RegValue);
	...
	/* get INT_OINTCFG */
		pu32RegAddr = CIPHER_REG_CHANn_INT_OCNTCFG(i);
	    (HI_VOID)HAL_CIPHER_ReadReg(pu32RegAddr, &u32RegValue);
