...
//when have not finded sync ,return pu32Pos = (u32Bytes/2-1)
HI_VOID AUTIL_IEC61937_FindSync(HI_U16 *pu16Buf, HI_U32 u32Bytes, HI_BOOL bDts, HI_U32 *pu32Pos, HI_BOOL *pbDtscd)
{
    HI_U32 i;

    for (i = 0; i < (u32Bytes / 2 - 1); i++)
    {
        if ((IEC_61937_SYNC1 == pu16Buf[i]) && (IEC_61937_SYNC2 == pu16Buf[i + 1]))
        {
            *pbDtscd = HI_FALSE;
            *pu32Pos = i;
            return;
    ...
    if ((HI_TRUE == bDts) && ((u32Bytes / 2 - 1) == i))
    {
        for (i = 0; i < (u32Bytes / 2 - 1); i++)
        {
            if ((((HI_U16)(DCA_MARKER_RAW_BE >> 16) == pu16Buf[i]) && ((HI_U16)((DCA_MARKER_RAW_BE << 16) >> 16) == pu16Buf[i + 1])) ||
                (((HI_U16)(DCA_MARKER_RAW_LE >> 16) == pu16Buf[i]) && ((HI_U16)((DCA_MARKER_RAW_LE << 16) >> 16) == pu16Buf[i + 1])) ||
                (((HI_U16)(DCA_MARKER_14B_BE >> 16) == pu16Buf[i]) && ((HI_U16)((DCA_MARKER_14B_BE << 16) >> 16) == pu16Buf[i + 1])) ||
                (((HI_U16)(DCA_MARKER_14B_LE >> 16) == pu16Buf[i]) && ((HI_U16)((DCA_MARKER_14B_LE << 16) >> 16) == pu16Buf[i + 1])))
            {
                *pbDtscd = HI_TRUE;
                break;
            }
...
#else
    enChipPlatform = AUTIL_CHIP_PLATFORM_S40;
#endif
