...
#if defined (CHIP_TYPE_hi3751v600) || defined (CHIP_TYPE_hi3751lv500) ||  defined (CHIP_TYPE_hi3751av500) // only support dsp1
                    uTmpVal.u32 = g_pstRegPeri->PERI_INT_DSP1TOA9.u32;
                    uTmpVal.bits.int_swi_dsp1toa9 = 1;
                    g_pstRegPeri->PERI_INT_DSP1TOA9.u32 = uTmpVal.u32;
#else  //only need a interrupt, either dsp0 or dsp1, stb only support dsp0
...
#endif
        Ret = iHAL_AIAO_P_Open(ID2PORT(Id), pstConfig, &pstPort, &pisr);

        if (HI_SUCCESS == Ret)
        {
            g_AIAORm.pPort[Id]   = (HI_VOID*)pstPort;
            g_AIAORm.fRegIsr[Id] = pisr;
            g_AIAORm.sub[Id] = pstConfig->pSubstream;////g_AIAORm.sub[Id] for alsa isr       HI_ALSA_AI_SUPPORT
#if defined (HI_I2S0_SUPPORT) || defined (HI_I2S1_SUPPORT)
            ...
            if ((AIAO_PORT_RX2 == enPortID) || (AIAO_PORT_TX3 == enPortID) || (AIAO_PORT_TX4 == enPortID) || (AIAO_PORT_TX5 == enPortID))
            {
...
#if 0 //defined (CHIP_TYPE_hi3751v600) || defined (CHIP_TYPE_hi3751lv500) || defined (CHIP_TYPE_hi3751av500)

            if (((AIAO_PORT_RX3 == enPortID) && (AIAO_TX_CRG2 == pstConfig->stIfAttr.eCrgSource))\
                || ((AIAO_PORT_TX6 == enPortID) && (AIAO_TX_CRG2 == pstConfig->stIfAttr.eCrgSource))\
                || (AIAO_PORT_TX3 == enPortID)\
                || (AIAO_PORT_TX4 == enPortID)\
                || (AIAO_PORT_TX5 == enPortID))
            {
                Ret = iHAL_AIAO_P_SetI2SMasterClk(AIAO_PORT_TX2, ( AIAO_IfAttr_S*)&pstConfig->stIfAttr);
            }

            if ((AIAO_PORT_TX6 == enPortID) && (AIAO_TX_CRG1 == pstConfig->stIfAttr.eCrgSource))
            {
                Ret = iHAL_AIAO_P_SetI2SMasterClk(AIAO_PORT_TX1, ( AIAO_IfAttr_S*)&pstConfig->stIfAttr);
            }


            if ((AIAO_PORT_RX3 == enPortID) && (AIAO_RX_CRG1 == pstConfig->stIfAttr.eCrgSource))
            {
                Ret = iHAL_AIAO_P_SetI2SlaveClk(AIAO_PORT_RX1, ( AIAO_IfAttr_S*)&pstConfig->stIfAttr);
            }

            if ((AIAO_PORT_RX3 == enPortID) && (AIAO_RX_CRG4 == pstConfig->stIfAttr.eCrgSource))
            {
                Ret = iHAL_AIAO_P_SetI2SlaveClk(AIAO_PORT_RX4, ( AIAO_IfAttr_S*)&pstConfig->stIfAttr);
            }
...
HI_S32 HAL_AIAO_P_SetAttr(AIAO_PORT_ID_E enPortID, AIAO_PORT_ATTR_S* pstAttr)
{
    ...
    if (g_AIAORm.pPort[Id])
    {
        Ret = iHAL_AIAO_P_SetAttr((AIAO_PORT_S)g_AIAORm.pPort[Id], pstAttr);
    }
...
HI_S32 HAL_AIAO_P_GetAttr(AIAO_PORT_ID_E enPortID, AIAO_PORT_ATTR_S* pstAttr)
{
    ...
    if (g_AIAORm.pPort[Id])
    {
        Ret = iHAL_AIAO_P_GetAttr((AIAO_PORT_S)g_AIAORm.pPort[Id], pstAttr);
    }
...
HI_S32 HAL_AIAO_P_SetBypass(AIAO_PORT_ID_E enPortID, HI_BOOL bByBass)
{
    ...
    if (g_AIAORm.pPort[Id])
    {
        Ret = iAIAO_HAL_P_SetBypass((AIAO_PORT_S)g_AIAORm.pPort[Id], bByBass);
    }
...
HI_S32 HAL_AIAO_P_SetMode(AIAO_PORT_ID_E enPortID, HI_BOOL bMode)
{
    ...
    if (g_AIAORm.pPort[Id])
    {
        Ret = iAIAO_HAL_P_SetMode((AIAO_PORT_S)g_AIAORm.pPort[Id], bMode);
    }
