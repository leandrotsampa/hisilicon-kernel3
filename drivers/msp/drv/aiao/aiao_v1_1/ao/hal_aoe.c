...
HI_S32  HAL_AOE_AIP_SetDre(AOE_AIP_ID_E enAIP, HI_BOOL bDre)
{
    AOE_AIP_CHN_STATE_S* state = HI_NULL;
    ...
    iHAL_AOE_AIP_SetDre(enAIP, bDre);
...
HI_U32 HAL_AOE_AIP_ChangeReadPos(AOE_AIP_ID_E enAIP, HI_U32 u32RptrLen)
{
	...
	ret = CIRC_BUF_ChangeRptr(&state->stCB, u32RptrLen);
...
typedef struct
{
    ...
    HI_BOOL               bBypass;//TODO
    HI_U32                u32DelayMs;
} AOE_AOP_CHN_STATE_S;

HI_S32 AOECheckFreeAOP(AOE_AOP_ID_E enAOP)
{
    if (g_AoeRm.pAop[enAOP])
    {
        return HI_FAILURE;
    }
    return HI_SUCCESS;
...
HI_S32  HAL_AOE_AOP_SetDelay(AOE_AOP_ID_E enAOP, HI_U32 u32RealDelayMs)
{
    ...
    HI_INFO_AO("Set Aop(%x) DelayMs %d\n", enAOP, u32RealDelayMs);
    return HI_SUCCESS;
}
...
HI_S32  HAL_AOE_AOP_GetDelay(AOE_AOP_ID_E enAOP, HI_U32* pu32RealDelayMs)
{
    ...
    *pu32RealDelayMs = state->u32DelayMs;

    HI_INFO_AO("Get DelayMs %d\n",  *pu32RealDelayMs);
    return HI_SUCCESS;
}
#endif
...
HI_S32  HAL_AOE_AOP_SetAttr(AOE_AOP_ID_E enAOP, AOE_AOP_CHN_ATTR_S* pstAttr)
{
    ...
    if (enAOP)
        pstAttr->bRbfHwPriority = HI_TRUE;
#endif
...
HI_S32 HAL_AOE_AOP_SetLowLatency(AOE_AOP_ID_E enAOP, HI_U32 u32Latency)
{
    ...
    CHECK_AOP_OPEN(enAOP);
    state = (AOE_AOP_CHN_STATE_S*)g_AoeRm.pAop[enAOP];
#ifndef HI_SOUND_SPDIF_COMPENSATION_SUPPORT
    if(state->stUserAttr.stRbfOutAttr.u32BufDataFormat != 0)
    {
        HI_WARN_AO("Passthrough not set low latency\n");
        return HI_SUCCESS;
    }
#endif

    Ret = iHAL_AOE_AOP_SetLatency(enAOP, u32Latency);
    if (HI_SUCCESS != Ret)
    {
        return Ret;
    }
...
HI_S32 HAL_AOE_AOP_GetLowLatency(AOE_AOP_ID_E enAOP, HI_U32 *pu32Latency)
{
    ...
    Ret = iHAL_AOE_AOP_GetLatency(enAOP, pu32Latency);
    if (HI_SUCCESS != Ret)
    {
        return Ret;
    }
