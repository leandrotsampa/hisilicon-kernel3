...
HI_S32                  iHAL_AOE_AIP_SetAttr(AOE_AIP_ID_E enAIP, AOE_AIP_CHN_ATTR_S* pstAttr)
{
    ...
    //set  AIP_BUFF_ATTR
    u32DataFormat = pstAttr->stBufInAttr.u32BufDataFormat;
    AipReg->AIP_BUFF_ATTR.bits.aip_format = 
    ...
    AipReg->AIP_BUFF_ATTR.bits.aip_pcm_type = pstAttr->stBufInAttr.eAipType;

    if (HI_TRUE == g_bSwAoeFlag)
    {
        ADSP_WriteAddr(pstAttr->stBufInAttr.stRbfAttr.tBufVirAddr, AipReg->AIP_BUF_ADDR);
        
        AipReg->AIP_BUF_PHYADDR = pstAttr->stBufInAttr.stRbfAttr.tBufPhyAddr; //for mamp track phy addr to user
    }
    else
    {
        HI_PHYS_ADDR_T u32DspRemapAddr = pstAttr->stBufInAttr.stRbfAttr.tBufPhyAddr;
        ...
        AipReg->AIP_BUF_PHYADDR = u32DspRemapAddr;  //same as AIP_BUF_ADDR for mamp track phy addr to user
    }
    ...
    if (pstAttr->stBufInAttr.stRbfAttr.u32BufWptrRptrFlag)
    {
        if (HI_TRUE == g_bSwAoeFlag)
        {
            ADSP_WriteAddr(pstAttr->stBufInAttr.stRbfAttr.tBufVirWptr, AipReg->AIP_BUF_WPTR);
            ADSP_WriteAddr(pstAttr->stBufInAttr.stRbfAttr.tBufVirRptr, AipReg->AIP_BUF_RPTR);
        }
        else
        {
            HI_PHYS_ADDR_T u32DspRemapAddr = pstAttr->stBufInAttr.stRbfAttr.tBufPhyWptr;
#if defined(DSP_DDR_DMAREMAP_SUPPORT)
            if ((u32DspRemapAddr >= DSP_DDR_DMAREMAP_BEG_ADDR) && (u32DspRemapAddr < DSP_DDR_DMAREMAP_END_ADDR))
            {
                u32DspRemapAddr += DSP_DDR_DMAREMAP_MAP_ADDR;
            ...
            u32DspRemapAddr = pstAttr->stBufInAttr.stRbfAttr.tBufPhyRptr;
#if defined(DSP_DDR_DMAREMAP_SUPPORT)
            if ((u32DspRemapAddr >= DSP_DDR_DMAREMAP_BEG_ADDR) && (u32DspRemapAddr < DSP_DDR_DMAREMAP_END_ADDR))
            {
                u32DspRemapAddr += DSP_DDR_DMAREMAP_MAP_ADDR;
    ...
    AipReg->AIP_FIFO_ATTR.bits.fifo_format = 
        (0 == u32DataFormat) ? AOE_FORMAT_LPCM : ((u32DataFormat & 0xff00) ? AOE_FORMAT_IEC61937_COMP : AOE_FORMAT_IEC61937);
...
{
    //HI_ERR_AO("enAIP = %d, RegAddr = %#x\n", (HI_U32)enAIP, ((AOE_COM_REG_BASE + AOE_AIP_REG_OFFSET) + AOE_AIP_REG_BANDSIZE * (HI_U32)enAIP));
...
HI_S32 iHAL_AOE_AIP_SetCmd(AOE_AIP_ID_E enAIP, AOE_AIP_CMD_E newcmd)
{
    ...
    if (1)
#else
    if (AOE_AIP_TYPE_PCM_ALSA != (AOE_AIP_TYPE_PCM_E)(AipReg->AIP_BUFF_ATTR.bits.aip_pcm_type))
#endif
    {
...
HI_VOID iHAL_AOE_AIP_SetDre(AOE_AIP_ID_E enAIP, HI_BOOL bDre)
{
    S_AIP_REGS_TYPE* AipReg = (S_AIP_REGS_TYPE*)g_pAipReg[enAIP];

    AipReg->AIP_EXT_CTRL.bits.dre = (HI_U32)bDre;
    AipReg->AIP_EXT_CTRL.bits.dre_thrhd = 0x100 - 0x20;
