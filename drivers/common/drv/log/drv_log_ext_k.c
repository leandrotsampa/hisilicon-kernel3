...
HI_VOID HI_LogOut(HI_U32 u32Level, HI_MOD_ID_E enModId, HI_U8 *pFuncName, HI_U32 u32LineNum, const char *format, ...)
{
    ...
    if (bLevel)
    {
        ...
        #ifndef ANDROID
            HI_U32  TimeMs      = LOGGetTimeMs() % HI_DAY;
            HI_U32  Hour        = TimeMs / HI_HOUR;
            HI_U32  Minute      = (TimeMs % HI_HOUR) / HI_MINUTE;
            HI_U32  Second      = ((TimeMs % HI_HOUR) % HI_MINUTE) / HI_SECOND;
            HI_U32  Millisecond = TimeMs % HI_SECOND;
        #endif
